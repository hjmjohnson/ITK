...
macro(itk_module_link_dependencies)
  ...
  foreach(dep IN LISTS ITK_MODULE_${itk-module}_PUBLIC_DEPENDS)
    if(DEFINED ${dep}_LIBRARIES)
      ...
      message(STATUS "MODULE_DEPENDANCIES:       ${itk-module} requires :${${dep}_LIBRARIES}: with LINK_PUBLIC")
    elseif(DEFINED ${dep})
      target_link_libraries(${itk-module} LINK_PUBLIC ${${dep}})
      message(STATUS "MODULE_DEPENDANCIES:       ${itk-module} requires :${${dep}}: with LINK_PUBLIC")
    else()
  ...
  foreach(dep IN LISTS ITK_MODULE_${itk-module}_PRIVATE_DEPENDS)
    if(DEFINED ${dep}_LIBRARIES)
      ...
      message(STATUS "MODULE_DEPENDANCIES:       ${itk-module} requires :${${dep}_LIBRARIES}: with LINK_PRIVATE")
    elseif(DEFINED ${dep})
      target_link_libraries(${itk-module} LINK_PRIVATE ${${dep}})
      message(STATUS "MODULE_DEPENDANCIES:       ${itk-module} requires :${${dep}}: with LINK_PRIVATE")
    else()
      message(FATAL_ERROR "Dependency \"${dep}\" not found: could not find [${dep}] or [${dep}_LIBRARIES]")
    endif()
  endforeach()
  #show_module_dependency_tree(${itk-module} "TOP_LEVEL" "TOP_LEVEL")
endmacro()

### DEBUG
macro(show_module_dependency_tree module from type)
  list(APPEND ALL_DEPENDS ${module})
  message(STATUS "${module} needed by ${from} (${type})")
  foreach(public_dep IN LISTS ITK_MODULE_${itk-module}_PUBLIC_DEPENDS)
    if(NOT ";${${public_dep}_needed_by};" MATCHES ";${module};")
      list(APPEND ${public_dep}_needed_by ${module})
      #list(REMOVE_DUPLICATES ${public_dep}_needed_by)
      show_module_dependency_tree(${public_dep} ${module} "Public")
    endif()
  endforeach()
  foreach(compile_dep IN LISTS ITK_MODULE_${itk-module}_COMPILE_DEPENDS)
    if(NOT ";${${compile_dep}_needed_by};" MATCHES ";${module};")
      list(APPEND ${compile_dep}_needed_by ${module})
      #list(REMOVE_DUPLICATES ${compile_dep}_needed_by)
      show_module_dependency_tree(${compile_dep} ${module} "Compile")
    endif()
  endforeach()
  foreach(private_dep IN LISTS ITK_MODULE_${itk-module}_PRIVATE_DEPENDS)
    if(NOT ";${${private_dep}_needed_by};" MATCHES ";${module};")
      list(APPEND ${private_dep}_needed_by ${module})
      #list(REMOVE_DUPLICATES ${private_dep}_needed_by)
      message(STATUS "${private_dep} needed by ${module} Private")
    endif()
  endforeach()
endmacro()
### END DEBUG
