...
main(int argc, char * argv[])
{
  ...
  ImageType::ConstPointer const inputImage = reader->GetOutput();

  ImageType::PointType           origin = inputImage->GetOrigin();
  ImageType::SpacingType         spacing = inputImage->GetSpacing();
  ImageType::DirectionType const direction = inputImage->GetDirection();

  if (argc > 3)
  {
    double const scale = std::stod(argv[3]);
  ...
  if (argc > 7)
  {
    double const additionalAngle = std::stod(argv[7]);

    itk::Versor<itk::SpacePrecisionType> rotation;
    double const angleInRadians = additionalAngle * itk::Math::pi / 180.0;
    rotation.SetRotationAroundZ(angleInRadians);

    ImageType::DirectionType const newDirection =
