...
main(int argc, char * argv[])
{
  ...
  ImageType::Pointer const localImage = reader->GetOutput();
  // Software Guide : EndCodeSnippet
  ...
  unsigned char const threshBelow = 0;
  unsigned char const threshAbove = 255;
  ...
  houghFilter->Update();
  AccumulatorImageType::Pointer const localAccumulator =
  ...
  OutputImageType::RegionType const region(
  ...
  while (itLines != lines.end())
  {
    ...
    double const norm = std::sqrt(v[0] * v[0] + v[1] * v[1]);
    v[0] /= norm;
    v[1] /= norm;
    // Software Guide : EndCodeSnippet
    ...
    for (auto i = static_cast<int>(-diag); i < static_cast<int>(diag); ++i)
    {
      ...
      OutputImageType::RegionType const outputRegion =
