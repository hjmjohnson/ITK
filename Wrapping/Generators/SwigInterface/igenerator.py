...
def remove_class_type(itkclass: str):
    for m in re.finditer("[A-Z_0-9]+$", itkclass):
        # Symbol IO and v4 are never used as types (there doesn't appear to be a class that ends and v and has type 4)
        if "IO" in itkclass:
...
'''
    ...
    def convert_cpp_to_python_value(self, cpp_value: str):
...
"""
    ...
    def generate_method(self, typedef, method, indent, w, is_operator=False):
        ...
        for argIndex in range(len(method.arguments)):
            ...
            if self.current_class is not None and not is_operator and method.name not in self.notWrappedMethods:
                python_arg_type = self.convert_cpp_to_python_type(method.argument_types[argIndex])
                name = arg.name
                if iskeyword(name):
                    name = "_" + name    # above names can not be used as a variable name in python

                # append the default value if it exists
                if arg.default_value:
                    # filter our invalid default values
                    # eventually it would be best to convert these defaults
                    # but for now unknown defaults can just be replaced with ...
                    method_hints.append((name, python_arg_type, self.convert_cpp_to_python_value(arg.default_value)))
                else:
        ...
        if self.current_class is not None and not is_operator and method.name not in self.notWrappedMethods:
            ...
            # last element in methods arg list will be the return type
            # the third tuple element that is normally reserved for the args default value will contain a boolean
            # that indicates if the method is static for this special final value.
            method_hints.append((None, return_type, method.has_static))

            if method.name not in self.classes[self.current_class].python_method_headers:
                self.classes[self.current_class].python_method_headers[method.name] = [method_hints]
            elif method_hints not in self.classes[self.current_class].python_method_headers[method.name]:
                self.classes[self.current_class].python_method_headers[method.name].append(method_hints)
...
"""
    ...
    def create_interfacefile(self, interfaceFile, idxFile, wrappersNamespace):
        ...
        for typedef in typedefs:
            ...
            if (typedef.name.endswith("_Pointer") or
                    typedef.name.endswith("Factory_Pointer") or
                    typedef.name.endswith("Factory") or
                    typedef.name.endswith("Enums") or
                    typedef.name.endswith("_Superclass")) and typedef.name != self.current_class:
            ...
            if self.current_class is not None and self.classes[self.current_class].parent_class == "":
                for base in getType(typedef).bases:
                    ...
                    # at this point if a : is in base it is std or some other class we have no refrence to
                    # just ignore it
                    if ":" in base:
                        continue
            ...
            # make Python class abstract if appropriate
            if self.current_class is not None and getType(typedef).is_abstract and not self.classes[self.current_class].is_abstract:
                self.classes[self.current_class].is_abstract = True

        for itkclass in self.classes.keys():
            self.generate_pyi_file(self.classes[itkclass])
    ...
    def generate_pyi_file(self, itkclass: ITKClass):

        class_name = itkclass.class_name
        if "itk" == class_name[:3]:
            class_name = class_name[3:]

        if not itkclass.is_abstract:
            self.outputPYIHeaderFile.write(self.generate_class_pyi_header(class_name,
                                                                          itkclass.has_new_method,
                                                                          itkclass.number_of_typedefs))
        self.outputPYIMethodFile.write(f"class {class_name}({itkclass.parent_class}):\n")
        if len(itkclass.python_method_headers.keys()) == 0:
            self.outputPYIMethodFile.write(f"\t...\n\n")

        for method in itkclass.python_method_headers.keys():
            is_overloaded = len(itkclass.python_method_headers[method]) > 1
            for method_variation in itkclass.python_method_headers[method]:
                method_name = method
                ...
                if is_overloaded:
                    self.outputPYIMethodFile.write("\t@overload\n")

                self.outputPYIMethodFile.write(f"\tdef {method_name}({self_str}{params})")
                if return_type is not None:  # If Python return type is unknown, function may or may not return
                    self.outputPYIMethodFile.write(f" -> {return_type}")
                self.outputPYIMethodFile.write(f":\n"
                                               f"\t\t\"\"\"\"\"\"\n"
                                               f"\t\t...\n\n")
    ...
    def generate_class_pyi_header(self, class_name: str, has_new_method: bool, number_of_typedefs: int):
        ...
        if number_of_typedefs == 1:
            class_header = (
                    ...
                    * f"\n        Create a new {class_name} Object:\n"
                    ...
                    + f"\"\"\"\n\n" + has_new_method *
                    f"    @staticmethod\n" +
                    f"    def {'New()' if has_new_method else '__new__()'} -> methods.{class_name}:\n"
                    f'        """Instantiate itk::{class_name}"""\n'
                    f"        ...\n"
                    + (not has_new_method)
                    * f"    def __call__(self) -> methods.{class_name}:\n"
                      f'        """Instantiate itk::{class_name}"""\n'
                      f"        ...\n" +
                    f"\n"
            ...
            # not positive that this is the approach for all classes that are not typed...
            return class_header  # f"from itk.methods import {class_name} as {class_name}\n\n"
        ...
        class_header = (
                ...
                * f"        Create a new {class_name} Object (of default type):\n"
                ...
                + (not has_new_method)
                * f"    def __call__(self) -> methods.{class_name}:\n"
                  f'        """Instantiate itk::{class_name}< INSERT_TYPE_NAMES_HERE >"""\n'
                  f"        ...\n" +
                  f"\n"
                  f"itk.{class_name} = _{class_name}Template"
                  f"\n"
