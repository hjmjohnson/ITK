...

{
  ...
  OptimizerPointer const estimateNewtonStepThreader = OptimizerType::New();
...
QuasiNewtonOptimizerv4Template<TInternalComputationValueType>::ResetNewtonStep(IndexValueType loc)
{
  ...
  IndexValueType const offset = loc * numLocalPara;
  ...
  const IndexRangeType & subrange)
{
  ...
  IndexValueType const low = subrange[0] / numLocalPara;
...
QuasiNewtonOptimizerv4Template<TInternalComputationValueType>::ComputeHessianAndStepWithBFGS(IndexValueType loc)
{
  ...
  vnl_matrix<TInternalComputationValueType> const plus = outer_product(dg, dg) / dot_dg_dx;
  vnl_matrix<TInternalComputationValueType> const minus = outer_product(edg, edg) / dot_edg_dx;
