...
itkGradientDescentOptimizerv4Test2(int, char *[])
{
  ...
  NumberOfParametersType const numLocal = metric->GetNumberOfLocalParameters();
  NumberOfParametersType const numLoops = metric->GetNumberOfParameters() / numLocal;
  for (NumberOfParametersType i = 0; i < numLoops; ++i)
  {
    for (NumberOfParametersType j = 0; j < numLocal; ++j)
    {
      NumberOfParametersType const ind = i * numLocal + j;
