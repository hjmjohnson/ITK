...
public:
  ...
  GetValue() const override
  {
    ...
    MeasureType const measure = 0.5 * (3 * x * x + 4 * x * y + 6 * y * y) - 2 * x + 8 * y;
...
itkOnePlusOneEvolutionaryOptimizerv4Test(int, char *[])
{
  ...
  itk::OnePlusOneCommandIterationUpdate::Pointer const observer = itk::OnePlusOneCommandIterationUpdate::New();
  itkOptimizer->AddObserver(itk::IterationEvent(), observer);

  // Declaration of the CostFunction
  itk::OnePlusOneMetric::Pointer const metric = itk::OnePlusOneMetric::New();
  ...
  unsigned int const maximumIteration = 8000;
  ...
  double const metricWorstPossibleValue = 0;
  ...
  bool const catchGetValueException = false;
