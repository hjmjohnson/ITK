...
public:
  ...
  GetValue() const override
  {
    ...
    MeasureType const val = 0.5 * (3 * x * x + 4 * x * y + 6 * y * y) - 2 * x + 8 * y;
...
public:
  ...
  Execute(const itk::Object * caller, const itk::EventObject &) override
  {
    ...
    if (nullptr != optimizer)
    {
      ...
      itk::SizeValueType const      currentIteration = optimizer->GetCurrentIteration();
...
itkExhaustiveOptimizerv4Test(int, char *[])
{
  ...
  bool const minimumValuePass = itk::Math::abs(itkOptimizer->GetMinimumMetricValue() - -10) < 1E-3;
  ...
  bool const maximumValuePass = itk::Math::abs(itkOptimizer->GetMaximumMetricValue() - 926) < 1E-3;
  ...
  size_t const requiredNumberOfSteps = (2 * steps[0] + 1) * (2 * steps[1] + 1);
