  ...
  bool itkNotUsed(doOnlyInitialization))
{
  ...
  if (this->GetScalesInitialized())
  {
    ...
    for (SizeType i = 0; i < this->m_Scales.Size(); ++i)
    {
      ...
      const SValueType difference = itk::Math::Absolute(NumericTraits<SValueType>::OneValue() - this->m_Scales[i]);
  ...
  if (this->m_Weights.Size() > 0)
  {
    ...
    {
      const SValueType difference = itk::Math::Absolute(NumericTraits<SValueType>::OneValue() - this->m_Weights[i]);
