  ...
  bool itkNotUsed(doOnlyInitialization))
{
  ...
  if (this->GetScalesInitialized())
  {
    ...
    for (SizeType i = 0; i < this->m_Scales.Size(); ++i)
    {
      ...
      SValueType const difference = itk::Math::abs(NumericTraits<SValueType>::OneValue() - this->m_Scales[i]);
  ...
  if (this->m_Weights.Size() > 0)
  {
    ...
    {
      SValueType const difference = itk::Math::abs(NumericTraits<SValueType>::OneValue() - this->m_Weights[i]);
