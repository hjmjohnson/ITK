...
ParticleSwarmOptimizerBase::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  ParameterBoundsType::const_iterator const end = this->m_ParameterBounds.end();
...
ParticleSwarmOptimizerBase::PrintSwarm(std::ostream & os, Indent indent) const
{
  ...
  std::vector<ParticleData>::const_iterator const end = this->m_Particles.end();
  os << indent << "[\n";
...
ParticleSwarmOptimizerBase::StartOptimization()
{
  ...
  unsigned int const bestValueMemorySize = this->m_NumberOfGenerationsWithMinimalImprovement + 1;
  ...
  for (this->m_IterationIndex = 1; m_IterationIndex < m_MaximalNumberOfIterations && !converged; ++m_IterationIndex)
  {
    ...
    unsigned int const index = this->m_IterationIndex % bestValueMemorySize;
...
ParticleSwarmOptimizerBase::ValidateSettings()
{
  ...
  if (!this->m_Particles.empty())
  {
    ...
    std::vector<ParticleData>::iterator const end = this->m_Particles.end();
    for (it = this->m_Particles.begin(); it != end; ++it)
    {
      ParticleData & p = (*it);
      for (unsigned int i = 0; i < n; ++i)
...
{
  unsigned int const                                                    n = GetInitialPosition().Size();
