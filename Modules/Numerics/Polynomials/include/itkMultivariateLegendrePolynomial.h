...
public:
  ...
  Evaluate(IndexType & index)
  {
    if (m_Dimension == 2)
    {
      ...
      {
        // normalized y [-1, 1]
        double const norm_y = m_NormFactor[1] * static_cast<double>(index[1] - 1);
        this->CalculateXCoef(norm_y, m_CoefficientArray);
        m_PrevY = index[1];
      }

      // normalized x [-1, 1]
      double const norm_x = m_NormFactor[0] * static_cast<double>(index[0] - 1);
    ...
    else if (m_Dimension == 3)
    {
      ...
      {
        // normalized z [-1, 1]
        double const norm_z = m_NormFactor[2] * static_cast<double>(index[2] - 1);
