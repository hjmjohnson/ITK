...
itkSpatialNeighborSubsamplerTest(int, char *[])
{
  ...
  IndexType const  idx{};
  RegionType const region{ idx, sz };
  ...
  idxConstraint[0] = 0;
  idxConstraint[1] = 5;
  RegionType const regionConstraint{ idxConstraint, szConstraint };
  ...
  SamplerType::Pointer const sampler = sampler_orig->Clone().GetPointer();
  ...
  queryIdx[0] = 2;
  queryIdx[1] = 6;
  ImageType::OffsetValueType const queryOffset = inImage->ComputeOffset(queryIdx);
  ...
  validStart[0] = 0;
  validStart[1] = 5;
  RegionType const validRegion{ validStart, validSz };

  IteratorType it(inImage, region);
  it.GoToBegin();
  while (!it.IsAtEnd())
  {
    PixelType const curValue = it.Get();
    IndexType const curIdx = it.GetIndex();
