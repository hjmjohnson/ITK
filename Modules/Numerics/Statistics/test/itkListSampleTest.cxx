...
itkListSampleTest(int argc, char * argv[])
{
  ...
  SampleType::MeasurementVectorSizeType const measurementVectorSize = std::stoi(argv[1]);
  std::cerr << "Measurement vector size: " << measurementVectorSize << std::endl;

  unsigned int const sampleSize = 25;
  ...
  std::cerr << "Const Iterators..." << std::endl;
  {
    ...
    SampleType::Iterator const nonconst_iter = sample->Begin();
  ...
  unsigned int const returnedSize = variableSizeSample->GetMeasurementVectorSize();
  ...
  try
  {
    MeasurementVectorType const measurement = sample->GetMeasurementVector(largestId + 10);
  ...
  {
    using IteratorType = SampleType::Iterator;
    IteratorType const iter = sample->Begin();
  ...
  // Testing methods specific to ConstIterators
  {
    ...
    using ConstIteratorType = SampleType::ConstIterator;
    ConstIteratorType const iter = sample->Begin();
    ...
    ConstIteratorType const iter4(constSample->Begin());
    ConstIteratorType const iter5(sample->Begin());
    if (iter4 != iter5)
    {
      std::cerr << "FAILED" << std::endl;
      return EXIT_FAILURE;
    ...
    ConstIteratorType const iter6(constSample);
    ConstIteratorType const iter7(sample);
    if (iter6 != iter7)
    {
      std::cerr << "FAILED" << std::endl;
      return EXIT_FAILURE;
    ...
    ConstIteratorType const iter8(sample);
