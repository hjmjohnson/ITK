...
itkImageToListSampleAdaptorTestTemplate()
{
  ...
  try
  {
    typename ImageToListSampleAdaptorType::MeasurementVectorType const m = sample->GetMeasurementVector(0);
  ...
  try
  {
    typename ImageToListSampleAdaptorType::ImageConstPointer const image = sample->GetImage();
    std::cerr << "Exception should have been thrown since the input image is not set yet" << std::endl;
  }
  catch (const itk::ExceptionObject & excp)
  {
  ...
  std::cerr << "Const Iterators..." << std::endl;
  {
    ...
    typename ImageToListSampleAdaptorType::Iterator const nonconst_iter = sample->Begin();
