...
itkSubsampleTest(int, char *[])
{
  ...
  itk::SizeValueType const totalSize = size[0] * size[1] * size[2];

  source->SetSize(size);
  float const minValue = -100.0;
  float const maxValue = 1000.0;
  ...
  int const idOutisdeRange = listSample->Size() + 2;
  ...
  try
  {
    MeasurementVectorType const vec = subsample->GetMeasurementVector(idOutisdeRange);
  ...
  try
  {
    unsigned int const          index = listSample->Size() + 2;
    MeasurementVectorType const measurementVector = subsample->GetMeasurementVectorByIndex(index);
    std::cerr
  ...
  {
    unsigned int const                         index = listSample->Size() + 2;
    SubsampleType::AbsoluteFrequencyType const frequency = subsample->GetFrequencyByIndex(index);
  ...
  {
    unsigned int const                       index = listSample->Size() + 2;
    ListSampleType::InstanceIdentifier const id = subsample->GetInstanceIdentifier(index);
    std::cerr
      << "Exception should have been thrown since the index specified is outside the range of the sample container"
      << std::endl;
  ...
  ListSampleType::InstanceIdentifier const ind =
