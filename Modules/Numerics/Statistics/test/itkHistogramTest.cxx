...
itkHistogramTest(int, char *[])
{
  ...
  size.Fill(64);
  unsigned long const totalSize = size[0] * size[1] * size[2];
  ...
  {
    float const expectedValF = 8.0;
    float const obtainedValF = measurement[kid0];
  ...
  {
    float const expectedValF = 8.0;
    float const obtainedValF = measurement[kid1];
    if (itk::Math::NotAlmostEquals(obtainedValF, expectedValF))
    {
      std::cerr << "Test failed!" << std::endl;
      std::cerr << "Error in itk::Statistics::Histogram::GetMeasurementVector at index [" << index << "]" << std::endl;
  ...
  {
    float const expectedValF = 8.0;
    float const obtainedValF = measurement[kid2];
    if (itk::Math::NotAlmostEquals(obtainedValF, expectedValF))
    {
      std::cerr << "Test failed!" << std::endl;
      std::cerr << "Error in itk::Statistics::Histogram::GetMeasurementVector at index [" << index << "]" << std::endl;
  ...
  unsigned int const dimension = 0;
  double const       mean = histogram->Mean(dimension);
  ...
  HistogramType::TotalAbsoluteFrequencyType const totalFrequency = histogram->GetTotalFrequency();

  InstanceIdentifier const histogramSize = histogram->Size();
  ...
  HistogramType::TotalAbsoluteFrequencyType const newTotalFrequency = histogram->GetTotalFrequency();
  ...
  // Exercise GetMin / GetMax methods
  {
    ...
    for (unsigned int dim = 0; dim < numberOfComponents; ++dim)
    {
      ...
      {
        HistogramType::MeasurementType const minA = binMinimums[dim][k];
        HistogramType::MeasurementType const minB = binDimensionMinimums[k];
        ...
        HistogramType::MeasurementType const minC = histogram->GetBinMin(dim, k);
    ...
    for (unsigned int dim = 0; dim < numberOfComponents; ++dim)
    {
      ...
      for (unsigned int k = 0; k < size2[dim]; ++k)
      {
        HistogramType::MeasurementType const maxA = binMaximums[dim][k];
        HistogramType::MeasurementType const maxB = binDimensionMaximums[k];
        ...
        HistogramType::MeasurementType const maxC = histogram->GetBinMax(dim, k);
  ...
  // Test methods specific to Iterators
  {
    ...
    IteratorType const iter = histogram->Begin();
    ...
    IteratorType const iter4(iter2);
    ITK_TEST_EXPECT_TRUE(iter4 == iter2);

    IteratorType const iter5 = iter2;
    ITK_TEST_EXPECT_TRUE(iter5 == iter2);

    itk::Statistics::Sample<HistogramType::MeasurementVectorType>::InstanceIdentifier const id2 = 7;
    IteratorType const                                                                      iter6(id2, histogram);
  ...
  // Test methods specific to ConstIterators
  {
    ...
    ConstIteratorType const iter = histogram->Begin();
    ...
    ConstIteratorType const iter3(iter2);
    ITK_TEST_EXPECT_TRUE(iter3 == iter2);

    const HistogramType * constHistogram = histogram.GetPointer();

    ConstIteratorType const iter4(constHistogram->Begin());
    ConstIteratorType const iter5(histogram->Begin());
    ITK_TEST_EXPECT_TRUE(iter5 == iter4);

    ConstIteratorType const iter6(constHistogram);
    ConstIteratorType const iter7(histogram);
    ITK_TEST_EXPECT_TRUE(iter6 == iter7);

    ConstIteratorType const                                                                 iter8(histogram);
    itk::Statistics::Sample<HistogramType::MeasurementVectorType>::InstanceIdentifier const id3 = 0;
