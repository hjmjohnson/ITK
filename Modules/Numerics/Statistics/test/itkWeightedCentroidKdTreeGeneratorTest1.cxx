...
int
itkWeightedCentroidKdTreeGeneratorTest1(int argc, char * argv[])
{
  if (argc < 4)
  {
  ...
  unsigned int const                     numberOfNeighbors = 1;
  ...
  for (unsigned int j = 0; j < numberOfTestPoints; ++j)
  {
    ...
    //
    double const result_dist = std::sqrt((result[0] - queryPoint[0]) * (result[0] - queryPoint[0]) +
