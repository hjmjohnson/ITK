...
HistogramToTextureFeaturesFilter<THistogram>::GenerateData()
{
  ...
  {
    AbsoluteFrequencyType const frequency = hit.GetFrequency();
    RelativeFrequencyType const relativeFrequency = frequency / totalFrequency;
  ...
  for (HistogramIterator hit = inputHistogram->Begin(); hit != inputHistogram->End(); ++hit)
  {
    RelativeFrequencyType const frequency = *rFreqIterator;
    ++rFreqIterator;
                                                                       ...
                                                                       double & pixelVariance)
{
  ...
  typename HistogramType::SizeValueType const binsPerAxis = inputHistogram->GetSize(0);
  ...
  */
  ...
  {
    int const    k = arrayIndex + 1;
    double const M_k_minus_1 = marginalMean;
    double const S_k_minus_1 = marginalDevSquared;
    double const x_k = marginalSums[arrayIndex];
