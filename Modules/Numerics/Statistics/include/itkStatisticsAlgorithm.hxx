          ...
          const typename TSubsample::MeasurementType partitionValue)
{
  ...
  //
  int const beginOfSectionEqualToPartition = moveToFrontIndex;
  moveToBackIndex = endIndex - 1;
  while (moveToFrontIndex < moveToBackIndex)
  {
    //
  ...
  int const endOfSectionEqualToPartition = moveToFrontIndex - 1;

  int const storeIndex = (beginOfSectionEqualToPartition + endOfSectionEqualToPartition) / 2;
  ...
  if (pivotValue != partitionValue)
  {
    ...
    {
      SampleMeasurementType const nodeValue = sample->GetMeasurementVectorByIndex(kk)[activeDimension];
      SampleMeasurementType const boundaryValue = sample->GetMeasurementVectorByIndex(storeIndex)[activeDimension];
            ...
            typename TSubsample::MeasurementType medianGuess)
{
  ...
  int       end = endIndex - 1;
  int const kthIndex = kth + beginIndex;
  ...
  while (true)
  {
    ...
    // [begin,end].
    int const pivotNewIndex = Partition<TSubsample>(sample, activeDimension, begin, end + 1, tempMedian);
...
{
  using SampleMeasurementType = typename TSubsample::MeasurementType;
  SampleMeasurementType const medianGuess = NumericTraits<SampleMeasurementType>::NonpositiveMin();
...
NthElement(TSubsample * sample, unsigned int activeDimension, int beginIndex, int endIndex, int nth)
{
  ...
  while (endElement - beginElement > 3)
  {
    ...
    int const cut = UnguardedPartition(sample, activeDimension, beginElement, endElement, tempMedian);
...
DownHeap(TSubsample * sample, unsigned int activeDimension, int beginIndex, int endIndex, int node)
{
  ...
  using SampleMeasurementType = typename TSubsample::MeasurementType;
  SampleMeasurementType const currentNodeValue = sample->GetMeasurementVectorByIndex(currentNode)[activeDimension];
