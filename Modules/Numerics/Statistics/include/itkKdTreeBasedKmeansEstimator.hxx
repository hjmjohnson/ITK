...
template <typename TKdTree>
void
KdTreeBasedKmeansEstimator<TKdTree>::PrintSelf(std::ostream & os, Indent indent) const
{
                                               ...
                                               MeasurementVectorType & upperBound)
{
  ...
  if (m_DistanceMetric->Evaluate(pointA, m_TempVertex) >= m_DistanceMetric->Evaluate(pointB, m_TempVertex))
  {
    return true;
  }
                                            ...
                                            MeasurementVectorType & upperBound)
{
  ...
  if (node->IsTerminal())
  {
    ...
    {
      typename TKdTree::InstanceIdentifier const tempId = node->GetInstanceIdentifier(i);
      this->GetPoint(individualPoint, m_KdTree->GetMeasurementVector(tempId));
      int const closest = this->GetClosestCandidate(individualPoint, validIndexes);
  ...
  else
  {
    ...
    int const     closest = this->GetClosestCandidate(centroid, validIndexes);
    ...
    {
      if (*iter != closest &&
          this->IsFarther(m_CandidateVector[*iter].Centroid, closestPosition, lowerBound, upperBound))
      {
    ...
    else
    {
      ...
      this->Filter(node->Left(), validIndexes, lowerBound, upperBound);
      upperBound[partitionDimension] = tempValue;

      tempValue = lowerBound[partitionDimension];
      lowerBound[partitionDimension] = partitionValue;
      this->Filter(node->Right(), validIndexes, lowerBound, upperBound);
...
KdTreeBasedKmeansEstimator<TKdTree>::StartOptimization()
{
  ...
  Algorithm::FindSampleBound<SampleType>(
    m_KdTree->GetSample(), m_KdTree->GetSample()->Begin(), m_KdTree->GetSample()->End(), lowerBound, upperBound);
  ...
  while (true)
  {
    ...
    this->Filter(m_KdTree->GetRoot(), validIndexes, lowerBound, upperBound);
    ...
    m_CentroidPositionChanges = this->GetSumOfSquaredPositionChanges(previousPosition, currentPosition);
...
template <typename TKdTree>
void
KdTreeBasedKmeansEstimator<TKdTree>::SetKdTree(TKdTree * tree)
{
  m_KdTree = tree;
...
template <typename TKdTree>
const TKdTree *
KdTreeBasedKmeansEstimator<TKdTree>::GetKdTree() const
{
...
template <typename TKdTree>
void
KdTreeBasedKmeansEstimator<TKdTree>::GetPoint(ParameterType & point, MeasurementVectorType measurements)
{
  for (unsigned int i = 0; i < m_MeasurementVectorSize; ++i)
  {
...
template <typename TKdTree>
void
KdTreeBasedKmeansEstimator<TKdTree>::PrintPoint(ParameterType & point)
{
  std::cout << "[ ";
  for (unsigned int i = 0; i < m_MeasurementVectorSize; ++i)
  {
