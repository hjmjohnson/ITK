...
{
  OffsetVectorPointer const offsetVector = OffsetVector::New();
...
ScalarImageToRunLengthMatrixFilter<TImageType, THistogramFrequencyContainer>::GenerateData()
{
  ...
  for (offsets = this->GetOffsets()->Begin(); offsets != this->GetOffsets()->End(); ++offsets)
  {
    ...
    for (neighborIt.GoToBegin(); !neighborIt.IsAtEnd(); ++neighborIt)
    {
      ...
      IndexType const centerIndex = neighborIt.GetIndex();
      ...
      MeasurementType const centerBinMin = this->GetOutput()->GetBinMinFromValue(0, centerPixelIntensity);
      MeasurementType const centerBinMax = this->GetOutput()->GetBinMaxFromValue(0, centerPixelIntensity);
      MeasurementType const lastBinMax = this->GetOutput()->GetDimensionMaxs(0)[this->GetOutput()->GetSize(0) - 1];
