...
ScalarImageToRunLengthFeaturesFilter<TImage, THistogramFrequencyContainer>::ScalarImageToRunLengthFeaturesFilter()
{
  ...
  // ClusterProminence}
  FeatureNameVectorPointer const requestedFeatures = FeatureNameVector::New();
  ...
  unsigned int const        centerIndex = hood.GetCenterNeighborhoodIndex();
  OffsetVectorPointer const offsets = OffsetVector::New();
  for (unsigned int d = 0; d < centerIndex; ++d)
  {
    OffsetType const offset = hood.GetOffset(d);
...
{
  size_t const numOffsets = this->m_Offsets->size();
  size_t const numFeatures = this->m_RequestedFeatures->size();
  ...
  for (offsetIt = this->m_Offsets->Begin(), offsetNum = 0; offsetIt != this->m_Offsets->End(); ++offsetIt, offsetNum++)
  {
    ...
    for (fnameIt = this->m_RequestedFeatures->Begin(), featureNum = 0; fnameIt != this->m_RequestedFeatures->End();
         ++fnameIt, featureNum++)
    {
  ...
  */
  ...
  {
    int const k = offsetNum + 1;
    for (featureNum = 0; featureNum < numFeatures; ++featureNum)
    {
      double const M_k_minus_1 = tempFeatureMeans[featureNum];
      double const S_k_minus_1 = tempFeatureDevs[featureNum];
      double const x_k = features[offsetNum][featureNum];

      double const M_k = M_k_minus_1 + (x_k - M_k_minus_1) / k;
      double const S_k = S_k_minus_1 + (x_k - M_k_minus_1) * (x_k - M_k);
...
ScalarImageToRunLengthFeaturesFilter<TImage, THistogramFrequencyContainer>::FastCompute()
{
  ...
  typename OffsetVector::ConstIterator const offsetIt = this->m_Offsets->Begin();
  this->m_RunLengthMatrixGenerator->SetOffset(offsetIt.Value());
  ...
  this->m_FeatureStandardDeviations->clear();
  typename FeatureNameVector::ConstIterator fnameIt;
  for (fnameIt = this->m_RequestedFeatures->Begin(); fnameIt != this->m_RequestedFeatures->End(); ++fnameIt)
  {
