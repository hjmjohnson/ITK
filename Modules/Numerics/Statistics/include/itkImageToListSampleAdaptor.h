...
public:
  ...
  Begin()
  {
    ImagePointer const nonConstImage = const_cast<ImageType *>(m_Image.GetPointer());
    ImageIteratorType  imageIterator(nonConstImage, nonConstImage->GetLargestPossibleRegion());
    imageIterator.GoToBegin();
    Iterator const iter(imageIterator, 0);
    return iter;
  }
  ...
  End()
  {
    ...
    Iterator const iter(imageIterator, largestRegion.GetNumberOfPixels());
  ...
  Begin() const
  {
    ...
    ConstIterator const iter(imageConstIterator, 0);
  ...
  End() const
  {
    ...
    ConstIterator const iter(imageConstIterator, largestRegion.GetNumberOfPixels());
