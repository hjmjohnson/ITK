...
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::Initialize()
{
  ...
  if (!this->m_NoiseSigmaIsSet)
  {
    ...
    for (unsigned int pc = 0; pc < m_NumPixelComponents; ++pc)
    {
      ...
      RealValueType const sigma = 5.0 / invFactor;
...
PatchBasedDenoisingImageFilter<TInputImage, TOutputImage>::InitializePatchWeightsSmoothDisc()
{
  ...
  typename WeightsImageType::RegionType const physicalRegion(physicalSize);
  ...
  typename PatchWeightsType::ValueType const centerWeight = patchWeights[(this->GetPatchLengthInVoxels() - 1) / 2];
  if (centerWeight != 1.0)
  {
    if (centerWeight <= 0.0)
    {
    ...
    typename PatchWeightsType::SizeValueType const pSize = patchWeights.Size();
                             ...
                             ThreadDataStruct             threadData)
{
  ...
  for (typename FaceListType::iterator fIt = faceList.begin(); fIt != faceList.end(); ++fIt)
  {
    ...
    bool const                        useCachedComputations = false;
  ...
  RealArrayType &         norm)
{
  for (unsigned int pc = 0; pc < m_NumPixelComponents; ++pc)
  {
    RealValueType const tmpDiff = this->GetComponent(b, pc) - this->GetComponent(a, pc);
    RealValueType const tmpWeight = weight[pc];
  ...
  Matrix<TensorValueT, 3, 3> &            eigenVecs)
{
  ...
  RealTensorValueT const I1 = D0 + D3 + D5;
  RealTensorValueT const I2 = D0 * D3 + D0 * D5 + D3 * D5 - (DSq1 + DSq2 + DSq4);
  RealTensorValueT const I3 = D0 * D3 * D5 + 2 * D1 * D2 * D4 - (D5 * DSq1 + D3 * DSq2 + D0 * DSq4);
  RealTensorValueT const I1div3 = I1 / 3;
  ...
  double const acos_arg = (s / n) * 1 / sqrtn;
  ...
  RealTensorValueT const phi = (itk::Math::abs(acos_arg) <= 1.0) ? std::acos(acos_arg) / 3 : 0.0;
  ...
  RealTensorValueT const lambda1 = I1div3 + 2 * sqrtn * std::cos(phi);
  RealTensorValueT const lambda2 = I1div3 - 2 * sqrtn * std::cos(itk::Math::pi / 3 + phi);
  RealTensorValueT const lambda3 = I1 - lambda1 - lambda2;
  ...
  for (unsigned int i = 0; i < 2; ++i)
  {
    ...
    // Bi = Dyy - eigenVals[i]
    // Ci = Dzz - eigenVals[i]
    RealTensorValueT const A = D0 - eigenVals[i];
    RealTensorValueT const B = D3 - eigenVals[i];
    RealTensorValueT const C = D5 - eigenVals[i];
    ...
    // eiy = term2 * term3
    // eiz = term1 * term3
    RealTensorValueT const term1 = D1 * D4 - B * D2;
    RealTensorValueT const term2 = D2 * D4 - C * D1;
    RealTensorValueT const term3 = D2 * D1 - A * D4;
    RealTensorValueT const ex = term1 * term2;
    RealTensorValueT const ey = term2 * term3;
    RealTensorValueT const ez = term1 * term3;

    // Now normalize the vector
    // e = [ex ey ez]
    // eigenVec = e / sqrt(e'e)
    RealTensorValueT const norm = ex * ex + ey * ey + ez * ez;
    RealTensorValueT const sqrtnorm = std::sqrt(norm);
  ...
  RealArrayType &                           geodesicDist)
{
  ...
  RealValueType const wt = weight[0];
  ...
  ThreadDataStruct             threadData) -> ThreadDataStruct
{
  ...
  BaseSamplerPointer const sampler = threadData.sampler;
  ...
  typename FaceListType::iterator const fIt = faceList.begin();
  if (!(fIt->GetNumberOfPixels()))
  {
  ...
  for (SampleIteratorType sampleIt = inList->Begin(); sampleIt != inList->End(); ++sampleIt, ++sampleNum)
  {
    ...
    InputImagePatchIterator const currentPatch = sampleIt.GetMeasurementVector()[0];
    IndexType                     nIndex = currentPatch.GetIndex();
    InstanceIdentifier const      currentPatchId = inList->GetImage()->ComputeOffset(nIndex);
    ...
    {
      InputImagePatchIterator const queryIt = sampler->GetSample()->GetMeasurementVector(currentPatchId)[0];
    ...
    for (typename BaseSamplerType::SubsampleConstIterator selectedIt = selectedPatches->Begin();
         ...
         ++selectedIt)
    {
      IndexType const currSelectedIdx = selectedIt.GetMeasurementVector()[0].GetIndex();
  ...
  ThreadDataStruct &                  threadData) -> RealType
{
       ...
       ++selectedIt)
  {
    ...
    for (unsigned int ic = 0; ic < m_NumIndependentComponents; ++ic)
    {
      RealValueType const kernelSigma = m_KernelBandwidthSigma[ic];
