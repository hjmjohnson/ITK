...
itkDivideOrZeroOutImageFilterTest(int, char *[])
{
  ...
  ImageType::SizeType const size = { { 10, 12, 14 } };
  ...
  ImageType::IndexType const zeroIndex = { { 4, 5, 6 } };
  ...
  ImageType::RegionType const                       region = divider->GetOutput()->GetLargestPossibleRegion();
  itk::ImageRegionConstIteratorWithIndex<ImageType> iter(divider->GetOutput(), region);
  for (; !iter.IsAtEnd(); ++iter)
  {
    ImageType::IndexType const index = iter.GetIndex();
    if (index != zeroIndex)
    {
      if (iter.Get() != 1.0f)
      {
