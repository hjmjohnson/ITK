...
TestClampFromTo()
{
  ...
  typename InputImageType::SizeType const randomSize = { { 18, 17, 23 } };
...
TestClampFromToWithCustomBounds()
{
  ...
  while (!it.IsAtEnd())
  {
    ...
    double const expectedMin = filter->GetLowerBound();
    double const expectedMax = filter->GetUpperBound();
...
{
  bool const success = TestClampFromToWithCustomBounds<TInputPixelType, char>() &&
...
itkClampImageFilterTest(int, char *[])
{
  ...
  bool const success = TestClampFrom<char>() && TestClampFrom<unsigned char>() && TestClampFrom<short>() &&
