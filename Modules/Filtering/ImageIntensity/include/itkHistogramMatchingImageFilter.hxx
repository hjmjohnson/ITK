...
HistogramMatchingImageFilter<TInputImage, TOutputImage, THistogramMeasurement>::BeforeThreadedGenerateData()
{
  ...
  if (m_GenerateReferenceHistogramFromImage)
  {
    InputImageConstPointer const reference = this->GetReferenceImage();
    if (reference.IsNull())
    {
    ...
    {
      HistogramPointer const tempHistptr = HistogramType::New();
  ...
  InputImageConstPointer const source = this->GetSourceImage();
...
{
  InputImageConstPointer const input = this->GetSourceImage();
  OutputImagePointer const     output = this->GetOutput();
