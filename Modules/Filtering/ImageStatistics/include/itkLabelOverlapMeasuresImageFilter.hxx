...
LabelOverlapMeasuresImageFilter<TLabelImage>::ThreadedStreamedGenerateData(const RegionType & outputRegionForThread)
{
  ...
  {
    LabelType const sourceLabel = itS.Get();
    LabelType const targetLabel = itT.Get();
...
{
  RealType const uo = this->GetUnionOverlap();
...
{
  RealType const uo = this->GetUnionOverlap(label);
  return (2.0 * uo / (1.0 + uo));
}
