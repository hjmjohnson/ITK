...
AccumulateImageFilter<TInputImage, TOutputImage>::GenerateOutputInformation()
{
  ...
  typename Superclass::OutputImagePointer const output = this->GetOutput();
  typename Superclass::InputImagePointer const  input = const_cast<TInputImage *>(this->GetInput());
...
AccumulateImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  typename TOutputImage::Pointer const              outputImage = this->GetOutput();
  ...
  typename TInputImage::SizeValueType const SizeAccumulateDimension = AccumulatedSize[m_AccumulateDimension];
  const auto sizeAccumulateDimensionDouble = static_cast<double>(SizeAccumulateDimension);
  typename TInputImage::IndexValueType const IndexAccumulateDimension = AccumulatedIndex[m_AccumulateDimension];
  for (unsigned int i = 0; i < InputImageDimension; ++i)
  {
    if (i != m_AccumulateDimension)
    {
