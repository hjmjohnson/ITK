...
ImageMomentsCalculator<TImage>::Compute()
{
  ...
  while (!it.IsAtEnd())
  {
    ...
    if (m_SpatialObjectMask.IsNull() || m_SpatialObjectMask->IsInsideInWorldSpace(physicalPosition))
    {
      ...
      for (unsigned int i = 0; i < ImageDimension; ++i)
      {
        ...
        {
          double const weight = value * static_cast<double>(indexPosition[i]) * static_cast<double>(indexPosition[j]);
          m_M2[i][j] += weight;
        }
      }
      ...
      for (unsigned int i = 0; i < ImageDimension; ++i)
      {
        ...
        for (unsigned int j = 0; j < ImageDimension; ++j)
        {
          double const weight = value * physicalPosition[i] * physicalPosition[j];
          m_Cm[i][j] += weight;
        }
      }
    }
  ...
  vnl_symmetric_eigensystem<double> const eigen{ m_Cm.GetVnlMatrix().as_matrix() };
  ...
  vnl_real_eigensystem const            eigenrot{ m_Pa.GetVnlMatrix().as_matrix() };
...
ImageMomentsCalculator<TImage>::GetPhysicalAxesToPrincipalAxesTransform() const -> AffineTransformPointer
{
  ...
  AffineTransformPointer const result = AffineTransformType::New();
  result->SetMatrix(matrix);
  result->SetOffset(offset);
