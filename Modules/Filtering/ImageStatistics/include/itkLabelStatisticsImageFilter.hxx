...
{
  MapConstIterator const mapIt = m_LabelStatistics.find(label);
...
LabelStatisticsImageFilter<TInputImage, TLabelImage>::GetMedian(LabelPixelType label) const -> RealType
{
  ...
  else
  {
    ...
    RealType const lowRange = mapIt->second.m_Histogram->GetBinMin(0, bin);
    RealType const highRange = mapIt->second.m_Histogram->GetBinMax(0, bin);
