...
ImagePCAShapeModelEstimator<TInputImage, TOutputImage>::GenerateInputRequestedRegion()
{
  ...
  if (this->GetInput(0))
  {
    ...
    InputImagePointer const input = const_cast<TInputImage *>(this->GetInput(0));
    ...
    for (idx = 1; idx < this->GetNumberOfIndexedInputs(); ++idx)
    {
      ...
      {
        typename TInputImage::RegionType const requestedRegion = this->GetInput(0)->GetLargestPossibleRegion();

        typename TInputImage::RegionType const largestRegion = this->GetInput(idx)->GetLargestPossibleRegion();
        ...
        InputImagePointer const ptr = const_cast<TInputImage *>(this->GetInput(idx));
...
ImagePCAShapeModelEstimator<TInputImage, TOutputImage>::GenerateData()
{
  ...
  {
    OutputImagePointer const output = this->GetOutput(j);
  ...
  unsigned int const numberOfValidOutputs = std::min(numberOfOutputs, m_NumberOfTrainingImages + 1);
...
ImagePCAShapeModelEstimator<TInputImage, TOutputImage>::CalculateInnerProduct()
{
  ...
  {
    InputImageConstPointer const inputImagePtr = dynamic_cast<const TInputImage *>(ProcessObject::GetInput(i));

    inputImagePointerArray[i] = inputImagePtr;

    InputImageConstIterator const inputImageIt(inputImagePtr, inputImagePtr->GetBufferedRegion());
...
ImagePCAShapeModelEstimator<TInputImage, TOutputImage>::EstimatePCAShapeModelParameters()
{
  ...
  vnl_generalized_eigensystem const eigenVectors_eigenValues(m_InnerProduct, identityMatrix);
