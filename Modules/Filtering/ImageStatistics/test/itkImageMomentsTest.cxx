...
using MatrixType = itk::Matrix<double, 3>;
using ImageType = itk::Image<PixelType, 3>;
using CalculatorType = itk::ImageMomentsCalculator<ImageType>;
using AffineTransformType = CalculatorType::AffineTransformType;
...
itkImageMomentsTest(int argc, char * argv[])
{
  ...
  double const ttm = 6.0; // Total mass
  double       pad[3][3] = {
    // Principal axes
    { 0.0, 0.0, 1.0 },
    { 0.6, -0.8, 0.0 },
  ...
  if (maskCondition == std::string("mask"))
  {
    ...
    typename itk::SpatialObject<MaskImageType::ImageDimension>::Pointer const test =
  ...
  double const ctm = moments->GetTotalMass();
  ...
  int const stat = tmerr > maxerr || cgerr > maxerr || pmerr > maxerr || paerr > maxerr || trerr > maxerr;
