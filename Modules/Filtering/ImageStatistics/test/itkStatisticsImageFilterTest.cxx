...
itkStatisticsImageFilterTest(int argc, char * argv[])
{
  ...
  FloatImage::IndexType const index{};

  region.SetIndex(index);
  region.SetSize(size);

  // first try a constant image
  float const fillValue = -100.0;
  image->SetRegions(region);
  image->Allocate();
  image->FillBuffer(static_cast<FloatImage::PixelType>(fillValue));

  float const sum = fillValue * static_cast<float>(region.GetNumberOfPixels());
  float const sumOfSquares = std::pow(fillValue, 2.0) * static_cast<float>(region.GetNumberOfPixels());
  ...
  double const expectedSigma = std::sqrt((maxValue - minValue) * (maxValue - minValue) / 12.0);
  double const epsilon = (maxValue - minValue) * .001;
  ...
  double const knownMean = 12.0;
  double const knownVariance = 10.0;
  ...
  double const testMean = dfilter->GetMean();
  double const testVariance = dfilter->GetVariance();
