...
itkFastMarchingImageFilterRealTest2(int itkNotUsed(argc), char * itkNotUsed(argv)[])
{
  ...
  itk::SimpleMemberCommand<ShowProgressObject>::Pointer const command =
  ...
  bool const isForbiddenImageBinaryMask = true;
  ...
  typename AdaptorType::OutputPixelType const aliveValue = 0.0;
  ...
  typename AdaptorType::OutputPixelType const trialValue = 1.0;
  ...
  // Check the results
  FloatImageType::Pointer const output = marcher->GetOutput();

  itk::ImageRegionIterator<FloatImageType> iterator(output, output->GetBufferedRegion());

  bool passed = true;

  double const threshold = 1.42;
