...
itkFastMarchingQuadEdgeMeshFilterBaseTest4(int, char *[])
{
  ...
  int          k = 0;
  double const alpha = (30.0 / 180.0) * itk::Math::pi;
  double const delta = 2.0 / std::tan(alpha);
  ...
  NodePairType const node_pair(0, 0.);
  ...
  MeshType::Pointer const output = fmm_filter->GetOutput();

  PointDataContainerPointer const output_data = output->GetPointData();

  PointDataContainer::ConstIterator       o_data_it = output_data->Begin();
  PointDataContainer::ConstIterator const o_data_end = output_data->End();
  ...
  {
    PixelType const expected_value = p.EuclideanDistanceTo(p_it->Value());
