...
FastMarchingImageFilter(unsigned int argc, char * argv[])
{
  ...
  InternalPixelType const stoppingValue = std::stod(argv[5]);

  CriterionPointer const criterion = CriterionType::New();
  criterion->SetThreshold(stoppingValue);

  using ReaderType = itk::ImageFileReader<InternalImageType>;
  using ReaderPointer = typename ReaderType::Pointer;

  ReaderPointer const reader = ReaderType::New();
  ...
  FastMarchingPointer const fastMarching = FastMarchingType::New();
  ...
  LabelImageReaderPointer const labelImageReader = LabelImageReaderType::New();
  ...
  {
    {
      std::string const filename = std::string(argv[7]) + std::string("LevelSet.nii.gz");
    ...
    {
      std::string const filename = std::string(argv[7]) + std::string("LabelMap.nii.gz");
