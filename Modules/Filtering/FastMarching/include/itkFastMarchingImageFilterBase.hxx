...
template <typename TInput, typename TOutput>
void
FastMarchingImageFilterBase<TInput, TOutput>::GenerateOutputInformation()
{
...
template <typename TInput, typename TOutput>
void
FastMarchingImageFilterBase<TInput, TOutput>::EnlargeOutputRequestedRegion(DataObject * output)
{
...
template <typename TInput, typename TOutput>
IdentifierType
FastMarchingImageFilterBase<TInput, TOutput>::GetTotalNumberOfNodes() const
{
...
template <typename TInput, typename TOutput>
unsigned char
FastMarchingImageFilterBase<TInput, TOutput>::GetLabelValueForGivenNode(const NodeType & iNode) const
{
...
FastMarchingImageFilterBase<TInput, TOutput>::UpdateNeighbors(OutputImageType * oImage, const NodeType & iNode)
{
  ...
  {
    typename NodeType::IndexValueType const v = iNode[j];
    typename NodeType::IndexValueType const start = m_StartIndex[j];
    typename NodeType::IndexValueType const last = m_LastIndex[j];
    ...
    for (int s = -1; s < 2; s += 2)
    {
      ...
      unsigned char const label = m_LabelImage->GetPixel(neighIndex);
...
template <typename TInput, typename TOutput>
void
FastMarchingImageFilterBase<TInput, TOutput>::UpdateValue(OutputImageType * oImage, const NodeType & iNode)
{
                                                                   ...
                                                                   InternalNodeStructureArray & ioNodesUsed)
{
  ...
  for (unsigned int j = 0; j < ImageDimension; ++j)
  {
    ...
    {
      typename NodeType::IndexValueType const temp = v + s;
      ...
      if ((temp <= last) && (temp >= start))
      {
        ...
        {
          OutputPixelType const neighValue = this->GetOutputValue(oImage, neighbor_node);
                                                    ...
                                                    InternalNodeStructureArray & iNeighbors) const
{
  ...
  {
    double const value = static_cast<double>(neighbor.m_Value);

    if (oSolution >= value)
    {
      unsigned int const axis = neighbor.m_Axis;
      // spaceFactor = \frac{1}{spacing[axis]^2}
      double const spaceFactor = itk::Math::sqr(1.0 / m_OutputSpacing[axis]);

      aa += spaceFactor;
      bb += value * spaceFactor;
      cc += itk::Math::sqr(value) * spaceFactor;

      double const discrim = itk::Math::sqr(bb) - aa * cc;
...
template <typename TInput, typename TOutput>
void
FastMarchingImageFilterBase<TInput, TOutput>::InitializeOutput(OutputImageType * oImage)
{
...
FastMarchingImageFilterBase<TInput, TOutput>::DoesVoxelChangeViolateStrictTopology(const NodeType & idx) const
{
  ...
  for (unsigned int d = 0; d < ImageDimension; ++d)
  {
    ...
    if (It.GetNext(d) == Traits::Alive && It.GetPrevious(d) == Traits::Alive)
    {
...
template <typename TInput, typename TOutput>
void
FastMarchingImageFilterBase<TInput, TOutput>::InitializeIndices2D()
{
...
template <typename TInput, typename TOutput>
bool
FastMarchingImageFilterBase<TInput, TOutput>::IsChangeWellComposed3D(const NodeType & idx) const
{
...
template <typename TInput, typename TOutput>
void
FastMarchingImageFilterBase<TInput, TOutput>::PrintSelf(std::ostream & os, Indent indent) const
{
