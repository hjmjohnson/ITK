...
template <typename TLevelSet, typename TSpeedImage>
void
FastMarchingImageFilter<TLevelSet, TSpeedImage>::GenerateOutputInformation()
{
  ...
  {
    LevelSetPointer const output = this->GetOutput();
...
template <typename TLevelSet, typename TSpeedImage>
void
FastMarchingImageFilter<TLevelSet, TSpeedImage>::EnlargeOutputRequestedRegion(DataObject * output)
{
...
template <typename TLevelSet, typename TSpeedImage>
void
FastMarchingImageFilter<TLevelSet, TSpeedImage>::Initialize(LevelSetImageType * output)
{
  ...
  if (m_AlivePoints)
  {
    ...
    typename NodeContainer::ConstIterator const pointsEnd = m_AlivePoints->End();
  ...
  if (m_OutsidePoints)
  {
    ...
    typename NodeContainer::ConstIterator const pointsEnd = m_OutsidePoints->End();
  ...
  if (m_TrialPoints)
  {
    ...
    typename NodeContainer::ConstIterator const pointsEnd = m_TrialPoints->End();
...
template <typename TLevelSet, typename TSpeedImage>
void
FastMarchingImageFilter<TLevelSet, TSpeedImage>::GenerateData()
{
  ...
  LevelSetPointer const        output = this->GetOutput();
  SpeedImageConstPointer const speedImage = this->GetInput();
