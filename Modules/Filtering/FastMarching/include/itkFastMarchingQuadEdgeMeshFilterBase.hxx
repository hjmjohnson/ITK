...
FastMarchingQuadEdgeMeshFilterBase<TInput, TOutput>::UpdateNeighbors(OutputMeshType * oMesh, const NodeType & iNode)
{
  ...
  if (qe)
  {
    ...
    do
    {
      if (qe_it)
      {
        OutputPointIdentifierType const neigh_id = qe_it->GetDestination();
      ...
      if (qe_it2)
      {
        if (qe_it->GetLeft() != OutputMeshType::m_NoFace)
        {
          ...
          if (IsFar1 || IsFar2)
          {
            ...
            if (val1 > val2)
            {
              OutputPointType const temp_pt(q1);
              q1 = q2;
              q2 = temp_pt;
                                                           ...
                                                           const OutputVectorRealType & iVal2) const
{
  ...
  OutputVectorRealType const sq_norm1 = Edge1.GetSquaredNorm();
  OutputVectorRealType       norm1 = 0.;

  OutputVectorRealType const epsilon = NumericTraits<OutputVectorRealType>::epsilon();

  if (sq_norm1 > epsilon)
  {
    norm1 = std::sqrt(sq_norm1);

    OutputVectorRealType const inv_norm1 = 1. / norm1;
    Edge1 *= inv_norm1;
  }

  OutputVectorRealType const sq_norm2 = Edge2.GetSquaredNorm();
  ...
  if (sq_norm2 > epsilon)
  {
    ...
    OutputVectorRealType const inv_norm2 = 1. / norm2;
    Edge2 *= inv_norm2;
  }
  ...
  if (iIsFar1 && iIsFar2)
  {
    ...
    else
    {
      ...
      if (unfolded)
      {
        OutputVectorRealType const t_sq_norm3 = norm3 * norm3;

        auto                       val3 = static_cast<OutputVectorRealType>(this->GetOutputValue(oMesh, new_id));
        OutputVectorRealType const t1 = ComputeUpdate(iVal1, val3, norm3, t_sq_norm3, norm1, sq_norm1, dot1, iF);
        OutputVectorRealType const t2 = ComputeUpdate(iVal2, val3, norm3, t_sq_norm3, norm2, sq_norm2, dot2, iF);
                                                                   ...
                                                                   const OutputVectorRealType & iF) const
{
  ...
  OutputVectorRealType const CosAngle = iDot;
  OutputVectorRealType const SinAngle = std::sqrt(1. - iDot * iDot);

  OutputVectorRealType const u = iVal2 - iVal1;

  OutputVectorRealType const sq_u = u * u;
  OutputVectorRealType const f2 = iSqNorm1 + iSqNorm2 - 2. * iNorm1 * iNorm2 * CosAngle;
  OutputVectorRealType const f1 = iNorm2 * u * (iNorm1 * CosAngle - iNorm2);
  OutputVectorRealType const f0 = iSqNorm2 * (sq_u - iF * iF * iSqNorm1 * SinAngle * SinAngle);

  OutputVectorRealType const delta = f1 * f1 - f0 * f2;
                                                                    ...
                                                                    OutputPointIdentifierType &       oId) const
{
  ...
  if (Norm1 > epsilon)
  {
    OutputVectorRealType const inv_Norm = 1. / Norm1;
    Edge1 *= inv_Norm;
  }
  ...
  if (Norm2 > epsilon)
  {
    OutputVectorRealType const inv_Norm = 1. / Norm2;
    Edge2 *= inv_Norm;
  }
  ...
  Vector2DType const x_start1(x1);
  Vector2DType const x_start2(x2);
  ...
  while (nNum < 50 && qe->GetLeft() != OutputMeshType::m_NoFace)
  {
    ...
    OutputPointIdentifierType const t_id = qe_Lnext->GetDestination();
    ...
    */
    ...
    Vector2DType const x = rotation * vv + x1;


    /* compute the intersection points.
       We look for x=x1+lambda*(x-x1) or x=x2+lambda*(x-x2) with <x,eqi>=0, so */
    OutputVectorRealType const lambda11 = -(x1 * v1) / ((x - x1) * v1); // left most
    OutputVectorRealType const lambda12 = -(x1 * v2) / ((x - x1) * v2); // right most
    OutputVectorRealType const lambda21 = -(x2 * v1) / ((x - x2) * v1); // left most
    OutputVectorRealType const lambda22 = -(x2 * v2) / ((x - x2) * v2); // right most
    bool const                 bIntersect11 = (lambda11 >= 0.) && (lambda11 <= 1.);
    bool const                 bIntersect12 = (lambda12 >= 0.) && (lambda12 <= 1.);
    bool const                 bIntersect21 = (lambda21 >= 0.) && (lambda21 <= 1.);
    bool const                 bIntersect22 = (lambda22 >= 0.) && (lambda22 <= 1.);
...
FastMarchingQuadEdgeMeshFilterBase<TInput, TOutput>::InitializeOutput(OutputMeshType * oMesh)
{
  ...
  {
    OutputCellsContainerPointer const       cells = oMesh->GetCells();
    OutputCellsContainerConstIterator       c_it = cells->Begin();
    OutputCellsContainerConstIterator const c_end = cells->End();
  ...
  OutputPointsContainerPointer const points = oMesh->GetPoints();

  OutputPointDataContainerPointer const pointdata = OutputPointDataContainer::New();
  pointdata->Reserve(points->Size());

  OutputPointsContainerIterator       p_it = points->Begin();
  OutputPointsContainerIterator const p_end = points->End();
  ...
  if (this->m_AlivePoints)
  {
    ...
    while (pointsIter != pointsEnd)
    {
      ...
      NodeType const idx = pointsIter->Value().GetNode();

      if (points->IndexExists(idx))
      {
        OutputPixelType const outputPixel = pointsIter->Value().GetValue();
