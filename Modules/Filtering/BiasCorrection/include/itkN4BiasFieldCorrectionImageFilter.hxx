...
N4BiasFieldCorrectionImageFilter<TInputImage, TMaskImage, TOutputImage>::GenerateData()
{
  ...
  RealImagePointer const logInputImage = RealImageType::New();
  ...
  RealImagePointer const logSharpenedImage = RealImageType::New();
  ...
  for (this->m_CurrentLevel = 0; this->m_CurrentLevel < maximumNumberOfLevels; this->m_CurrentLevel++)
  {
           ...
           this->m_CurrentConvergenceMeasurement > this->m_ConvergenceThreshold)
    {
      ...
      RealImagePointer const residualBiasField = subtracter1->GetOutput();
      ...
      RealImagePointer const newLogBiasField = this->UpdateBiasFieldEstimate(residualBiasField, numberOfIncludedPixels);
  ...
  RealImageType *       sharpenedImage) const
{
  ...
  for (size_t indexValue = 0; indexValue < numberOfPixels; ++indexValue)
  {
    if ((maskImageBufferRange.empty() || (useMaskLabel && maskImageBufferRange[indexValue] == maskLabel) ||
        ...
        (confidenceImageBufferRange.empty() || confidenceImageBufferRange[indexValue] > 0.0))
    {
      RealType const pixel = unsharpenedImageBufferRange[indexValue];
      if (pixel > binMaximum)
      {
        binMaximum = pixel;
      }
  ...
  RealType const histogramSlope = (binMaximum - binMinimum) / static_cast<RealType>(this->m_NumberOfHistogramBins - 1);
  ...
  for (size_t indexValue = 0; indexValue < numberOfPixels; ++indexValue)
  {
        ...
        (confidenceImageBufferRange.empty() || confidenceImageBufferRange[indexValue] > 0.0))
    {
      ...
      RealType const     cidx = (static_cast<RealType>(pixel) - binMinimum) / histogramSlope;
      unsigned int const idx = itk::Math::floor(cidx);
      RealType const     offset = cidx - static_cast<RealType>(idx);
  ...
  RealType const scaledFWHM = this->m_BiasFieldFullWidthAtHalfMaximum / histogramSlope;
  RealType const expFactor = 4.0 * std::log(2.0) / itk::Math::sqr(scaledFWHM);
  RealType const scaleFactor = 2.0 * std::sqrt(std::log(2.0) / itk::Math::pi) / scaledFWHM;
  ...
  {
    FFTComplexType const c = vnl_complex_traits<FFTComplexType>::conjugate(Ff[n]);
  ...
  for (size_t indexValue = 0; indexValue < numberOfPixels; ++indexValue)
  {
    if ((maskImageBufferRange.empty() || (useMaskLabel && maskImageBufferRange[indexValue] == maskLabel) ||
        ...
        (confidenceImageBufferRange.empty() || confidenceImageBufferRange[indexValue] > 0.0))
    {
      RealType const     cidx = (unsharpenedImageBufferRange[indexValue] - binMinimum) / histogramSlope;
      unsigned int const idx = itk::Math::floor(cidx);
  ...
  const size_t    numberOfIncludedPixels)
{
  ...
  PointSetPointer const fieldPoints = PointSetType::New();
  ...
  typename BiasFieldControlPointLatticeType::Pointer const phiLattice = bspliner->GetPhiLattice();
  ...
  const BiasFieldControlPointLatticeType * controlPointLattice)
{
  ...
  typename ScalarImageType::Pointer const biasFieldBsplineImage = reconstructer->GetOutput();
  ...
  const RealImageType * fieldEstimate2) const
{
  ...
  for (size_t indexValue = 0; indexValue < numberOfPixels; ++indexValue)
  {
    if ((maskImageBufferRange.empty() || (useMaskLabel && maskImageBufferRange[indexValue] == maskLabel) ||
        ...
        (confidenceImageBufferRange.empty() || confidenceImageBufferRange[indexValue] > 0.0))
    {
      RealType const pixel = std::exp(subtracterImageBufferRange[indexValue]);
      N += 1.0;
