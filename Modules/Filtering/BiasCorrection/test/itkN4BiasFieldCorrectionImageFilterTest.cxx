...
ConvertVector(std::string optionString)
{
  ...
  else
  {
    ...
    {
      std::string::size_type const crossposfrom = crosspos;
...
N4(int argc, char * argv[])
{
  ...
  typename CorrecterType::RealType const biasFieldFullWidthAtHalfMaximum = 0.15;
  ...
  typename CorrecterType::RealType const convergenceThreshold = 0.0000001;
  ...
  unsigned int const numberOfHistogramBins = 200;
  ...
  ImagePointer const originalInputImage = reader->GetOutput();
  reader->UpdateOutputInformation();
  correcter->SetMaskImage(nullptr);
  correcter->SetInput(originalInputImage);
  typename CorrecterType::RealImagePointer const biasField =
