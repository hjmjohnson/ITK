...
Stapler<VDimension>::Execute()
{
  ...
  typename itk::ImageFileWriter<OutputImageType>::Pointer const writer = itk::ImageFileWriter<OutputImageType>::New();

  size_t const numberOfFiles = m_Files.size();
...
itkSTAPLEImageFilterTest(int argc, char * argv[])
{
  ...
  std::vector<double> const specificity = stapler->GetSpecificity();
  ...
  std::vector<double> const sensitivity = stapler->GetSensitivity();
  ...
  unsigned int const i = stapler->GetNumberOfFiles() + 1;
