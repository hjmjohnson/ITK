...
LabelContourImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  OutputRegionType const reqRegion = this->GetOutput()->GetRequestedRegion();
...
LabelContourImageFilter<TInputImage, TOutputImage>::BeforeThreadedGenerateData()
{
  ...
  SizeValueType const pixelcount = output->GetRequestedRegion().GetNumberOfPixels();
  SizeValueType const xsize = output->GetRequestedRegion().GetSize()[0];
  SizeValueType const linecount = pixelcount / xsize;
  ...
  const OutputRegionType & outputRegionForThread)
{
  ...
  {
    SizeValueType const lineId = this->IndexToLinearIndex(inLineIt.GetIndex());
    LineEncodingType    thisLine;
    while (!inLineIt.IsAtEndOfLine())
    {
      InputPixelType const PVal = inLineIt.Get();

      SizeValueType        length = 0;
      InputIndexType const thisIndex = inLineIt.GetIndex();
      ...
      RunLength const thisRun = { length, thisIndex, static_cast<InternalLabelType>(PVal) };
  ...
  const OutputRegionType & outputRegionForThread)
{
  ...
  SizeValueType const   pixelcount = output->GetRequestedRegion().GetNumberOfPixels();
  SizeValueType const   xsize = output->GetRequestedRegion().GetSize()[0];
  OffsetValueType const linecount = pixelcount / xsize;
  ...
  {
    SizeValueType const thisIdx = this->IndexToLinearIndex(outLineIt.GetIndex());
    if (!m_LineMap[thisIdx].empty())
    {
      for (OffsetVectorConstIterator I = this->m_LineOffsets.begin(); I != this->m_LineOffsets.end(); ++I)
      {
        OffsetValueType const neighIdx = thisIdx + (*I);
