...
setConnectivityEarlyBox(TIterator * it, bool fullyConnected = false)
{
  ...
  else
  {
    ...
    unsigned int const centerIndex = it->GetCenterNeighborhoodIndex();
...
CornerOffsets(const TImage * im)
{
  ...
  NIterator const                          n1(unitradius, im, im->GetRequestedRegion());
  unsigned int const                       centerIndex = n1.GetCenterNeighborhoodIndex();
                          ...
                          typename TInputImage::SizeType    radius)
{
  ...
  ZeroFluxNeumannBoundaryCondition<TInputImage> const nbc;
  ...
  for (const auto & face : faceList)
  {
    ...
    else
    {
      ...
      for (oIt.GoToBegin(); !oIt.IsAtEnd(); ++oIt)
      {
        ...
        IndexType const centIndex = kernelRegionIdx;
        for (unsigned int i = 0; i < TInputImage::ImageDimension; ++i)
        {
        ...
        OffsetValueType const edgepixelscount = currentKernelRegion.GetNumberOfPixels();
        AccPixType            sum = 0;
  ...
  for (const auto & face : faceList)
  {
    ...
    else
    {
      ...
      for (oIt.GoToBegin(); !oIt.IsAtEnd(); ++oIt)
      {
        ...
        currentKernelRegion.SetIndex(kernelRegionIdx);
        currentKernelRegion.Crop(inputRegion);
        SizeValueType const edgepixelscount = currentKernelRegion.GetNumberOfPixels();
        AccPixType          sum = 0;
        AccPixType          squareSum = 0;
        // rules are : for each corner,
        //               for each dimension
