...
RecursiveGaussianImageFilter<TInputImage, TOutputImage>::SetUp(ScalarRealType spacing)
{
  ...
  switch (m_Order)
  {
    case GaussianOrderEnum::ZeroOrder:
    {
      ...
      ScalarRealType const alpha0 = 2 * SN / SD - this->m_N0;
    ...
    case GaussianOrderEnum::SecondOrder:
    {
      ...
      ScalarRealType const beta = -(2 * SN2 - SD * N0_2) / (2 * SN0 - SD * N0_0);
...
RecursiveGaussianImageFilter<TInputImage, TOutputImage>::ComputeNCoefficients(ScalarRealType   sigmad,
                                                                              ...
                                                                              ScalarRealType & EN)
{
  ScalarRealType const Sin1 = std::sin(W1 / sigmad);
  ScalarRealType const Sin2 = std::sin(W2 / sigmad);
  ScalarRealType const Cos1 = std::cos(W1 / sigmad);
  ScalarRealType const Cos2 = std::cos(W2 / sigmad);
  ScalarRealType const Exp1 = std::exp(L1 / sigmad);
  ScalarRealType const Exp2 = std::exp(L2 / sigmad);
