...
NormalizeSineWave(double frequencyPerImage, unsigned int order, double pixelSpacing = 1.0)
{
  ...
  double const frequency = frequencyPerImage * 2.0 * itk::Math::pi / (imageSize * pixelSpacing);

  // The theoretical maximal value should occur at this sigma
  double const sigma_max = std::sqrt(static_cast<double>(order)) / frequency;

  // the theoreical maximal value of the derivative, obtained at sigma_max
  double const expected_max = std::pow(static_cast<double>(order), order * 0.5) * std::exp(-0.5 * order);
  ...
  while (!iter.IsAtEnd())
  {
    ...
    const double x = p[0];
    double const value = std::sin(x * frequency);
