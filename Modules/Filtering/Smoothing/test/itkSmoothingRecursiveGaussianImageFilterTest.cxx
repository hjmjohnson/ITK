...
InPlaceTest(char * inputFilename, bool normalizeAcrossScale, typename TFilter::SigmaArrayType::ValueType sigmaValue)
{
  ...
  typename TFilter::OutputImageType::Pointer const outputImage1 = filter->GetOutput();
  ...
  typename TFilter::OutputImageType::Pointer const outputImage2 = filter->GetOutput();
  ...
  it1.GoToBegin();
  it2.GoToBegin();
  double const epsilon = itk::NumericTraits<double>::epsilon();
  while (!it1.IsAtEnd())
  {
...
itkSmoothingRecursiveGaussianImageFilterTest(int argc, char * argv[])
{
  ...
  bool const normalizeAcrossScale = std::stoi(argv[3]);
  ...
  SmoothingRecursiveGaussianImageFilterType::SigmaArrayType::ValueType const sigmaValue = std::stod(argv[4]);
