...
itkDiscreteGaussianImageFilterTest2(int argc, char * argv[])
{
  ...
  unsigned int const img_dim = std::stoi(argv[1]);
  ...
  unsigned int const vec_dim = std::stoi(argv[2]);
  ...
  float const        sigma = (argc > 5) ? std::stof(argv[5]) : 0.0;
  float const        kernelError = (argc > 6) ? std::stof(argv[6]) : 0.01;
  unsigned int const kernelWidth = (argc > 7) ? static_cast<unsigned int>(std::stoi(argv[7])) : 32;
  unsigned int const filterDimensionality = (argc > 8) ? static_cast<unsigned int>(std::stoi(argv[8])) : img_dim;
