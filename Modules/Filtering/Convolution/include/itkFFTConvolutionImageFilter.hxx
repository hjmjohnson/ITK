...
FFTConvolutionImageFilter<TInputImage, TKernelImage, TOutputImage, TInternalPrecision>::GenerateInputRequestedRegion()
{
  ...
  if (this->GetInput())
  {
    ...
    bool const       wasPartiallyInside = inputRegion.Crop(inputPtr->GetLargestPossibleRegion());
  ...
  float                      progressWeight)
{
  ...
  InputRegionType const  inputLargestRegion = input->GetLargestPossibleRegion();
  InputSizeType          inputLargestSize = inputLargestRegion.GetSize();
  InputIndexType         inputLargestIndex = inputLargestRegion.GetIndex();
  InputRegionType const  inputRequestedRegion = input->GetRequestedRegion();
  ...
  OutputRegionType const outputRequestedRegion = this->GetOutput()->GetRequestedRegion();
  ...
  for (unsigned int dim = 0; dim < ImageDimension; ++dim)
  {
    ...
    int const largestUpperCorner = inputLargestIndex[dim] + inputLargestSize[dim];
    int const requestedUpperCorner = inputRequestedIndex[dim] + inputRequestedSize[dim];

    int const largestLowerCorner = inputLargestIndex[dim];
    int const requestedLowerCorner = inputRequestedIndex[dim];

    // Pad for difference between lower corner of largest vs requested region
    int const lower = static_cast<int>(kernelRadius[dim]) - (requestedLowerCorner - largestLowerCorner);
    // Pad for difference between upper corner of largest vs requested region
    int const upper = static_cast<int>(kernelRadius[dim]) - (largestUpperCorner - requestedUpperCorner);
  ...
  InputRegionType const regionOfInterest = InputRegionType(regionOfInterestIndex, regionOfInterestSize);
  ...
  float                             progressWeight)
{
  ...
  float const paddingWeight = 0.2f;
  if (this->GetNormalize())
  {
    ...
    KernelPadPointer const kernelPadder = KernelPadType::New();
  ...
  float                 progressWeight)
{
  ...
  InternalRegionType const extractionRegion(extractionIndex, requestedRegionSize);
