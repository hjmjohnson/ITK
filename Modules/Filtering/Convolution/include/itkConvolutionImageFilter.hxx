                                                                                    ...
                                                                                    ProgressAccumulator * progress)
{
  ...
  bool const kernelNeedsPadding = this->GetKernelNeedsPadding();
  ...
  KernelSizeType const radius = this->GetKernelRadius(kernelImage);
  ...
  else // OutputRegionMode == Self::VALID
  {
    ...
    CropSizeType const upperCropSize(radius);
    ...
    CropFilterPointer const cropFilter = CropFilterType::New();
...
ConvolutionImageFilter<TInputImage, TKernelImage, TOutputImage>::GetKernelNeedsPadding() const
{
  ...
  InputRegionType const   kernelRegion = kernel->GetLargestPossibleRegion();
...
{
  const KernelImageType * kernel = this->GetKernelImage();
  KernelRegionType const  kernelRegion = kernel->GetLargestPossibleRegion();
...
ConvolutionImageFilter<TInputImage, TKernelImage, TOutputImage>::GenerateInputRequestedRegion()
{
  ...
  if (this->GetInput())
  {
    ...
    KernelSizeType const radius = this->GetKernelRadius(this->GetKernelImage());
    ...
    bool const                             cropped = inputRegion.Crop(inputPtr->GetLargestPossibleRegion());
    if (!cropped)
    {
  ...
  if (this->GetKernelImage())
  {
    ...
    // the requested region.
    typename KernelImageType::Pointer const kernelPtr = const_cast<KernelImageType *>(this->GetKernelImage());
