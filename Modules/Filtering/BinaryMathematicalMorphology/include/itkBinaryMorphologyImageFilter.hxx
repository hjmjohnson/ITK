...
BinaryMorphologyImageFilter<TInputImage, TOutputImage, TKernel>::AnalyzeKernel()
{
  ...
  SizeValueType const neighborhoodSize = SEoNeighbIt.Size();
  ...
  while (!kernelImageItIndex.IsAtEnd())
  {
    ...
    if (kernelImageItIndex.Get())
    {
      ...
      OffsetType const offset = this->GetKernel().GetOffset(kernel_it - KernelBegin);
  ...
  for (SizeValueType ii = 0; ii < adjNeigh.Size(); ++ii)
  {
         ...
         ++kernelOnElementsIt)
    {
      ...
      IndexValueType const k = *kernelOnElementsIt;
      ...
      OffsetType const currentOffset = this->GetKernel().GetOffset(k);
      ...
      else
      {
        ...
        // shifted SE element
        unsigned int const currentRelativeIndexOffset = this->GetKernel().GetNeighborhoodIndex(adjNeigh.GetOffset(ii)) -
        ...
        unsigned int const currentShiftedIndex = k /*NOT shifted position*/ + currentRelativeIndexOffset;
  ...
  unsigned int const centerKernelIndex = adjNeigh.Size() / 2;
  kernel_it = KernelBegin;
