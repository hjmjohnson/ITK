...
test_fft(unsigned int * SizeOfDimensions)
{
  ...
  typename ComplexImageType::Pointer const    complexImage = R2C->GetOutput();
  ...
  {
    unsigned int const zStride = i * sizes[1] * sizes[0];
    for (unsigned int j = 0; j < sizes[1]; ++j)
    {
      unsigned int const yStride = j * sizes[0];
  ...
  typename RealImageType::Pointer const imageAfterHalfHermitianToRealInverseFFT = C2R->GetOutput();
...
test_fft_rtc(unsigned int * SizeOfDimensions)
{
  ...
  // Get the size and the pointer to the complex image.
  typename ComplexImageType::Pointer const    complexImageA = R2Ca->GetOutput();
  ...
  typename ComplexImageType::Pointer const    complexImageB = R2Cb->GetOutput();
  ...
  {
    unsigned int const zStride = i * sizesA[1] * sizesA[0];
    for (unsigned int j = 0; j < sizesA[1]; ++j)
    {
      unsigned int const yStride = j * sizesA[0];
  ...
  {
    unsigned int const zStride = i * sizesB[1] * sizesB[0];
    for (unsigned int j = 0; j < sizesB[1]; ++j)
    {
      unsigned int const yStride = j * sizesB[0];
  ...
  {
    unsigned int const zStrideA = i * sizesA[1] * sizesA[0];
    unsigned int const zStrideB = i * sizesB[1] * sizesB[0];
    for (unsigned int j = 0; j < std::min(sizesA[1], sizesB[1]); ++j)
    {
      unsigned int const yStrideA = j * sizesA[0];
      unsigned int const yStrideB = j * sizesB[0];
      for (unsigned int k = 0; k < std::min(sizesA[0], sizesB[0]); ++k)
      {
        double const val = itk::Math::abs(fftbufA[zStrideA + yStrideA + k]);
