 ...
 *
 *=========================================================================*/
#ifndef itkPocketFFTForward1DFFTImageFilter_hxx
#define itkPocketFFTForward1DFFTImageFilter_hxx
...
void
PocketFFTForward1DFFTImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  multiThreader->template ParallelizeImageRegionRestrictDirection<TOutputImage::ImageDimension>(
    ...
    [input, output, direction, vectorSize](const typename OutputImageType::RegionType & lambdaRegion) {
      ...
      using ComplexVectorType = std::vector<ComplexType>;
      ...
      for (inputIt.GoToBegin(), outputIt.GoToBegin(); !inputIt.IsAtEnd(); outputIt.NextLine(), inputIt.NextLine())
      {
        ...
        // do the transform
        PocketFFTCommon::Transform1D(inputBuffer.data(), vectorSize, true, PixelType{ 1 });
