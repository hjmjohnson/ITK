...
itkBSplineSmoothingOnUpdateDisplacementFieldTransformTest(int, char *[])
{
  ...
  typename DisplacementTransformType::ArrayType::ValueType const controlPointsUpdateFieldVal = 4;
  ...
  typename DisplacementTransformType::ArrayType::ValueType const controlPointsTotalFieldVal = 0;
  ...
  DisplacementTransformType::ParametersValueType const paramsFillValue = 0.0;
  paramsFill.Fill(paramsFillValue);
  // Add an outlier to visually see that some smoothing is taking place.
  unsigned int const outlier = dimLength * dimensions * 4 + dimLength * dimensions / 2;
  ...
  typename DisplacementTransformType::SplineOrderType const splineOrder = 3;
  ...
  DisplacementTransformType::NumberOfParametersType const numberOfParameters =
  ...
  for (int i = -2; i < 3; ++i)
  {
    for (int j = -2; j < 3; ++j)
    {
      unsigned int const index = outlier + static_cast<unsigned int>(i * (int)(dimLength * dimensions) + j);
