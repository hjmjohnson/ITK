...
ExponentialDisplacementFieldImageFilter<TInputImage, TOutputImage>::ExponentialDisplacementFieldImageFilter()
{
  ...
  FieldInterpolatorPointer const VectorInterpolator = FieldInterpolatorType::New();
...
ExponentialDisplacementFieldImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  if (m_AutomaticNumberOfIterations)
  {
    ...
    {
      InputPixelRealValueType const norm2 = InputIt.Get().GetSquaredNorm();
      if (norm2 > maxnorm2)
      {
        maxnorm2 = norm2;
      }
    ...
    InputPixelRealValueType const numiterfloat = (maxnorm2 > 0) ? 2.0 + 0.5 * std::log(maxnorm2) / itk::Math::ln2
  ...
  for (unsigned int i = 0; i < numiter; ++i)
  {
    ...
    OutputImagePointer const warpedIm = m_Warper->GetOutput();
