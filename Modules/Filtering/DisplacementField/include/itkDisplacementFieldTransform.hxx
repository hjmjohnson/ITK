  ...
  bool                   useSVD) const
{
  ...
  {
    this->ComputeJacobianWithRespectToPositionInternal(index, jacobian, false);
    vnl_svd<typename JacobianPositionType::element_type> const svd{ jacobian.as_ref() };
  ...
  bool                   doInverseJacobian) const
{
  ...
  if (isValidJacobianCalcLocat)
  {
    ...
    for (unsigned int row = 0; row < VDimension; ++row)
    {
      FixedArray<TParametersValueType, VDimension> const localComponentGrad(jacobian[row]);
...
DisplacementFieldTransform<TParametersValueType, VDimension>::VerifyFixedParametersInformation()
{
  if (!this->m_DisplacementField.IsNull() && !this->m_InverseDisplacementField.IsNull())
  {
    ...
    SizeType const inverseFieldSize = this->m_InverseDisplacementField->GetLargestPossibleRegion().GetSize();
    ...
    SizeType const fieldSize = this->m_DisplacementField->GetLargestPossibleRegion().GetSize();
    ...
    std::ostringstream const spacingString;
    std::ostringstream const directionString;
