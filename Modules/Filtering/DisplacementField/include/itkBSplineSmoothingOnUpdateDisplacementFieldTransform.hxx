...
BSplineSmoothingOnUpdateDisplacementFieldTransform<TParametersValueType, VDimension>::UpdateTransformParameters(
  ...
  ScalarType             factor)
{
  DisplacementFieldPointer const displacementField = this->GetModifiableDisplacementField();
  ...
  if (smoothUpdateField)
  {
    ...
    DisplacementFieldPointer const updateField = importer->GetOutput();
    updateField->Update();
    updateField->DisconnectPipeline();

    DisplacementFieldPointer const updateSmoothField =
    ...
    bool const letArrayManageMemory = false;
    // Pass data pointer to required container. No copying is done.
    DerivativeType const smoothedUpdate(updatePointer, update.GetSize(), letArrayManageMemory);
  ...
  if (smoothTotalField)
  {
    ...
    DisplacementFieldPointer const totalField = importer->GetOutput();
    totalField->Update();
    totalField->DisconnectPipeline();

    DisplacementFieldPointer const totalSmoothField =
