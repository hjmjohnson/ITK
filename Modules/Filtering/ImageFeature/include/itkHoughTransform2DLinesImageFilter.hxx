...
HoughTransform2DLinesImageFilter<TInputPixelType, TOutputPixelType>::GenerateOutputInformation()
{
  ...
  InputImageConstPointer const input = this->GetInput();
...
HoughTransform2DLinesImageFilter<TInputPixelType, TOutputPixelType>::GenerateInputRequestedRegion()
{
  ...
  if (this->GetInput())
  {
    InputImagePointer const image = const_cast<InputImageType *>(this->GetInput());
...
HoughTransform2DLinesImageFilter<TInputPixelType, TOutputPixelType>::GenerateData()
{
  ...
  InputImageConstPointer const inputImage = this->GetInput(0);
  OutputImagePointer const     outputImage = this->GetOutput(0);
    ...
    do
    {
      ...
      for (it_input.GoToBegin(); !it_input.IsAtEnd(); ++it_input)
      {
        if (Math::ExactlyEquals(it_input.Get(), max))
        {
          ...
          double const radius = it_input.GetIndex()[0];
          double const teta = ((it_input.GetIndex()[1]) * 2 * Math::pi / this->GetAngleResolution()) - Math::pi;
          double const Vx = radius * std::cos(teta);
          double const Vy = radius * std::sin(teta);
