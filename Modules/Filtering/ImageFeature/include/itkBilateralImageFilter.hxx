  ...
  const OutputImageRegionType & outputRegionForThread)
{
  ...
  for (const auto & face : faceList)
  {
    ...
    {
      // Setup
      OutputPixelRealType const centerPixel = static_cast<OutputPixelRealType>(b_iter.GetCenterPixel());
      ...
      for (typename TInputImage::IndexValueType i = 0; k_it < kernelEnd; ++k_it, ++i)
      {
        ...
        OutputPixelRealType const pixel = static_cast<OutputPixelRealType>(b_iter.GetPixel(i));
        // flip sign if needed
        OutputPixelRealType const rangeDistance = std::abs(pixel - centerPixel);
        ...
        if (rangeDistance < rangeDistanceThreshold)
        {
          ...
          OutputPixelRealType const tableArg = rangeDistance * distanceToTableIndex;
          OutputPixelRealType const rangeGaussian = m_RangeGaussianTable[Math::Floor<SizeValueType>(tableArg)];

          // normalization factor so filter integrates to one
          // (product of the domain and the range gaussian)
          OutputPixelRealType const gaussianProduct = (*k_it) * rangeGaussian;
