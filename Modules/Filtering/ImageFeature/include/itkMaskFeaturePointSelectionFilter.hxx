...
MaskFeaturePointSelectionFilter<TImage, TMask, TFeatures>::GenerateData()
{
  ...
  FeaturePointsPointer const pointSet = this->GetOutput();

  using PointsContainer = typename FeaturePointsType::PointsContainer;
  using PointsContainerPointer = typename PointsContainer::Pointer;

  PointsContainerPointer const points = PointsContainer::New();
  ...
  PointDataContainerPointer const pointData = PointDataContainer::New();
  ...
  NeighborSizeType const numPixelsInNeighborhood = imageItr.Size();
  ...
  IndexValueType const maxNumberPointsToInserted = Math::Floor<SizeValueType>(0.5 + pointMap.size() * m_SelectFraction);
  ...
  while (rit != pointMap.rend() && numberOfPointsInserted < maxNumberPointsToInserted)
  {
    ...
    if (selectionMap->GetPixel(indexOfPointToPick) && region.IsInside(indexOfPointToPick))
    {
      ...
      if (m_ComputeStructureTensors)
      {
        ...
        SizeType const                       neighborRadiusForTensor = m_BlockRadius + m_BlockRadius;
        ...
        {
          OffsetType const off = gradientItr.GetOffset(i);
          ...
          StructureTensorType const            product(gradI * tnspose);
          tensor += product;
        }
