  ...
  const OutputImageRegionType & outputRegionForThread)
{
  ...
  for (const auto & face : faceList)
  {
    ...
    while (!bit.IsAtEnd())
    {
      InputImagePixelType const this_one = bit.GetPixel(center);
      it.Set(m_BackgroundValue);
      for (unsigned int i = 0; i < ImageDimension * 2; ++i)
      {
        InputImagePixelType const that = bit.GetPixel(center + offset[i]);
        ...
        {
          InputImagePixelType const abs_this_one = itk::Math::abs(this_one);
          InputImagePixelType const abs_that = itk::Math::abs(that);
