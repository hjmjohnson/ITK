...
itkHessianRecursiveGaussianFilterScaleSpaceTest(int, char *[])
{
  ...
  for (double const objectSize : scales)
  {
    ...
    while (!it.IsAtEnd())
    {
      ...
      double const value = std::exp(-point[0] * point[0] / (2.0 * objectSize * objectSize));
      it.Set(value);
      ++it;
    }
    ...
    HessianImageType::Pointer const outputImage = filter->GetOutput();

    // Get the value at the center of the image, the location of the peak of the Gaussian
    PointType const center{};

    IndexType const centerIndex = outputImage->TransformPhysicalPointToIndex(center);

    // Irrespective of the scale, the Hxx component should be the same
    double const centerHxx = outputImage->GetPixel(centerIndex)[0];
