...
itkUnsharpMaskImageFilterTestSimple(int, char *[])
{
  ...
  itk::SimpleFilterWatcher const watchit(filter);
  ...
  UnsharpMaskImageFilterFilterType::SigmaArrayType::ValueType const sigma = 2.5;
  ...
  {
    UnsharpMaskImageFilterFilterType::SigmaArrayType::ValueType const sigma2 = sigmas[i];
  ...
  UnsharpMaskImageFilterFilterType::InternalPrecisionType const amount = 0.8;
  ...
  GradientImageType::Pointer const outputImage = filter->GetOutput();

  // check that output is correct near the step
  start[0] = 9;
  float const mins[4] = { -0.21f, -0.33f, 1.32f, 1.20f };
  float const maxs[4] = { -0.20f, -0.32f, 1.33f, 1.21f };
