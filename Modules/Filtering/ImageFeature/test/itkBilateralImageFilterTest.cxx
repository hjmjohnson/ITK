...
itkBilateralImageFilterTest(int, char *[])
{
  ...
  bool const automaticKernelSize = true;
  ITK_TEST_SET_GET_BOOLEAN(filter, AutomaticKernelSize, automaticKernelSize);

  typename FilterType::SizeType::SizeValueType const radiusVal = 2;
  typename FilterType::SizeType const                radius = FilterType::SizeType::Filled(radiusVal);
  filter->SetRadius(radius);
  ITK_TEST_SET_GET_VALUE(radius, filter->GetRadius());

  unsigned long const numberOfRangeGaussianSamples = 150;
