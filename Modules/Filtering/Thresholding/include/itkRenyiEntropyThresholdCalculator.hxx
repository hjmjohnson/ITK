...
RenyiEntropyThresholdCalculator<THistogram, TOutput>::GenerateData()
{
  ...
  TotalAbsoluteFrequencyType const total = histogram->GetTotalFrequency();
  if (total == TotalAbsoluteFrequencyType{})
  {
    itkExceptionMacro("Histogram is empty");
  }
  m_Size = histogram->GetSize(0);
  ProgressReporter const progress(this, 0, m_Size);
  if (m_Size == 1)
  {
  ...
  double const omega = P1[t_star3] - P1[t_star1];

  // Determine the optimal threshold value
  double const realOptThreshold = static_cast<double>(t_star1) * (P1[t_star1] + 0.25 * omega * beta1) +
  ...
  -> InstanceIdentifier
{
  ...
  for (InstanceIdentifier it = m_FirstBin; it <= m_LastBin; ++it)
  {
    ...
    for (InstanceIdentifier ih = 0; ih <= it; ++ih)
    {
      ...
      {
        double const x = (normHisto[ih] / P1[it]);
    ...
    for (InstanceIdentifier ih = it + 1; ih < m_Size; ++ih)
    {
      if (histogram->GetFrequency(ih, 0) != AbsoluteFrequencyType{})
      {
        double const x = (normHisto[ih] / P2[it]);
        ent_obj -= x * std::log(x);
      }
    }

    // Total entropy
    double const tot_ent = ent_back + ent_obj;
  ...
  const std::vector<double> & P2) -> InstanceIdentifier
{
  ...
  double const term = 1.0 / (1.0 - alpha);
  ...
  for (InstanceIdentifier it = m_FirstBin; it <= m_LastBin; ++it)
  {
    ...
    // Total entropy
    double const product = ent_back * ent_obj;
    double       tot_ent = 0.;
