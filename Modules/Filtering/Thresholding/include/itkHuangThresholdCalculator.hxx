...
void
HuangThresholdCalculator<THistogram, TOutput>::GenerateData()
{
  ...
  {
    double const mu = 1. / (1. + static_cast<double>(i) / C);
  ...
  for (InstanceIdentifier threshold = m_FirstBin; threshold < m_LastBin; ++threshold)
  {
    ...
    if (histogram->GetIndex(v, muFullIdx))
    {
      ...
      bool const status = histogram->GetIndex(v, muFullIdx);
      if (!status)
      {
...
template <typename THistogram, typename TOutput>
void
HuangThresholdCalculator<THistogram, TOutput>::PrintSelf(std::ostream & os, Indent indent) const
{
