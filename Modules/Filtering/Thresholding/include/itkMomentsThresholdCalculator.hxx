...
MomentsThresholdCalculator<THistogram, TOutput>::GenerateData()
{
  ...
  //
  double const cd = m0 * m2 - m1 * m1;
  double const c0 = (-m2 * m2 + m1 * m3) / cd;
  double const c1 = (m0 * -m3 + m2 * m1) / cd;
  double const z0 = 0.5 * (-c1 - std::sqrt(c1 * c1 - 4.0 * c0));
  double const z1 = 0.5 * (-c1 + std::sqrt(c1 * c1 - 4.0 * c0));
  double const p0 = (z1 - m1) / (z1 - z0); // Fraction of the object pixels in the target binary image
