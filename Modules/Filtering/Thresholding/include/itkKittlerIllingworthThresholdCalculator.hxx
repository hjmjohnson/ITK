...
KittlerIllingworthThresholdCalculator<THistogram, TOutput>::Mean()
{
  ...
  bool const                        status = data->GetIndex(v, idx);
...
KittlerIllingworthThresholdCalculator<THistogram, TOutput>::GenerateData()
{
  ...
  SizeValueType const    size = histogram->GetSize(0);
  ProgressReporter const progress(this, 0, size);
  if (size == 1)
  {
  ...
  double const As1 = A(size - 1);
  double const Bs1 = B(size - 1);
  double const Cs1 = C(size - 1);
  ...
  while (threshold != Tprev)
  {
    ...
    double const At = A(threshold);
    double const Bt = B(threshold);
    double const Ct = C(threshold);

    if (itk::Math::abs(At) < itk::Math::eps)
    {
      itkGenericExceptionMacro("At = 0.");
    }
    double const mu = Bt / At;
    ...
    double const nu = (Bs1 - Bt) / (As1 - At);

    double const p = At / As1;
    double const q = (As1 - At) / As1;
    double const sigma2 = Ct / At - (mu * mu);
    double const tau2 = (Cs1 - Ct) / (As1 - At) - (nu * nu);
    ...
    double const w0 = 1.0 / sigma2 - 1.0 / tau2;
    double const w1 = mu / sigma2 - nu / tau2;
    double const w2 = (mu * mu) / sigma2 - (nu * nu) / tau2 + std::log10((sigma2 * (q * q)) / (tau2 * p * p));

    // If the next threshold would be imaginary, return with the current one.
    double const sqterm = w1 * w1 - w0 * w2;
    ...
    {
      double const temp = -w2 / w1;

      typename HistogramType::MeasurementVectorType v(1);
      typename HistogramType::IndexType             idx;
      v[0] = temp;
      bool const status = histogram->GetIndex(v, idx);
      itkAssertInDebugAndIgnoreInReleaseMacro(status);
      if (status)
      {
    ...
    else
    {
      ...
      Tprev = threshold;
      double const temp = (w1 + std::sqrt(sqterm)) / w0;
