...
itkBinaryThresholdSpatialFunctionTest(int, char *[])
{
  ...
  // Set the thresholds
  double const lowerThreshold = -3.0;
  double const upperThreshold = 4.0;
  ...
  {
    point.Fill(p);
    FunctionType::OutputType const output = function->Evaluate(point);
    std::cout << "f(" << point << ") = " << output;
    std::cerr << " [" << function->GetFunction()->Evaluate(point);
    std::cout << "] " << std::endl;

    // Check results
    CoordRep const val = p * std::sqrt(2.0) - parameters[0];
    bool const     expected = (lowerThreshold <= val && upperThreshold >= val);
    if (output != expected)
    {
  ...
  while (!iterator.IsAtEnd())
  {
    ...
    double const value = sphere->Evaluate(point);
