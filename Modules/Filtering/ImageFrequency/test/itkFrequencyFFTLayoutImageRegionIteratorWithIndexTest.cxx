...
public:
  ...
  explicit itkFrequencyFFTLayoutImageRegionIteratorWithIndexTester(size_t inputImageSize)
  {
    ...
    unsigned int const isImageSizeOdd = m_ImageIsOdd ? 1 : 0;
    size.Fill(inputImageSize / 2);
  ...
  TestLargestRegion()
  {
    ...
    IndexType const zero_freq_index{};
    it.GoToBegin();
    ...
    while (!it.IsAtEnd())
    {
      IndexType const index = it.GetIndex();
      // Check to see if the index is within allowed bounds
      bool const isInside = m_Image->GetLargestPossibleRegion().IsInside(index);
      if (!isInside)
      {
        std::cerr << "Test failed! " << std::endl;
...
itkFrequencyFFTLayoutImageRegionIteratorWithIndexTest(int, char *[])
{
  ...
  // Even input image size test
  {
    size_t const inputImageSize(10);
