...
compareImages(ImageType * baseline, ImageType * test)
{
  ...
  unsigned int const numberOfDiffPixels = differenceFilter->GetNumberOfPixelsWithDifferences();
...
itkFrequencyBandImageFilterTest(int argc, char * argv[])
{
  ...
  BandFilterType::FrequencyValueType const lowFreqThresholdRadians = itk::Math::pi_over_4;
  passBandFilter->SetLowFrequencyThresholdInRadians(lowFreqThresholdRadians);

  BandFilterType::FrequencyValueType const highFreqThresholdRadians = itk::Math::pi_over_2;
  passBandFilter->SetHighFrequencyThresholdInRadians(highFreqThresholdRadians);

  BandFilterType::FrequencyValueType const knownLowFrequencyHertz = lowFreqThresholdRadians / (2 * itk::Math::pi);
  BandFilterType::FrequencyValueType const knownHighFrequencyHertz = highFreqThresholdRadians / (2 * itk::Math::pi);
  ...
  std::function<double(const UnaryFilterType::FrequencyIteratorType &)> const func1 =
  ...
  std::function<void(UnaryFilterType::FrequencyIteratorType &)> const func2 =
