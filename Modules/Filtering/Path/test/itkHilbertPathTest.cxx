...
HilbertPathTestHelper(unsigned int maxHilbertPathOder)
{
  ...
  for (unsigned int order = 1; order < maxHilbertPathOder; ++order)
  {
    ...
    typename PathType::InputType const endOfInput = path->EndOfInput();
    ...
    {
      IndexType const index = path->Evaluate(d);
    ...
    {
      IndexType const index = path->TransformPathIndexToMultiDimensionalIndex(d);
