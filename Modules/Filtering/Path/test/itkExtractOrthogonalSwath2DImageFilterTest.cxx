...
itkExtractOrthogonalSwath2DImageFilterTest(int argc, char * argv[])
{
  ...
  ChainCodePathType::Pointer const chainPath = pathToChainCodePathFilter->GetOutput();

  // Set up the second filter
  ChainCodeToFourierSeriesPathFilterType::Pointer const chainCodeToFourierSeriesPathFilte =
  ...
  FourierSeriesPathType::Pointer const outputPath = chainCodeToFourierSeriesPathFilte->GetOutput();

  // Set up the third filter; THIS IS THE MAIN FILTER TO BE TESTED
  ExtractOrthogonalSwath2DImageFilterType::Pointer const extractOrthogonalSwath2DImageFilter =
  ...
  typename ImageType::PixelType const defaultPixelValue{};
  ...
  // Set up the output
  ImageType::Pointer const outputImage = extractOrthogonalSwath2DImageFilter->GetOutput();
