                                                                                    ...
                                                                                    OutputImageRegionType & splitRegion)
{
  ...
  unsigned int const maxThreadIdUsed =
  ...
  ThreadIdType                  threadId)
{
  ...
  InputRegionType const region = outputRegionForThread;
  ...
  InputSizeValueType const tempRow = NumberOfRows[m_CurrentDimension];
  OutputIndexType          idx{};
  ...
  if (m_CurrentDimension == ImageDimension - 1 && !this->m_SquaredDistance)
  {
    ...
    typename OutputImageType::RegionType const outputRegion = outputRegionForThread;
...
{
  OutputRegionType const    oRegion = output->GetRequestedRegion();
  OutputSizeValueType const nd = oRegion.GetSize()[d];

  vnl_vector<OutputPixelType> g(nd, 0);
  vnl_vector<OutputPixelType> h(nd, 0);


  InputRegionType const iRegion = m_InputCache->GetRequestedRegion();
  ...
  for (unsigned int i = 0; i < nd; ++i)
  {
    ...
    while (l < ns)
    {
      ...
      OutputPixelType const d2 = itk::Math::abs(g(l + 1)) + (h(l + 1) - iw) * (h(l + 1) - iw);
...
SignedMaurerDistanceMapImageFilter<TInputImage, TOutputImage>::Remove(OutputPixelType d1,
                                                                      ...
                                                                      OutputPixelType xf)
{
  OutputPixelType const a = x2 - x1;
  OutputPixelType const b = xf - x2;
  OutputPixelType const c = xf - x1;

  OutputPixelType const value = (c * itk::Math::abs(d2) - b * itk::Math::abs(d1) - a * itk::Math::abs(df) - a * b * c);
