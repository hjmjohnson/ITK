...
DanielssonDistanceMapImageFilter<TInputImage, TOutputImage, TVoronoiImage>::PrepareData()
{
  ...
  VoronoiImagePointer const voronoiMap = this->GetVoronoiMap();

  InputImagePointer const inputImage = dynamic_cast<const InputImageType *>(ProcessObject::GetInput(0));
  ...
  OutputImagePointer const distanceMap = this->GetDistanceMap();
  ...
  typename OutputImageType::RegionType const region = voronoiMap->GetRequestedRegion();
  ...
  VectorImagePointer const distanceComponents = GetVectorDistanceMap();
...
DanielssonDistanceMapImageFilter<TInputImage, TOutputImage, TVoronoiImage>::ComputeVoronoiMap()
{
  ...
  VoronoiImagePointer const voronoiMap = this->GetVoronoiMap();
  OutputImagePointer const  distanceMap = this->GetDistanceMap();
  VectorImagePointer const  distanceComponents = this->GetVectorDistanceMap();
  ...
  while (!ot.IsAtEnd())
  {
    IndexType const index = ct.GetIndex() + ct.Get();
    ...
    if (m_UseImageSpacing)
    {
      ...
      {
        double const component = distanceVector[i] * static_cast<double>(m_InputSpacingCache[i]);
...
{
  IndexType const there = here + offset;
...
DanielssonDistanceMapImageFilter<TInputImage, TOutputImage, TVoronoiImage>::GenerateData()
{
  ...
  RegionType const region = voronoiMap->GetRequestedRegion();
  ...
  SizeValueType const visitsPerPixel = (1 << InputImageDimension);
  ...
  while (!it.IsAtEnd())
  {
    ...
    if (!inputIt.Get())
    {
      IndexType const here = it.GetIndex();
      for (unsigned int dim = 0; dim < InputImageDimension; ++dim)
      {
        if (region.GetSize()[dim] <= 1)
        {
