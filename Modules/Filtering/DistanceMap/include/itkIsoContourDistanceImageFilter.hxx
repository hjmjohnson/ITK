...
IsoContourDistanceImageFilter<TInputImage, TOutputImage>::ThreaderFullCallback(void * arg)
{
  ...
  ThreadIdType const                total = filter->SplitRequestedRegion(workUnitID, workUnitCount, splitRegion);
...
IsoContourDistanceImageFilter<TInputImage, TOutputImage>::BeforeThreadedGenerateData()
{
  ...
  unsigned int const    actualThreads = this->SplitRequestedRegion(0, this->GetNumberOfWorkUnits(), dummy);
  ...
  ThreadIdType                  itkNotUsed(threadId))
{
  ...
  ImageConstPointer const inputPtr = this->GetInput();
  OutputPointer const     outputPtr = this->GetOutput();
  ...
  PixelType const negFarValue = -m_FarValue;
  ...
  ThreadIdType                  itkNotUsed(threadId))
{
  ...
  unsigned int const center = inNeigIt.Size() / 2;
  ...
  ThreadIdType                  threadId)
{
  ...
  ConstBandIterator const bandEnd = m_NarrowBandRegion[threadId].End;
...
{
  PixelRealType const val0 = static_cast<PixelRealType>(inNeigIt.GetPixel(center)) - m_LevelSetValue;
  bool const          sign = (val0 > 0);
  ...
  {
    PixelRealType const val1 = static_cast<PixelRealType>(inNeigIt.GetPixel(center + stride[n])) - m_LevelSetValue;

    bool const neighSign = (val1 > 0);
    ...
    if (sign != neighSign)
    {
      ...
      PixelRealType const alpha0 = 0.5; // Interpolation factor
      PixelRealType const alpha1 = 0.5; // Interpolation factor
      ...
      {
        PixelRealType const val = itk::Math::abs(grad[n]) * m_Spacing[n] / norm / diff;

        PixelRealType const               valNew0 = val0 * val;
        PixelRealType const               valNew1 = val1 * val;
