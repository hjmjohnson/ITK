...
itkApproximateSignedDistanceMapImageFilterTest(int argc, char * argv[])
{
  ...
  InputImageType::RegionType const region(size);
  ...
  while (!oIt.IsAtEnd())
  {
    ...
    OutputPixelType const distance = itk::Math::abs(oIt.Get() - SimpleSignedDistance(point));
  ...
  // Regression test
  OutputPixelType const maxAllowedDistance = 2;
