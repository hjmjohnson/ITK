...
FastChamferDistanceImageFilterTest(unsigned int iPositive, unsigned int iNegative, unsigned int iOther)
{
  ...
  typename ImageType::IndexType const  index{};
  ...
  typename ImageType::Pointer const outputImage = filter->GetOutput();
  ...
  itNBType const itNBend = band->End();
...
itkFastChamferDistanceImageFilterTest(int argc, char * argv[])
{
  ...
  int const Dimension = std::stoi(argv[1]);
