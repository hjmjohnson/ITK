...
itkDanielssonDistanceMapImageFilterTest(int, char *[])
{
  ...
  myImageType2D1::SizeType const   size2D = { { 9, 9 } };
  myImageType2D1::IndexType        index2D = { { 0, 0 } };
  myImageType2D1::RegionType const region2D{ index2D, size2D };
  ...
  myImageType2D2::Pointer const outputDistance2D = filter2D->GetOutput();

  using VoronoiImageType = myFilterType2D::VoronoiImageType;

  VoronoiImageType::Pointer const outputVoronoi2D = filter2D->GetVoronoiMap();

  myFilterType2D::VectorImagePointer const outputComponents = filter2D->GetVectorDistanceMap();
  ...
  bool const squaredDistance = true;
  ...
  myImageType2D2::PixelType const pixelValue = filter2D->GetOutput()->GetPixel(index2D);
  ...
  ImageType3D::IndexType const  index3D = { { 0, 0 } };
  ImageType3D::RegionType const region3D{ index3D, size3D };
  ...
  ImageType3D::RegionType const foregroundRegion{ foregroundIndex, foregroundSize };
