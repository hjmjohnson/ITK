...
CustomColormapFunction<TScalar, TRGBPixel>::operator()(const TScalar & v) const -> RGBPixelType
{
  ...
  {
    size_t const size = ColorChannel[color]->size();
    ...
    {
      RealType const p1 = (*ColorChannel[color])[index];
      RealType const m1 = (*ColorChannel[color])[index - 1u];
      RealType const d = p1 - m1;
