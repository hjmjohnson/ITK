...
itkFlatStructuringElementTest3(int argc, char * argv[])
{
  ...
  int               dimension = 2;
  std::string const outputImage = argv[1];
  int const         radius = std::stoi(argv[2]);
  int const         lines = std::stoi(argv[3]);
  if (argc > 4)
  {
  ...
  if (dimension == 2)
  {
    ...
    SE2Type const P = SE2Type::Polygon(r2, lines);
  ...
  else if (dimension == 3)
  {
    ...
    SE3Type const P = SE3Type::Polygon(r3, lines);
    SEToFile(P, outputImage);
  }
  else
  {
