...
itkClosingByReconstructionImageFilterTest(int argc, char * argv[])
{
  ...
  bool const preserveIntensities = static_cast<bool>(std::stoi(argv[4]));
  ITK_TEST_SET_GET_BOOLEAN(filter, PreserveIntensities, preserveIntensities);

  bool const fullyConnected = static_cast<bool>(std::stoi(argv[5]));
  ...
  if (argc == 7)
  {
    itk::SubtractImageFilter<InputImageType, OutputImageType, OutputImageType>::Pointer const subtract =
