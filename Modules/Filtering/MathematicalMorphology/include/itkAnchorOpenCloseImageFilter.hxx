  ...
  const InputImageRegionType & outputRegionForThread)
{
  ...
  {
    KernelLType const     ThisLine = decomposition[i];
    BresOffsetArray const TheseOffsets = BresLine.BuildLine(ThisLine, bufflength);
  ...
  {
    unsigned int const                   i = static_cast<unsigned int>(decomposition.size()) - 1;
    KernelLType const                    ThisLine = decomposition[i];
