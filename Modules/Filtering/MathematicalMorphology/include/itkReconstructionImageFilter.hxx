...
ReconstructionImageFilter<TInputImage, TOutputImage, TCompare>::GenerateData()
{
  ...
  MarkerImageConstPointer const markerImage = this->GetMarkerImage();
  MaskImageConstPointer const   maskImage = this->GetMaskImage();
  ...
  else
  {
    ...
    while (!outIt.IsAtEnd())
    {
      MarkerImagePixelType const currentValue = inIt.Get();
  ...
  if (m_UseInternalCopy)
  {
    ...
    NOutputIterator const tt(kernelRadius, markerImageP, *fit);
    outNIt = tt;

    InputIteratorType const ttt(maskImageP, *fit);
    mskIt = ttt;
    CNInputIterator const tttt(kernelRadius, maskImageP, *fit);
    mskNIt = tttt;
  }
  else
  {
    NOutputIterator const tt(kernelRadius, markerImageP, output->GetRequestedRegion());
    outNIt = tt;

    InputIteratorType const ttt(maskImageP, output->GetRequestedRegion());
    mskIt = ttt;
    CNInputIterator const tttt(kernelRadius, maskImageP, output->GetRequestedRegion());
    mskNIt = tttt;
  }
  ...
  for (outNIt.GoToBegin(), mskIt.GoToBegin(); !outNIt.IsAtEnd(); ++outNIt, ++mskIt)
  {
    ...
    {
      InputImagePixelType const VN = sIt.Get();
      if (compare(VN, V))
      {
  ...
  while (!outNIt.IsAtBegin())
  {
    ...
    InputImagePixelType const iV = mskNIt.GetCenterPixel();
    if (compare(V, iV))
    {
      outNIt.SetCenterPixel(iV);
      V = iV;
    ...
    {
      InputImagePixelType const VN = outNIt.GetPixel(*oLIt);
      InputImagePixelType const iN = mskNIt.GetPixel(*mLIt);
  ...
  typename CNInputIterator::ConstIterator const mIt;

  while (!IndexFifo.empty())
  {
    InputImageIndexType const I = IndexFifo.front();
    IndexFifo.pop();
    // reposition the iterators
    outNIt += I - outNIt.GetIndex();
    mskNIt += I - mskNIt.GetIndex();
    InputImagePixelType const V = outNIt.GetCenterPixel();
    for (oLIt = oIndexList.begin(), mLIt = mIndexList.begin(); oLIt != oIndexList.end(); ++oLIt, ++mLIt)
    {
      InputImagePixelType const VN = outNIt.GetPixel(*oLIt);
      InputImagePixelType const iN = mskNIt.GetPixel(*mLIt);
