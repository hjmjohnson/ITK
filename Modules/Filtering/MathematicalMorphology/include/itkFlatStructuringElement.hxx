                                                         ...
                                                         unsigned int                     lines)
{
  ...
  switch (faces)
  {
    ...
    {
      // dodecahedron
      float const        phi = (1.0 + std::sqrt(5.0)) / 2.0;
      float              b = 1.0 / phi;
      float              c = 2.0 - phi;
      unsigned int const facets = 12;
    ...
    case 20:
    {
      ...
      float const        phi = (1.0 + std::sqrt(5.0)) / 2.0;
      float const        a = 0.5;
      float const        b = 1.0 / (2.0 * phi);
      unsigned int const facets = 20;
      using FacetArrayType = std::vector<FacetType3>;
      FacetArrayType FacetArray;
      FacetArray.resize(facets);
      // set up vectors normal to the faces - only put in 3 points for
    ...
    case 32:
    {
      ...
      unsigned int const facets = 8 * static_cast<int>(std::pow(4.0, iterations));
  ...
  for (unsigned int i = 0; i < VDimension; ++i)
  {
    if ((size[i] % 2) == 0)
    {
      itkGenericExceptionMacro(
        "FlatStructuringElement constructor from image: size of input Image must be odd in all dimensions");
    }
  }
  return size;
}
