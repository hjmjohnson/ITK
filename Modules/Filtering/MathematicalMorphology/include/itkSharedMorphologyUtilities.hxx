...
NeedToDoFace(const TRegion AllImage, const TRegion face, const TLine line)
{
  ...
  IndexValueType const startI = ISt[smallDim];
  IndexValueType const facePos = FSt[smallDim] + FSz[smallDim] - 1;
  if (facePos == startI)
  {
...
{
  unsigned int const size = end - start + 1;
                 ...
                 const TLine                              line)
{
  ...
  if (foundFace)
  {
    ...
    unsigned int const              NonFaceLen = AllImage.GetSize()[NonFaceDim];
    for (unsigned int i = 0; i < TInputImage::RegionType::ImageDimension; ++i)
    {
      if (i != NonFaceDim)
      {
...
FillLineBuffer(typename TImage::ConstPointer             input,
               ...
               unsigned int &                            end)
{
  int const status = ComputeStartEnd<TImage, TBres, TLine>(StartIndex, line, tol, LineOffsets, AllImage, start, end);
  if (!status)
  {
    return (status);
  }
