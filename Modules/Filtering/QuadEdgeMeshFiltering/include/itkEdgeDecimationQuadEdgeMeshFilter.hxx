...
EdgeDecimationQuadEdgeMeshFilter<TInput, TOutput, TCriterion>::FillPriorityQueue()
{
  ...
  OutputCellsContainerIterator const end = output->GetEdgeCells()->End();
...
{
  OutputPointIdentifier const id_org = iEdge->GetOrigin();
  OutputPointIdentifier const id_dest = iEdge->GetDestination();

  OutputQEType *    temp = (id_org < id_dest) ? iEdge : iEdge->GetSym();
  MeasureType const measure = MeasureEdge(temp);
...
{
  typename OperatorType::EdgeStatusType const status = m_JoinVertexFunction->GetEdgeStatus();
  switch (status)
  {
    default:
...
EdgeDecimationQuadEdgeMeshFilter<TInput, TOutput, TCriterion>::ProcessWithoutAnyTopologicalGuarantee()
{
  ...
  OutputPointIdentifier const id_org = m_Element->GetOrigin();
  OutputPointIdentifier const id_dest = m_Element->GetDestination();
  OutputPointIdentifier const idx = (id_org < id_dest) ? id_org : id_dest;

  bool const to_be_processed(true);
  ...
  {
    OutputPointIdentifier const old_id = m_JoinVertexFunction->GetOldPointID();

    OutputPointIdentifier const new_id = (old_id == id_dest) ? id_org : id_dest;
...
EdgeDecimationQuadEdgeMeshFilter<TInput, TOutput, TCriterion>::CheckQEProcessingStatus()
{
  ...
  bool const LeftIsTriangle = qe->IsLnextOfTriangle();
  bool const RightIsTriangle = qe->GetSym()->IsLnextOfTriangle();
  ...
  if (LeftIsTriangle || RightIsTriangle)
  {
    ...
    {
      // two triangles
      bool const OriginOrderIsTwo = (qe->GetOrder() == 2);
      bool const DestinationOrderIsTwo = (qe_sym->GetOrder() == 2);
