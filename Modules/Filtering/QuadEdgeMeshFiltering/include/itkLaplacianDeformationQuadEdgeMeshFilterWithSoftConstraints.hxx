  ...
  VectorType & iBz)
{
  ...
  OutputMapPointIdentifierConstIterator const end = this->m_InternalMap.end();
  ...
  while (it != end)
  {
    ...
    RowConstIterator const rEnd = row.end();
...
LaplacianDeformationQuadEdgeMeshFilterWithSoftConstraints<TInputMesh, TOutputMesh, TSolverTraits>::GenerateData()
{
  ...
  if (!this->m_Constraints.empty())
  {
    ...
    MatrixType const Mt(M.transpose());
    ...
    for (ConstraintMapConstIterator cIt = this->m_Constraints.begin(); cIt != this->m_Constraints.end(); ++cIt)
    {
      ...
      typename std::unordered_map<OutputPointIdentifier, OutputCoordRepType>::const_iterator const lambdaIt =
