...
BSplineScatteredDataPointSetToImageFilter<TInputPointSet, TOutputImage>::SetSplineOrder(const ArrayType & order)
{
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    ...
    if (this->m_DoMultilevel)
    {
      ...
      {
        RealType const c = std::pow(static_cast<RealType>(2.0), static_cast<RealType>(C.cols()) - j - 1);
  ...
  ThreadIdType       threadId)
{
  ...
  RealImagePointer const neighborhoodWeightImage = RealImageType::New();
  ...
  ThreadIdType const numberOfWorkUnits = this->GetNumberOfWorkUnits();
  auto numberOfPointsPerThread = static_cast<SizeValueType>(input->GetNumberOfPoints() / numberOfWorkUnits);

  unsigned int const start = threadId * numberOfPointsPerThread;
  ...
  for (unsigned int n = start; n < end; ++n)
  {
    ...
    for (unsigned int i = 0; i < ImageDimension; ++i)
    {
      unsigned int const totalNumberOfSpans = this->m_CurrentNumberOfControlPoints[i] - this->m_SplineOrder[i];
    ...
    for (ItW.GoToBegin(); !ItW.IsAtEnd(); ++ItW)
    {
      ...
      for (unsigned int i = 0; i < ImageDimension; ++i)
      {
        RealType const u = static_cast<RealType>(p[i] - static_cast<unsigned int>(p[i]) - idx[i]) +
    ...
    for (ItW.GoToBegin(); !ItW.IsAtEnd(); ++ItW)
    {
      ...
      RealType const wc = this->m_PointWeights->GetElement(n);
      RealType const t = ItW.Get();
  ...
  ThreadIdType       itkNotUsed(threadId))
{
  ...
  typename PointDataImageType::IndexType const startPhiIndex =
...
BSplineScatteredDataPointSetToImageFilter<TInputPointSet, TOutputImage>::RefineControlPointLattice()
{
  ...
  PointDataImagePointer const refinedLattice = PointDataImageType::New();
  refinedLattice->SetRegions(size);
  refinedLattice->Allocate();

  PointDataType const data{};
  ...
  const unsigned int   dimension)
{
       ...
       ++It)
  {
    ...
    for (unsigned int i = 0; i < this->m_SplineOrder[dimension] + 1; ++i)
    {
      ...
      RealType const v = u - idx[dimension] + 0.5 * static_cast<RealType>(this->m_SplineOrder[dimension] - 1);
...
BSplineScatteredDataPointSetToImageFilter<TInputPointSet, TOutputImage>::SetPhiLatticeParametricDomainParameters()
{
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    RealType const domain = this->m_Spacing[i] * static_cast<RealType>(this->m_Size[i] - 1);
