  ...
  const OutputImageRegionType & region)
{
  ...
  typename PointDataImageType::IndexType const startPhiIndex = inputPtr->GetLargestPossibleRegion().GetIndex();

  RealArrayType epsilon;
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    RealType const r = static_cast<RealType>(this->m_NumberOfControlPoints[i] - this->m_SplineOrder[i]) /
                                                                                ...
                                                                                OutputImageRegionType & splitRegion)
{
  ...
  typename SizeType::SizeValueType const range = requestedRegionSize[splitAxis];
