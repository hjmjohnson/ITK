...
itkPadImageFilterTest(int, char *[])
{
  ...
  ShortImage::SizeType const   inputSize = { { 1, 1 } };
  ShortImage::RegionType const inputRegion(inputSize);
  ...
  ShortImage::Pointer const output = padFilter->GetOutput();
  for (int j = -2; j <= 2; ++j)
  {
    for (int i = -2; i <= 2; ++i)
    {
      ...
      short const pixel = output->GetPixel(index);
      std::cout << pixel << ' ';
