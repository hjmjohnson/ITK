...
using InterpolatorType2D = itk::InterpolateImagePointsFilter<ImageType2D, ImageType2D, CoordValueType>;
...
using InterpolatorType3D = itk::InterpolateImagePointsFilter<ImageType3D, ImageType3D, CoordValueType>;
...
test2DInterpolateImagePointsFilter()
{
  ...
  ImageType2DPointer const image = ImageType2D::New();
  ...
  double const xcoord[NPOINTS2] = { 0.1, 3.4, 4.0, 2.0 };
  double const ycoord[NPOINTS2] = { 0.2, 5.8, 6.0, 7.0 };
  double const truth[NPOINTS2] = { 151.650316034, 22.411473093, 36.2, DEFAULTPIXELVALUE };

  // Place continuous index coordinates into an image data structure
  CoordImageType2DPointer const index1 = CoordImageType2D::New();
  CoordImageType2DPointer const index2 = CoordImageType2D::New();

  CoordImage2DSizeType const   size = { { 2, 2 } };
  ...
  InterpolatorType2D::PixelType const defaultPixelValue = DEFAULTPIXELVALUE;
  ...
  {
    double const value = outIter.Get();
    std::cout.width(10);
...
test3DInterpolateImagePointsFilter()
{
  ...
  // Initialize input image
  ImageTypePtr3D const image = set3DData();
  ...
  for (auto & i : coord)
  {
    CoordImageType3DPointer const temp = CoordImageType3D::New();
    i = temp;
  ...
  while (!outIter.IsAtEnd())
  {
    double const temp = inIter.Get() - outIter.Get();
    rmse += temp * temp;
    ++outIter;
    ++inIter;
  }
...
{
  ImageType2DSizeType const size = { { 7, 7 } };
