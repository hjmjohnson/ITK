...
void
PrintImageData(ImageTypePtr2D imgPtr)
{
  ...
  int const dim = ImageType2D::ImageDimension;
...
{
  SizeType2D const size = { { 4, 4 } };
  double const     mydata[49] = { 0, 1, 2, 3, 1, 2, 3, 4, 2, 3, 4, 5, 3, 4, 3, 2 };
...
{
  IntSizeType2D const size = { { 4, 4 } };
  int const           mydata[49] = { 0, 1, 2, 3, 1, 2, 3, 4, 2, 3, 4, 5, 3, 4, 3, 2 };
...
VerifyResultsHigherOrderSpline(ImageTypePtr2D ActualResults, double * ExpectedResults)
{
  ...
  {
    double const val1 = ActualResultsIter.Get();
...
int
test2D_Standard_l2_NthOrderSpline_filter(unsigned int splineOrder)
{
  int flag = 0;

  /* Allocate a simple test image */
  ImageTypePtr2D const image = ImageType2D::New();

  set2DData(image);
  double ExpectedResults[] = { 0.517188, 1.575548, 2.634784, 2.847124, 1.547224, 2.323852, 3.101771, 3.257328,
                               2.578121, 3.073447, 3.570484, 3.669343, 2.784775, 3.223312, 3.663641, 3.751054 };
  ...
  itk::SimpleFilterWatcher const downWatcher(downSampler, "test2D_Standard_l2_filter");

  auto                           upSampler = UpsamplerType2D::New();
  itk::SimpleFilterWatcher const upWatcher(upSampler, "test2D_Standard_l2_filter");
  ...
  downSampler->Update();
  ImageTypePtr2D const outImage1 = downSampler->GetOutput();
  PrintImageData(outImage1);
  upSampler->SetInput(outImage1);
  upSampler->Update();
  ImageTypePtr2D const outImage2 = upSampler->GetOutput();
...
test2D_Standard_L2_NthOrderSpline_filter(unsigned int splineOrder)
{
  ...
  set2DData(image);
  double ExpectedResults[] = { 0.527796, 1.584850, 2.642784, 2.854860, 1.554069, 2.328161, 3.103548, 3.258594,
                               2.581206, 3.072767, 3.566038, 3.664140, 2.787102, 3.221627, 3.657944, 3.744552 };
...
int
test2D_Centered_l2_NthOrderSpline_filter(unsigned int splineOrder)
{
  int flag = 0;
  ...
  set2DData(image);
  double ExpectedResults[] = { 0.124139, 0.606412, 1.322693, 1.803619, 0.580514, 1.719005, 2.834988, 3.584282,
                               1.244998, 2.731395, 3.214231, 3.538417, 1.691146, 3.411135, 3.468862, 3.507621 };
  ...
  auto                           downSampler = DownsamplerType2D::New();
  itk::SimpleFilterWatcher const downWatcher(downSampler, "test2D_Centered_l2_filter");
  auto                           upSampler = UpsamplerType2D::New();
  itk::SimpleFilterWatcher const upWatcher(upSampler, "test2D_Centered_l2_filter");
  downSampler->SetSplineOrder(splineOrder);
  upSampler->SetSplineOrder(splineOrder);
...
testIntInputDoubleOutput()
{
  ...
  // Allocate a simple test image
  IntImageTypePtr2D const image = IntImageType2D::New();
  ...
  ImageTypePtr2D const outImage2 = upSampler->GetOutput();
  PrintImageData(outImage2);
  bool const sameResults = VerifyResults3rdOrderSpline(outImage2, ExpectedResults);
  if (!sameResults)
  {
    flag = 1;
...
test2D_Centered_L2_NthOrderSpline_filter(unsigned int splineOrder)
{
  ...
  set2DData(image);
  double ExpectedResults[] = { 0.119494, 0.600647, 1.323863, 1.802788, 0.574571, 1.712082, 2.837723, 3.583139,
                               1.245641, 2.733425, 3.217399, 3.537894, 1.690034, 3.409774, 3.468826, 3.507932 };
