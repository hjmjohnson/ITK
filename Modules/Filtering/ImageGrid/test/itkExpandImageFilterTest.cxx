...
itkExpandImageFilterTest(int, char *[])
{
  ...
  ImageType::SizeType const size = { { 64, 64 } };
  region.SetSize(size);
  ...
  ImageType::SizeType const validSize = validRegion.GetSize();
  ...
  while (!outIter.IsAtEnd())
  {
    ...
    if (validRegion.IsInside(index))
    {
      ...
      ImageType::IndexType const inputIndex = input->TransformPhysicalPointToIndex(point);
      double const               trueValue = pattern.Evaluate(inputIndex);
