...
itkDiffusionTensor3DTest(int, char *[])
{
  ...
  // Test GetTrace() and GetFractionalAnisotropy methods
  {
    ...
    AccumulateValueType const computedTrace = tensor3.GetTrace();
    ...
    RealValueType const computedInternalScalarProduct = tensor3.GetInnerScalarProduct();
    ...
    RealValueType const computedFractionalAnisotropy = tensor3.GetFractionalAnisotropy();
    ...
    RealValueType const computedRelativeAnisotropy = tensor3.GetRelativeAnisotropy();
  ...
  // Test Numeric Traits
  {
    ...
    TensorType const maxTensor = itk::NumericTraits<TensorType>::max();
    std::cout << maxTensor << std::endl;

    TensorType const minTensor = itk::NumericTraits<TensorType>::min();
    std::cout << minTensor << std::endl;

    TensorType const nonpositiveMinTensor = itk::NumericTraits<TensorType>::NonpositiveMin();
    std::cout << nonpositiveMinTensor << std::endl;

    TensorType const zeroValue{};
    std::cout << zeroValue << std::endl;

    TensorType const oneValue = itk::NumericTraits<TensorType>::OneValue();
    std::cout << oneValue << std::endl;

    TensorType const zero{};
    std::cout << zero << std::endl;

    TensorType const one = itk::NumericTraits<TensorType>::OneValue();
