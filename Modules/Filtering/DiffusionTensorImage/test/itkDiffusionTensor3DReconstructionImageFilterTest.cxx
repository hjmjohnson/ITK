...
itkDiffusionTensor3DReconstructionImageFilterTest(int argc, char * argv[])
{
  ...
  for (unsigned int pass = 0; pass < 2; ++pass)
  {
    ...
    TensorReconstructionImageFilterType::Pointer const tensorReconstructionFilter =
    ...
    ReferenceSizeType const   sizeReferenceImage = { { 4, 4, 4 } };
    ReferenceIndexType const  indexReferenceImage = { { 0, 0, 0 } };
    ReferenceRegionType const regionReferenceImage{ indexReferenceImage, sizeReferenceImage };
    ...
    //
    double const gradientDirections[6][3] = { { -1.000000, 0.000000, 0.000000 }, { -0.166000, 0.986000, 0.000000 },
    ...
    for (unsigned int i = 0; i < numberOfGradientImages; ++i)
    {
      ...
      GradientSizeType const   sizeGradientImage = { { 4, 4, 4 } };
      GradientIndexType const  indexGradientImage = { { 0, 0, 0 } };
      GradientRegionType const regionGradientImage{ indexGradientImage, sizeGradientImage };
      ...
      {
        TensorReconstructionImageFilterType::GradientDirectionType::element_type const gradientDirectionComponent =
          gradientDirection[j];
        TensorReconstructionImageFilterType::GradientDirectionType::element_type const outputComponent = output[j];
    ...
    unsigned int const idx = numberOfGradientImages + 1;
    ...
    if (pass == 1)
    {
      ...
      itk::ImageMaskSpatialObject<3>::Pointer const maskSpatialObject = itk::ImageMaskSpatialObject<3>::New();
    ...
    itk::SimpleFilterWatcher const watcher(tensorReconstructionFilter, "Tensor Reconstruction");

    tensorReconstructionFilter->Update();

    using TensorImageType = TensorReconstructionImageFilterType::TensorImageType;
    TensorImageType::Pointer const tensorImage = tensorReconstructionFilter->GetOutput();
    using TensorImageIndexType = TensorImageType::IndexType;

    TensorImageIndexType const tensorImageIndex = { { 3, 3, 3 } };
    GradientIndexType const    gradientImageIndex = { { 3, 3, 3 } };
    ReferenceIndexType const   referenceImageIndex = { { 3, 3, 3 } };
