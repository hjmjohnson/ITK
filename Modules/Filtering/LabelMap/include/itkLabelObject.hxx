...
LabelObject<TLabel, VImageDimension>::RemoveIndex(const IndexType & idx)
{
  ...
  while (it != m_LineContainer.end())
  {
    if (it->HasIndex(idx))
    {
      ...
      LengthType const orgLineLength = it->GetLength();
      ...
      else
      {
        ...
        ++newIdx[0];
        LengthType const newLength = orgLineLength - it->GetLength() - 1;
...
{
  LineType const line(idx, length);
...
LabelObject<TLabel, VImageDimension>::GetIndex(SizeValueType offset) const -> IndexType
{
  ...
  while (it != m_LineContainer.end())
  {
    SizeValueType const size = it->GetLength();
...
LabelObject<TLabel, VImageDimension>::Optimize()
{
  if (!m_LineContainer.empty())
  {
    ...
    // reorder the lines
    typename Functor::LabelObjectLineComparator<LineType> const comparator;
    ...
    while (it != lineContainer.end())
    {
      ...
      if (sameIdx && currentIdx[0] + (OffsetValueType)currentLength >= idx[0])
      {
        ...
        LengthType const newLength = idx[0] + (OffsetValueType)length - currentIdx[0];
