...
ChangeLabelLabelMapFilter<TImage>::GenerateData()
{
  ...
  ChangeMapIterator const backgroundLabelItr = m_MapOfLabelToBeReplaced.find(output->GetBackgroundValue());
  ...
  while (labelObjectItr != labelObjectsToBeRelabeled.end())
  {
    ...
    PixelType const   newLabel = m_MapOfLabelToBeReplaced[labelObjectSource->GetLabel()];
