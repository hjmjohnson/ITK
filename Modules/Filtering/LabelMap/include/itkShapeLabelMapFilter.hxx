...
ShapeLabelMapFilter<TImage, TLabelImage>::ThreadedProcessLabelObject(LabelObjectType * labelObject)
{
  ...
  while (!lit.IsAtEnd())
  {
    ...
    LengthType const  length = lit.GetLine().GetLength();
    ...
    if (length <= 2)
    {
      ...
      // loops.
      IndexValueType const endIdx0 = idx[0] + length;
  ...
  typename LabelObjectType::RegionType const boundingBox(mins, boundingBoxSize);
  ...
  vnl_symmetric_eigensystem<double> const eigen{ centralMoments.GetVnlMatrix().as_matrix() };
  ...
  vnl_real_eigensystem const            eigenrot{ principalAxes.GetVnlMatrix().as_matrix() };
  ...
  double const physicalSize = nbOfPixels * sizePerPixel;
  double const equivalentRadius = GeometryUtilities::HyperSphereRadiusFromVolume(ImageDimension, physicalSize);
  double const equivalentPerimeter = GeometryUtilities::HyperSpherePerimeter(ImageDimension, equivalentRadius);
...
ShapeLabelMapFilter<TImage, TLabelImage>::ComputePerimeter(LabelObjectType * labelObject)
{
  ...
  RegionType const                  boundingBox = labelObject->GetBoundingBox();
  ...
  double const perimeter = PerimeterFromInterceptCount(intercepts, this->GetOutput()->GetSpacing());
                                                                      ...
                                                                      const Spacing2Type  spacing)
{
  ...
  double const dx = spacing[0];
  double const dy = spacing[1];

  Offset2Type const nx = { { 1, 0 } };
  Offset2Type const ny = { { 0, 1 } };
  Offset2Type const nxy = { { 1, 1 } };
                                                                      ...
                                                                      const Spacing3Type  spacing)
{
  ...
  double const dx = spacing[0];
  double const dy = spacing[1];
  double const dz = spacing[2];
  double const dxy = std::sqrt(spacing[0] * spacing[0] + spacing[1] * spacing[1]);
  double const dxz = std::sqrt(spacing[0] * spacing[0] + spacing[2] * spacing[2]);
  double const dyz = std::sqrt(spacing[1] * spacing[1] + spacing[2] * spacing[2]);
  double const dxyz = std::sqrt(spacing[0] * spacing[0] + spacing[1] * spacing[1] + spacing[2] * spacing[2]);
  double const vol = spacing[0] * spacing[1] * spacing[2];
  ...
  double const c1 = 0.04577789120476 * 2; // Ox
  double const c2 = 0.04577789120476 * 2; // Oy
  double const c3 = 0.04577789120476 * 2; // Oz
  double const c4 = 0.03698062787608 * 2; // Oxy
  double const c5 = 0.03698062787608 * 2; // Oxz
  double const c6 = 0.03698062787608 * 2; // Oyz
  double const c7 = 0.03519563978232 * 2; // Oxyz
  // TODO - recompute those values if the spacing is non isotropic

  Offset3Type const nx = { { 1, 0, 0 } };
  Offset3Type const ny = { { 0, 1, 0 } };
  Offset3Type const nz = { { 0, 0, 1 } };
  Offset3Type const nxy = { { 1, 1, 0 } };
  Offset3Type const nxz = { { 1, 0, 1 } };
  Offset3Type const nyz = { { 0, 1, 1 } };
  Offset3Type const nxyz = { { 1, 1, 1 } };
...
ShapeLabelMapFilter<TImage, TLabelImage>::ComputeOrientedBoundingBox(LabelObjectType * labelObject)
{
  ...
  {
    typename LabelObjectType::LineType const line = labelObject->GetLine(l);
