  ...
  TemplatedGenerateData(const TAttributeAccessor & accessor)
  {
    ...
    while (!priorityQueue.empty())
    {
      ...
      else
      {
        ...
        OffsetValueType const length = l.line.GetLength();
        ...
        if (prevIdx[0] + prevLength > idx[0])
        {
          ...
          bool                                                  keepCurrent;
          typename TAttributeAccessor::AttributeValueType const prevAttr = accessor(prev.labelObject);
          typename TAttributeAccessor::AttributeValueType const attr = accessor(l.labelObject);
          ...
          if (keepCurrent)
          {
            ...
            if (prevIdx[0] + prevLength > idx[0] + length)
            {
              ...
              OffsetValueType const newLength = prevIdx[0] + prevLength - newIdx[0];
          ...
          else
          {
            ...
            else
            {
              ...
              OffsetValueType const newLength = idx[0] + length - newIdx[0];
              if (newLength > 0)
              {
    ...
    while (!it.IsAtEnd())
    {
      typename LabelObjectType::LabelType const label = it.GetLabel();
      LabelObjectType *                         labelObject = it.GetLabelObject();
