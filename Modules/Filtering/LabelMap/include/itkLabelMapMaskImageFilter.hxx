...
LabelMapMaskImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  else
  {
    ...
    else
    {
      ...
      bool const       testIdxIsInside = m_Crop && (inImage->GetBackgroundValue() == m_Label) ^ m_Negated;
      RegionType const outputRegion = output->GetLargestPossibleRegion();
...
LabelMapMaskImageFilter<TInputImage, TOutputImage>::ThreadedProcessLabelObject(LabelObjectType * labelObject)
{
  ...
  if (!m_Negated)
  {
    ...
    // Should we take care to not write outside the image ?
    bool const       testIdxIsInside = m_Crop && (input->GetBackgroundValue() == m_Label) ^ m_Negated;
    RegionType const outputRegion = output->GetLargestPossibleRegion();
