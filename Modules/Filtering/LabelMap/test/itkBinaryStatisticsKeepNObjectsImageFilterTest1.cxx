...
itkBinaryStatisticsKeepNObjectsImageFilterTest1(int argc, char * argv[])
{
  ...
  int const ForegroundValue = (std::stoi(argv[4]));
  ...
  int const BackgroundValue = (std::stoi(argv[5]));
  ...
  unsigned int const numberOfObjects = std::stoi(argv[6]);
  ...
  bool const reverseOrdering = std::stoi(argv[7]);
  ...
  bool const fullyConnected = std::stoi(argv[8]);
  ...
  BinaryKeepNObjectsType::AttributeType const attribute = std::stoi(argv[9]);
  KeepNObjects->SetAttribute(attribute);
  ITK_TEST_SET_GET_VALUE(attribute, KeepNObjects->GetAttribute());

  itk::SimpleFilterWatcher const watcher(KeepNObjects, "filter");
