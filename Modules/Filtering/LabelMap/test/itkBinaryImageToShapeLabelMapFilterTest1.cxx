...
itkBinaryImageToShapeLabelMapFilterTest1(int argc, char * argv[])
{
  ...
  bool const fullyConnected = std::stoi(argv[3]);
  ...
  int const inputForegroundValue = (std::stoi(argv[4]));
  ...
  int const outputBackgroundValue = (std::stoi(argv[5]));
  ...
  bool const computeFeretDiameter = (std::stoi(argv[6]));
  ITK_TEST_SET_GET_BOOLEAN(i2l, ComputeFeretDiameter, computeFeretDiameter);

  bool const computePerimeter = std::stoi(argv[7]);
  ITK_TEST_SET_GET_BOOLEAN(i2l, ComputePerimeter, computePerimeter);

  bool const computeOrientedBoundingBox = std::stoi(argv[8]);
