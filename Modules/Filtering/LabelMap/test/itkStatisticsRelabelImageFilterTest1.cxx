...
itkStatisticsRelabelImageFilterTest1(int argc, char * argv[])
{
  ...
  bool const reverseOrdering = std::stoi(argv[5]);
  ...
  RelabelType::AttributeType const attribute = std::stoi(argv[6]);
  statisticsRelabel->SetAttribute(attribute);
  ITK_TEST_SET_GET_VALUE(attribute, statisticsRelabel->GetAttribute());

  itk::SimpleFilterWatcher const watcher(statisticsRelabel, "filter");
