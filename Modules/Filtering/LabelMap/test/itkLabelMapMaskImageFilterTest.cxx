...
itkLabelMapMaskImageFilterTest(int argc, char * argv[])
{
  ...
  MaskFilterType::InputImagePixelType const label = std::stoi(argv[4]);
  maskFilter->SetLabel(label);
  ITK_TEST_SET_GET_VALUE(label, maskFilter->GetLabel());

  MaskFilterType::OutputImagePixelType const backgroundValue = std::stoi(argv[5]);
  ...
  itk::SimpleFilterWatcher const watcher(maskFilter, "LabelMapMaskImageFilter");
