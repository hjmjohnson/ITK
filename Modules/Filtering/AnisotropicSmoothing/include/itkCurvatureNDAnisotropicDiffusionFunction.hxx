                                                               ...
                                                               const FloatOffsetType &  itkNotUsed(offset)) -> PixelType
{
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    ...
    double const grad_mag = std::sqrt(m_MIN_NORM + grad_mag_sq);
    double const grad_mag_d = std::sqrt(m_MIN_NORM + grad_mag_sq_d);
    ...
    double const dx_forward_Cn = (dx_forward[i] / grad_mag) * Cx;
    double const dx_backward_Cn = (dx_backward[i] / grad_mag_d) * Cxd;
