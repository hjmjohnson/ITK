...
itkMinMaxCurvatureFlowImageFilterTest(int, char *[])
{
  ...
  radii[0] = 1;
  radii[1] = 3;
  int const err2D = testMinMaxCurvatureFlow(size2D, radius, numberOfRuns, niter, radii);
  ...
  niter[0] = 1;
  radii[1] = 1;
  int const err3D = testMinMaxCurvatureFlow(size3D, radius, numberOfRuns, niter, radii);
  ...
  niter[0] = 1;
  radii[1] = 1;
  int const err4D = testMinMaxCurvatureFlow(size4D, radius, numberOfRuns, niter, radii);
...
)
{
   ...
   */
  double const    sqrRadius = itk::Math::sqr(radius); // radius of the circle/sphere
  double const    fractionNoise = 0.30;               // salt & pepper noise fraction
  PixelType const foreground = 0.0;                   // intensity value of the foreground
  PixelType const background = 255.0;                 // intensity value of the background
  ...
  PixelType const tolerance = itk::Math::abs(foreground - background) * 0.1;
  ...
  for (; !outIter.IsAtEnd(); ++outIter)
  {
    ...
    PixelType const               value = outIter.Get();
  ...
  double const fractionWrong = static_cast<double>(numPixelsWrong) / static_cast<double>(region.GetNumberOfPixels());
