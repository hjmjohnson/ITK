...
{
  PixelType const     intensityTolerance = .001;
  int const           radiusTolerance = 0;
  unsigned long const numberOfPixelTolerance = 0;
  ...
  unsigned long const status = diff->GetNumberOfPixelsWithDifferences();
...
itkGradientAnisotropicDiffusionImageFilterTest2(int argc, char * argv[])
{
  ...
  itk::ImageFileReader<myFloatImage>::Pointer const input = itk::ImageFileReader<myFloatImage>::New();
  input->SetFileName(argv[1]);

  // Create a filter
  itk::GradientAnisotropicDiffusionImageFilter<myFloatImage, myFloatImage>::Pointer const filter =
  ...
  itk::CastImageFilter<myFloatImage, myUCharImage>::Pointer const caster =
  ...
  myFloatImage::Pointer const normalImage = filter->GetOutput();
