...
{
  std::string::size_type const pos = s.find_first_of(delims);
  if (pos != std::string::npos)
  {
...
StringTools::Split(const std::string & s, std::vector<std::string> & result, const std::string & delims)
{
  ...
  while (!str.empty())
  {
    std::string::size_type const pos = str.find_first_of(delims);
    if (pos != std::string::npos)
    {
...
{
  std::string::size_type const pos = s1.size() - s2.size();
