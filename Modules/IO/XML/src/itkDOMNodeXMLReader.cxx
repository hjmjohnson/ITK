...
DOMNodeXMLReader::Update(std::istream & is)
{
  ...
  bool const ok = XML_Parse(parser, s.data(), static_cast<int>(s.size()), false);
  if (!ok)
  {
    ExceptionObject   e(__FILE__, __LINE__);
    std::string const message(XML_ErrorString(XML_GetErrorCode(parser)));
...
DOMNodeXMLReader::StartElement(const char * name, const char ** atts)
{
  ...
  if (this->m_Context)
  {
    OutputPointer const node1 = OutputType::New();
  ...
  while (atts[i])
  {
    std::string const key(atts[i++]);
    std::string const value(atts[i++]);
