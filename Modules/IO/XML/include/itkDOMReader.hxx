...
DOMReader<TOutput>::Update(const DOMNodeType * inputdom, const void * userdata)
{
  ...
  FancyString       info;
  FancyString const tagname = inputdom->GetName();
...
DOMReader<TOutput>::Update()
{
  ...
  {
    DOMNodePointer const node = DOMNodeType::New();
    this->SetIntermediateDOM(node);
  }

  FancyString const fn(this->m_FileName);
  ...
  FancyString const sOldWorkingDir = itksys::SystemTools::GetCurrentWorkingDirectory();
  FancyString const sNewWorkingDir = itksys::SystemTools::GetFilenamePath(fn);
