...
MRCImageIO::Read(void * buffer)
{
  ...
  int const size = this->GetComponentSize();
  switch (size)
  {
    case 1:
      break;
...
MRCImageIO::UpdateHeaderWithMinMaxMean(const TPixelType * bufferBegin)
{
  ...
  std::pair<ConstPixelPointer, ConstPixelPointer> const mm = itk::min_max_element(bufferBegin, bufferEnd);

  double const mean = std::accumulate(bufferBegin, bufferEnd, 0.0) / std::distance(bufferBegin, bufferEnd);
