...
Test(const std::string & inFileName, const std::string & outFileName, const std::string & md5)
{
  ...
  DictionaryType::ConstIterator const end = dic.End();
  ...
  while (itr != end)
  {
    ...
    if (itr->first == itk::MRCImageIO::m_MetaDataHeaderName)
    {
      ...
      if (itk::ExposeMetaData(dic, itk::MRCImageIO::m_MetaDataHeaderName, header))
      {
        ...
        itk::MRCHeaderObject::Pointer const copyHeader = itk::MRCHeaderObject::New();
    ...
    else
    {
      ...
      MetaDataStringType::Pointer const entryvalue = dynamic_cast<MetaDataStringType *>(entry.GetPointer());
      if (entryvalue)
      {
