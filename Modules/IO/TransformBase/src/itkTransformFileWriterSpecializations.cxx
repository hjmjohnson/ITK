...
{
  ConstTransformPointer const res = m_TransformList.front();
...
TransformFileWriterTemplate<TParametersValueType>::Update()
{
  ...
  if (m_TransformIO.IsNull())
  {
    ...
    if (m_TransformIO.IsNull())
    {
      ...
      std::list<LightObject::Pointer> const allobjects =
...
struct TransformIOHelper
{
  ...
  CreateNewTypeTransform(std::string transformName)
  {
    ...
    LightObject::Pointer const i = ObjectFactoryBase::CreateInstance(transformName.c_str());
                   ...
                   std::list<typename TOutputTransformType::ConstPointer> & transformList)
{
  ...
  else
  {
    ...
    */
    OutputTransformPointer const outputComposite = IOhelper::CreateNewTypeTransform(transformName);
    ...
    for (; it != inputTransformList.end(); ++it)
    {
      ...
      std::string const      inSubName = inSub->GetTransformTypeAsString();
