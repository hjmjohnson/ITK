...
itkNiftiImageIOTest(int argc, char * argv[])
{
  ...
  if (argc > 1) // This is a mechanism for reading unsigned char images for testing.
  {
    ...
    itk::NiftiImageIO::Pointer const imageIO = itk::NiftiImageIO::New();
    ...
    for (int imagenameindex = 1; imagenameindex < argc; ++imagenameindex)
    {
      ...
      typename itk::NiftiImageIOEnums::NiftiFileEnum const fileType = imageIO->DetermineFileType(fileName.c_str());
  ...
  {
    itk::NiftiImageIOFactory::Pointer const MyFactoryTest = itk::NiftiImageIOFactory::New();
