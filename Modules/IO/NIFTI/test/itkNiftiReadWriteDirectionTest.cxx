...
ReadImage(const std::string & fileName, bool SFORM_Permissive)
{
  ...
  typename itk::NiftiImageIO::Pointer const imageIO = itk::NiftiImageIO::New();
  {
...
{
  vnl_matrix_fixed<itk::SpacePrecisionType, 3, 3> const rotation =
...
itkNiftiReadWriteDirectionTest(int argc, char * argv[])
{
  ...
  TestImageType::Pointer const inputImage = itk::ReadImage<TestImageType>(argv[1]);
  TestImageType::Pointer const inputImageNoQform = itk::ReadImage<TestImageType>(argv[2]);
  TestImageType::Pointer const inputImageNoSform = itk::ReadImage<TestImageType>(argv[3]);
  ...
  itk::ImageFileWriter<TestImageType>::Pointer const writer = itk::ImageFileWriter<TestImageType>::New();
  ...
  TestImageType::Pointer const reReadImage = itk::ReadImage<TestImageType>(testFilename);
  ...
  // This should work
  TestImageType::Pointer const inputImageNonOrthoSform = ReadImage<TestImageType>(argv[4], true);
  ...
  double const trace = rRelative[0][0] + rRelative[1][1] + rRelative[2][2];

  // Calculate the angle of rotation between the two matrices
  double const angle = std::acos((trace - 1.0) / 2.0) * 180.0 / vnl_math::pi;
