...
SlopeInterceptTest()
{
  ...
  for (unsigned int i = 0; i < 256; i++, ++it)
  {
    ...
    {
      //      return EXIT_FAILURE;
      double const error = itk::Math::abs(it.Value() - (static_cast<double>(i) / 256.0));
      if (error > maxerror)
      {
        maxerror = error;
      }
...
SlopeInterceptWriteTest()
{
  ...
  itk::NiftiImageIO::Pointer const niftiImageIO(itk::NiftiImageIO::New());
  ...
  for (unsigned int i = 0; i < 256; i++, ++it)
  {
    ...
    {
      //      return EXIT_FAILURE;
      double const error = itk::Math::abs(it.Value() - (static_cast<double>(i) / 256.0 - 10.0));
      if (error > maxerror)
      {
        maxerror = error;
      }
