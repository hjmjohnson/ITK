...
MakeNiftiImage(const char * filename)
{
  ...
  typename ImageType::RegionType const region(index, size);
  {
    ...
    { // Fill in left half
      ...
      typename ImageType::RegionType const RPIregion = typename ImageType::RegionType(RPIindex, RPIsize);
  ...
  try
  {
    typename ImageType::Pointer const input = itk::IOTestHelper::ReadImage<ImageType>(std::string(filename));
