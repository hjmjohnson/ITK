...
TestStreamWrite(char * file1, unsigned int numberOfStreams = 0)
{
  ...
  reader->Update();
  bool const imagesEqual = ImagesEqual(consValueImage, reader->GetOutput());

  std::string const componentType = itk::ImageIOBase::GetComponentTypeAsString(vtkIO->GetComponentType());
...
TestStreamRead(char * file1, unsigned int numberOfStreams = 0)
{
  ...
  typename ImageType::SizeValueType const width = (size[TDimension - 1] + numberOfStreams - 1) / numberOfStreams;
  typename ImageType::RegionType const    totalRegion = consValueImage->GetLargestPossibleRegion();
...
itkVTKImageIOStreamTest(int argc, char * argv[])
{
  ...
  unsigned int const numberOfStreams = 2;
  int                status = 0;
