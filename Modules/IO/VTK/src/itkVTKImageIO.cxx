...
{
  IOComponentEnum const compType = GetComponentTypeFromString(pixelType);
...
VTKImageIO::InternalReadImageInformation(std::ifstream & file)
{
  ...
  for (bool readAttribute = false; !readAttribute;)
  {
    ...
    else if (text.find("scalars") < text.length())
    {
      ...
      std::streampos const pos = file.tellg();
...
VTKImageIO::ReadSymmetricTensorBufferAsBinary(std::istream & is, void * buffer, StreamingImageIOBase::SizeType num)
{
  ...
  SizeType const  pixelSize = componentSize * 6;
...
VTKImageIO::Read(void * buffer)
{
  ...
  else
  {
    ...
    std::streampos const dataPos = static_cast<std::streampos>(this->GetHeaderSize());
                  ...
                  const ImageIOBase::SizeType components)
{
  ...
  if (components == 3)
  {
    PrintType const zero(TComponent{});
    PrintType       e12;
    while (i < num)
    {
      // row 1
...
VTKImageIO::Write(const void * buffer)
{
  if (this->RequestedToStream())
  {
    ...
    if (!itksys::SystemTools::FileExists(m_FileName.c_str()))
    {
      ...
      std::streampos const seekPos = this->GetImageSizeInBytes() + this->GetHeaderSize() - 1;
