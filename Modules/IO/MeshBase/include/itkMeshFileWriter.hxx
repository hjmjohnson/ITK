...
MeshFileWriter<TInputMesh>::WritePoints()
{
  ...
  SizeValueType const pointsBufferSize = input->GetNumberOfPoints() * TInputMesh::PointDimension;
...
MeshFileWriter<TInputMesh>::WriteCells()
{
  ...
  SizeValueType const cellsBufferSize = m_MeshIO->GetCellBufferSize();
...
MeshFileWriter<TInputMesh>::CopyCellsToBuffer(Output * data)
{
  ...
  while (cter != cells->End())
  {
    ...
    unsigned int const numberOfPoints = cellPtr->GetNumberOfPoints();
