...
IPLCommonImageIO::Read(void * buffer)
{
  ...
  for (; it != itend; ++it)
  {
    std::string const curfilename = (*it)->GetImageFileName();
    std::ifstream     f;
...
IPLCommonImageIO::ReadImageInformation()
{
  ...
  char              imagePath[IOCommon::ITK_MAXPATHLEN + 1];
  std::string const _imagePath = itksys::SystemTools::CollapseFullPath(FileNameToRead.c_str());
  ...
  bool              isCT = false;
  std::string const modality = m_ImageHeader->modality;
  if (modality == "CT")
  {
    isCT = true;
  }
