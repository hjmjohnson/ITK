...
ArchetypeSeriesFileNames::Scan()
{
  ...
  std::string const origFileName = itksys::SystemTools::GetFilenameName(unixArchetype.c_str());
  ...
  std::string const regExpString = "([0-9]+)";
  ...
  for (std::string::iterator sit = fileName.begin(); sit < fileName.end(); ++sit)
  {
    ...
    if ((*sit) >= '0' && (*sit) <= '9')
    {
      int const sIndex = static_cast<int>(sit - fileName.begin());
  ...
  while (regExpFileNameVectorItr != regExpFileNameVector.end())
  {
    ...
    std::vector<std::string>::iterator const ait = std::find(names.begin(), names.end(), pathPrefix + unixArchetype);
