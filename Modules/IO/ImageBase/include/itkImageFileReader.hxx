...
ImageFileReader<TOutputImage, ConvertPixelTraits>::GenerateOutputInformation()
{
  ...
  if (m_ImageIO.IsNull())
  {
    ...
    {
      std::list<LightObject::Pointer> const allobjects = ObjectFactoryBase::CreateAllInstance("itkImageIOBase");
      if (!allobjects.empty())
      {
...
ImageFileReader<TOutputImage, ConvertPixelTraits>::EnlargeOutputRequestedRegion(DataObject * output)
{
  ...
  typename TOutputImage::RegionType const largestRegion = out->GetLargestPossibleRegion();
  ...
  ImageRegionType const imageRequestedRegion = out->GetRequestedRegion();
...
ImageFileReader<TOutputImage, ConvertPixelTraits>::DoConvertBuffer(const void * inputData, size_t numberOfPixels)
{
  ...
  bool const             isVectorImage(strcmp(this->GetOutput()->GetNameOfClass(), "VectorImage") == 0);
  // TODO:
