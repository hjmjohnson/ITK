...
itkImageSeriesWriterTest(int argc, char * argv[])
{
  ...
  itk::GDCMImageIO::Pointer const io = itk::GDCMImageIO::New();

  // Get the DICOM filenames from the directory
  itk::GDCMSeriesFileNames::Pointer const nameGenerator = itk::GDCMSeriesFileNames::New();
  nameGenerator->SetDirectory(argv[1]);

  using SeriesIdContainer = std::vector<std::string>;
  const SeriesIdContainer & seriesUID = nameGenerator->GetSeriesUIDs();
  std::string const         seriesIdentifier = seriesUID.begin()->c_str();

  auto reader = ReaderType::New();
  reader->SetFileNames(nameGenerator->GetFileNames(seriesIdentifier));
  reader->SetImageIO(io);

  itk::SimpleFilterWatcher const watcher(reader);
  ...
  { // This API is being deprecated. Please use NumericSeriesFileNames in the future
    ...
    itk::SimpleFilterWatcher const watcher2(writer);

    writer->SetInput(rescaler->GetOutput());

    itk::SizeValueType const startIndex = 1;
    writer->SetStartIndex(startIndex);
    ITK_TEST_SET_GET_VALUE(startIndex, writer->GetStartIndex());

    itk::SizeValueType const incrementIndex = 1;
  ...
  { // This is the new API, using the NumericSeriesFileNames (or any other filename generator).
    ...
    ImageNDType::RegionType const region = reader->GetOutput()->GetBufferedRegion();
    ImageNDType::SizeType         size = region.GetSize();

    itk::SizeValueType const startIndex = 0;
    fit->SetStartIndex(startIndex);
    ITK_TEST_SET_GET_VALUE(startIndex, fit->GetStartIndex());

    itk::SizeValueType const endIndex = size[2] - 1;
    ...
    bool const useCompression = false;
