...
{
  itk::MetaImageIO::Pointer const reader = itk::MetaImageIO::New();
  ...
  { // Test string <-> type conversions
    itk::IOComponentEnum const listComponentType[] = { itk::IOComponentEnum::UNKNOWNCOMPONENTTYPE,
    ...
    const char *           listComponentTypeString[] = { "unknown", "unsigned_char",      "char",      "unsigned_short",
                                               ...
                                               "double" };
    itk::IOPixelEnum const listIOPixelType[] = { itk::IOPixelEnum::UNKNOWNPIXELTYPE,
    ...
    { // Test the static version of the string <-> type conversions
      ...
      {
        std::string const componentTypeString = itk::ImageIOBase::GetComponentTypeAsString(listComponentType[i]);
      ...
      {
        std::string const pixelTypeString = itk::ImageIOBase::GetPixelTypeAsString(listIOPixelType[i]);
      ...
      for (size_t i = 0; i < listPixelSize; ++i)
      {
        itk::IOPixelEnum const pixelType = itk::ImageIOBase::GetPixelTypeFromString(listIOPixelTypeString[i]);
    ...
    // Test the non-static version of the string <-> type conversions
    {
      ...
      itk::ImageIOBase::Pointer const imageIOBase =
        itk::ImageIOFactory::CreateImageIO("test", itk::ImageIOFactory::IOFileModeEnum::ReadMode);
      for (size_t i = 0; i < listComponentSize; ++i)
      {
        std::string const componentTypeString = imageIOBase->GetComponentTypeAsString(listComponentType[i]);
        if (componentTypeString.compare(listComponentTypeString[i]) != 0)
        {
          std::cerr << "GetComponentTypeAsString(" << listComponentType[i] << ") should return '"
                    << listComponentTypeString[i] << '\'' << std::endl;
      ...
      for (size_t i = 0; i < listPixelSize; ++i)
      {
        std::string const pixelTypeString = imageIOBase->GetPixelTypeAsString(listIOPixelType[i]);
        if (pixelTypeString.compare(listIOPixelTypeString[i]) != 0)
        {
          std::cerr << "GetPixelTypeAsString(" << listIOPixelType[i] << ") should return " << listIOPixelTypeString[i]
                    << std::endl;
      ...
      for (size_t i = 0; i < listComponentSize; ++i)
      {
        itk::IOComponentEnum const componentType = imageIOBase->GetComponentTypeFromString(listComponentTypeString[i]);
        if (componentType != listComponentType[i])
        {
          std::cerr << "GetComponentTypeFromString('" << listComponentTypeString[i] << "') should return "
                    << listComponentType[i] << std::endl;
      ...
      for (size_t i = 0; i < listPixelSize; ++i)
      {
        itk::IOPixelEnum const pixelType = imageIOBase->GetPixelTypeFromString(listIOPixelTypeString[i]);
        if (pixelType != listIOPixelType[i])
        {
          std::cerr << "GetPixelTypeFromString('" << listIOPixelTypeString[i] << "') should return "
                    << listIOPixelType[i] << std::endl;
  ...
  { // Test SetPixelTypeInfo
    ...
    itk::MetaImageIO::Pointer const imageIO = itk::MetaImageIO::New();
