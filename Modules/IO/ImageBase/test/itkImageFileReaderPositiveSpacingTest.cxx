...
itkImageFileReaderPositiveSpacingTest(int argc, char * argv[])
{
  ...
  reader->Update();
  ImageNDType::Pointer const image = reader->GetOutput();
  ...
  ImageNDType::DirectionType const direction = image->GetDirection();
  std::cout << "Direction: " << '\n' << direction << std::endl;
  ...
  {
    ImageNDType::IndexType const index = it.GetIndex();
