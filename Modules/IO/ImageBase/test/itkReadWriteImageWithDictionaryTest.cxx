...
itkReadWriteImageWithDictionaryTest(int argc, char * argv[])
{
  ...
  std::string const         voxelunitstr = "mm. "; // try to follow analyze format (length matters)
  itk::EncapsulateMetaData<std::string>(inputDictionary, itk::ITK_VoxelUnits, voxelunitstr);
  std::string const datestr = "26-05-2010"; // try to follow analyze format (length matters)
  itk::EncapsulateMetaData<std::string>(inputDictionary, itk::ITK_ExperimentDate, datestr);
  std::string const timestr = "13-44-00.0"; // try to follow analyze format (length matters)
  itk::EncapsulateMetaData<std::string>(inputDictionary, itk::ITK_ExperimentTime, timestr);
  std::string const patientstr = "patientid "; // try to follow analyze format (length matters)
  ...
  ImageType::Pointer const outputImage = reader->GetOutput();
  ...
  for (itk::MetaDataDictionary::ConstIterator it = inputDictionary.Begin(); it != inputDictionary.End(); ++it)
  {
    ...
    {
      itk::MetaDataDictionary::ConstIterator const it2 = outputDictionary.Find(it->first);
