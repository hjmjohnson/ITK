        ...
        const SizeType              frameCount)
{
  ...
  for (SizeType f = 0; f < frameCount; ++f)
  {
    ...
    {
      double const tmp = static_cast<double>(buffer[i]) * slopes[f] + offsets[f];
...
ReadJCAMPDX(const std::string & filename, MetaDataDictionary & dict)
{
  ...
  while (std::getline(paramsStream, line))
  {
    ...
    std::string::size_type const epos = line.find('=', 3);
    if (epos == std::string::npos)
    {
      itkGenericExceptionMacro("Invalid Bruker JCAMPDX parameter line (Missing =): " << line);
    }

    std::string const parname = line.substr(3, epos - 3);
    ...
    if (par[0] == '(')
    {
      ...
      if (leftBracket == std::string::npos)
      {
        ...
        if (left != std::string::npos)
        {
          ...
          {
            std::string::size_type const right = lines.find('>', left + 1);
...
Bruker2dseqImageIO::Read(void * buffer)
{
  ...
  MetaDataDictionary const & dict = this->GetMetaDataDictionary();
...
Bruker2dseqImageIO::CanReadFile(const char * FileNameToRead)
{
  ...
  std::string const fileVisu = itksys::SystemTools::GetFilenamePath(file2Dseq) + "/visu_pars";
...
Bruker2dseqImageIO::ReadImageInformation()
{
  ...
  std::string path2Dseq = itksys::SystemTools::CollapseFullPath(this->m_FileName);
  itksys::SystemTools::ConvertToUnixSlashes(path2Dseq);
  std::string const pathVisu = itksys::SystemTools::GetFilenamePath(path2Dseq) + "/visu_pars";
  ...
  std::string const methodFilename = itksys::SystemTools::GetFilenamePath(path2Dseq) + "/../../method";
  ...
  else
  {
    ...
    if (brukerDim == 2)
    {
      ...
      else
      {
        ...
        vnl_vector<double> const slice1(&position[0], 3);
        vnl_vector<double> const slice2(&position[3], 3);
        vnl_vector<double> const diff = slice2 - slice1;
      ...
      { // There appears to be a bug in 2dseq orientations for Coronal 2D slices.
        ...
        // changing.
        vnl_vector<double> const corner1(&position[0], 3);
        vnl_vector<double> const corner2(&position[3], 3);
    ...
    vnl_matrix<double> const dirMatrix(&orient[0], 3, 3);
    ...
    vnl_vector<double> const corner(&position[0], 3);
