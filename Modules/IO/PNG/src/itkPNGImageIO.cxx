...
PNGImageIO::CanReadFile(const char * file)
{
  ...
  PNGFileWrapper const pngfp(file, "rb");
  if (pngfp.m_FilePointer == nullptr)
  {
    return false;
  }
  unsigned char header[8];
  size_t const  temp = fread(header, 1, 8, pngfp.m_FilePointer);
  if (temp != 8)
  {
    return false;
  }
...
PNGImageIO::Read(void * buffer)
{
  ...
  PNGFileWrapper const pngfp(this->GetFileName(), "rb");
  ...
  unsigned char header[8];
  size_t const  temp = fread(header, 1, 8, fp);
  if (temp != 8)
  {
...
PNGImageIO::ReadImageInformation()
{
  ...
  // use this class so return will call close
  PNGFileWrapper const pngfp(m_FileName.c_str(), "rb");
  FILE *               fp = pngfp.m_FilePointer;
  if (!fp)
  {
    return;
...
{
  // use this class so return will call close
  PNGFileWrapper const pngfp(fileName.c_str(), "wb");
  FILE *               fp = pngfp.m_FilePointer;
  ...
  int                colorType;
  unsigned int const numComp = this->GetNumberOfComponents();
  switch (numComp)
  {
    case 1:
  ...
  png_uint_32 const width = this->GetDimensions(0);
  const double      colSpacing = m_Spacing[0];

  png_uint_32 const height = (m_NumberOfDimensions > 1) ? this->GetDimensions(1) : 1;
