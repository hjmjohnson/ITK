...
itkNrrdImageIOTestGenerateRandomImage(unsigned int size)
{
  ...
  typename itk::RandomImageSource<ImageType>::Pointer const source = itk::RandomImageSource<ImageType>::New();
...
itkNrrdImageIOTestReadWriteTest(std::string fn, unsigned int size, std::string inputFile, bool compression = false)
{
  ...
  typename itk::ImageFileReader<ImageType>::Pointer const reader = itk::ImageFileReader<ImageType>::New();
  typename itk::ImageFileWriter<ImageType>::Pointer const writer = itk::ImageFileWriter<ImageType>::New();

  itk::NrrdImageIO::Pointer const io = itk::NrrdImageIO::New();
  ...
  unsigned long const    dim = NRRD_DIM_MAX + 1;
  ...
  if (inputFile != "null")
  {
    typename itk::ImageFileReader<ImageType>::Pointer const tmpReader = itk::ImageFileReader<ImageType>::New();
