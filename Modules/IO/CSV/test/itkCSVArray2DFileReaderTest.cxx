...
itkCSVArray2DFileReaderTest(int argc, char * argv[])
{
  ...
  DataFrameObjectType::Pointer const dfo = reader->GetOutput();

  // Test the matrix
  using MatrixType = itk::Array2D<double>;
  MatrixType const test_matrix = dfo->GetMatrix();
  ...
  // Test Row Names
  std::vector<std::string> const test_row_names = dfo->GetRowHeaders();
  ...
  // Test Column Names
  std::vector<std::string> const test_col_names = dfo->GetColumnHeaders();
  ...
  std::vector<double> const test_row_1 = dfo->GetRow(1);
  ...
  std::vector<double> const test_row_Jan = dfo->GetRow("Jan");
  ...
  std::vector<double> const test_col_2 = dfo->GetColumn(2);
  ...
  std::vector<double> const test_col_Africa = dfo->GetColumn("Africa");
  ...
  try
  {
    std::vector<double> const test_row_Oct = dfo->GetRow("Oct");
  ...
  try
  {
    ...
    std::vector<double> const test_col_Eur = dfo->GetColumn("Eur");
  ...
  try
  {
    double const test_item1 = dfo->GetData("Feb", "Europe");
