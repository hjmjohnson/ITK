...
testArray(const itk::Array2D<T> & m1, const itk::Array2D<T> & m2)
{
  ...
  for (unsigned int i = 0; i < m1.rows(); ++i)
  {
    for (unsigned int j = 0; j < m1.cols(); ++j)
    {
      ...
      // quiet NaN.
      bool const m1_isNaN = (itk::Math::ExactlyEquals(m1[i][j], m1[i][j]));
      bool const m2_isNaN = (itk::Math::ExactlyEquals(m2[i][j], m2[i][j]));
...
itkCSVArray2DFileReaderWriterTest(int argc, char * argv[])
{
  ...
  int const fail1 = itkCSVFileReaderWriterTest_Func(argc, argv, false);

  // test reading and writing data with headers
  std::cout << std::endl << "Test With Headers." << std::endl;
  int const fail2 = itkCSVFileReaderWriterTest_Func(argc, argv, true);
