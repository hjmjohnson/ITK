...
HDF5TransformIOTemplate<TParametersValueType>::CanReadFile(const char * fileName)
{
  ...
  try
  {
    htri_t const ishdf5 = H5Fis_hdf5(fileName);
    ...
    H5::H5File const h5file(fileName, H5F_ACC_RDONLY);
...
HDF5TransformIOTemplate<TParametersValueType>::CanWriteFile(const char * fileName)
{
  ...
  std::string const ext(itksys::SystemTools::GetFilenameLastExtension(fileName));
                                                               ...
                                                               const ParametersType & parameters)
{
  ...
  H5::DataSpace const paramSpace(1, &dim);
...
HDF5TransformIOTemplate<TParametersValueType>::ReadParameters(const std::string & DataSetName) const -> ParametersType
{
  ...
  H5::FloatType const ParamType = paramSet.getFloatType();
  ...
  -> FixedParametersType
{
  ...
  H5::DataSet       paramSet = this->m_H5File->openDataSet(DataSetName);
  H5T_class_t const Type = paramSet.getTypeClass();
  if (Type != H5T_FLOAT)
  {
    itkExceptionMacro("Wrong data type for " << DataSetName << "in HDF5 File");
  }
...
HDF5TransformIOTemplate<TParametersValueType>::Read()
{
  ...
  try
  {
    ...
    {
      std::string const transformName(GetTransformName(i));
      ...
      std::string transformType;
      {
        hsize_t const       numStrings(1);
        H5::DataSpace const strSpace(1, &numStrings);
        H5::StrType const   typeType(H5::PredType::C_S1, H5T_VARIABLE);
      ...
      if (transformType.find("CompositeTransform") == std::string::npos)
      {
        ...
        FixedParametersType const fixedparams(this->ReadFixedParameters(fixedParamsName));
        ...
        ParametersType const params = this->ReadParameters(paramsName);
...
{
  std::string const transformName(GetTransformName(transformIndex));
  ...
  else
  {
    ...
    FixedParametersType const FixedtmpArray = curTransform->GetFixedParameters();
    const std::string         fixedParamsName(transformName + transformFixedName);
    this->WriteFixedParameters(fixedParamsName, FixedtmpArray);
    // parameters
    ParametersType const tmpArray = curTransform->GetParameters();
...
#  error The selected version of HDF5 library does not support setting backwards compatibility at run-time.\
  Please use a different version of HDF5, e.g. the one bundled with ITK (by setting ITK_USE_SYSTEM_HDF5 to OFF).
    ...
    std::string const compositeTransformType = transformList.front()->GetTransformTypeAsString();
    ...
    typename ConstTransformListType::const_iterator const end = transformList.end();
