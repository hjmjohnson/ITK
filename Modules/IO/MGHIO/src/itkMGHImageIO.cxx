...
int
MGHImageIO ::TRead(TOutType & outValue)
{
...
int
MGHImageIO ::TWrite(const TInType inValue)
{
...
int
MGHImageIO ::TWrite(const char * buf, const unsigned long count)
{
  if (this->m_IsCompressed)
  {
...
MGHImageIO ::IsCompressedFilename(const std::string fname)
{
  ...
  {
    const std::string fnameWithoutLastExtension = itksys::SystemTools::GetFilenameWithoutLastExtension(fname);
    const std::string penultimateExtension = itksys::SystemTools::GetFilenameLastExtension(fnameWithoutLastExtension);
...
MGHImageIO ::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  os << indent << "Data Dimensions: (" << m_Dimensions[0] << strSep << m_Dimensions[1] << strSep << m_Dimensions[2]
     << ")\n"
...
MGHImageIO ::ReadVolumeHeader()
{
  ...
  gzseek(
    this->m_GZFile, FS_WHOLE_HEADER_SIZE + (m_NumberOfComponents * numValues * this->GetComponentSize()), SEEK_SET);
  ...
  if (this->TRead<float, float>(fBufTR))
  {
    ...
    itk::EncapsulateMetaData<float>(thisDic, std::string("TR"), fBufTR);

    // try to read flipAngle
    float fBufFA;
    if (this->TRead<float, float>(fBufFA))
    {
      itk::EncapsulateMetaData<float>(thisDic, std::string("FlipAngle"), fBufFA);
      // TE
      float fBufTE;
      if (this->TRead<float, float>(fBufTE))
      {
        itk::EncapsulateMetaData<float>(thisDic, std::string("TE"), fBufTE);
        // TI
        float fBufTI;
        if (this->TRead<float, float>(fBufTI))
        {
          itk::EncapsulateMetaData<float>(thisDic, std::string("TI"), fBufTI);
          // FOV
          float fBufFOV;
          if (this->TRead<float, float>(fBufFOV))
          {
            itk::EncapsulateMetaData<float>(thisDic, std::string("FoV"), fBufFOV);
          }
        }
      }
    }
...
MGHImageIO ::Read(void * pData)
{
  ...
  if (m_NumberOfComponents > 1)
  {
    ...
    for (unsigned int frameIndex = 0; frameIndex < m_NumberOfComponents; ++frameIndex)
    {
      // read current frame
      ...
      for (unsigned int ui = 0; ui < numPixels; ++ui, pSrc += componentSize, pDst += pixelSize)
      {
...
void
MGHImageIO ::SwapBytesIfNecessary(void * const buffer, const unsigned long numberOfPixels)
{
...
MGHImageIO ::WriteHeader()
{
  ...
  switch (m_ComponentType)
  {
    ...
    break;
    default:
      itkExceptionMacro(<< "MGHImageIO supports unsigned char, int, float and short");
  }
...
void
MGHImageIO ::WriteData(const void * buffer)
{
  // swap bytes if necessary
  const unsigned int      numPixels = m_Dimensions[0] * m_Dimensions[1] * m_Dimensions[2];
...
MGHImageIO ::PermuteFrameValues(const void * buffer, char * tempmemory)
{
  ...
  for (unsigned int pixelIndex = 0; pixelIndex < numPixels; ++pixelIndex, pDst += valueSize)
  {
    for (unsigned int componentIndex = 0; componentIndex < m_NumberOfComponents; ++componentIndex, pSrc += valueSize)
    {
