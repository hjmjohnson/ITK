...
TxtTransformIOTemplate<TParametersValueType>::ReadComponentFile(std::string Value)
{
  ...
  std::string const filePath = itksys::SystemTools::GetFilenamePath(this->GetFileName()) + "/";

  /* Use TransformFileReader to read each component file. */
  auto              reader = TransformFileReaderTemplate<TParametersValueType>::New();
  std::string const componentFullPath = filePath + Value;
  ...
  TransformPointer const transform = reader->GetTransformList()->front().GetPointer();
...
TxtTransformIOTemplate<TParametersValueType>::Read()
{
  ...
  while (std::getline(in, line))
  {
    ...
    std::string const Name = trim(line.substr(0, end));
...
TxtTransformIOTemplate<TParametersValueType>::Write()
{
  ...
  for (typename ConstTransformListType::const_iterator it = transformList.begin(); it != end; ++it, ++count)
  {
    ...
    {
      {
        vnl_vector<ParametersValueType> const TempArray = (*it)->GetParameters();
        out << "Parameters: "; // << TempArray << std::endl;
        itk_impl_details::print_vector(out, TempArray);
        out << std::endl;
      }
      {
        vnl_vector<FixedParametersValueType> const FixedTempArray = (*it)->GetFixedParameters();
