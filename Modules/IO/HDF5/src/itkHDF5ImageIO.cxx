...
{
  hsize_t const       numScalars(1);
  H5::DataSpace const scalarSpace(1, &numScalars);
  H5::PredType const  scalarType = H5::PredType::NATIVE_HBOOL;
...
HDF5ImageIO::WriteScalar(const std::string & path, const long value)
{
  hsize_t const       numScalars(1);
  H5::DataSpace const scalarSpace(1, &numScalars);
  H5::PredType const  scalarType = H5::PredType::NATIVE_INT;
  H5::PredType const  attrType = H5::PredType::NATIVE_HBOOL;
  H5::DataSet         scalarSet = m_H5File->createDataSet(path, scalarType, scalarSpace);
  //
  // HDF5 can't distinguish
...
HDF5ImageIO::WriteScalar(const std::string & path, const unsigned long value)
{
  hsize_t const       numScalars(1);
  H5::DataSpace const scalarSpace(1, &numScalars);
  H5::PredType const  scalarType = H5::PredType::NATIVE_UINT;
  H5::PredType const  attrType = H5::PredType::NATIVE_HBOOL;
  H5::DataSet         scalarSet = m_H5File->createDataSet(path, scalarType, scalarSpace);
  //
  // HDF5 can't distinguish
...
HDF5ImageIO::WriteScalar(const std::string & path, const long long value)
{
  hsize_t const       numScalars(1);
  H5::DataSpace const scalarSpace(1, &numScalars);
  H5::PredType const  scalarType = H5::PredType::STD_I64LE;
  H5::PredType const  attrType = H5::PredType::NATIVE_HBOOL;
  H5::DataSet         scalarSet = m_H5File->createDataSet(path, scalarType, scalarSpace);
  //
  // HDF5 can't distinguish
...
HDF5ImageIO::WriteScalar(const std::string & path, const unsigned long long value)
{
  hsize_t const       numScalars(1);
  H5::DataSpace const scalarSpace(1, &numScalars);
  H5::PredType const  scalarType = H5::PredType::STD_U64LE;
  H5::PredType const  attrType = H5::PredType::NATIVE_HBOOL;
  H5::DataSet         scalarSet = m_H5File->createDataSet(path, scalarType, scalarSpace);
  //
  // HDF5 can't distinguish
...
HDF5ImageIO::WriteScalar(const std::string & path, const TScalar & value)
{
  hsize_t const       numScalars(1);
  H5::DataSpace const scalarSpace(1, &numScalars);
  H5::PredType const  scalarType = GetType<TScalar>();
  H5::DataSet         scalarSet = m_H5File->createDataSet(path, scalarType, scalarSpace);
  scalarSet.write(&value, scalarType);
  scalarSet.close();
}
...
HDF5ImageIO::ReadScalar(const std::string & DataSetName)
{
  ...
  H5::DataSpace const Space = scalarSet.getSpace();
...
{
  hsize_t const       numStrings(1);
  H5::DataSpace const strSpace(1, &numStrings);
  H5::StrType const   strType(H5::PredType::C_S1, H5T_VARIABLE);
...
{
  hsize_t const       dim(vec.size());
  H5::DataSpace const vecSpace(1, &dim);
  H5::PredType const  vecType = GetType<TScalar>();
...
HDF5ImageIO::ReadVector(const std::string & DataSetName)
{
  ...
  H5::DataSpace const  Space = vecSet.getSpace();
...
HDF5ImageIO::WriteDirections(const std::string & path, const std::vector<std::vector<double>> & dir)
{
  ...
  H5::DataSpace const dirSpace(2, dim);
...
HDF5ImageIO::ReadDirections(const std::string & path)
{
  ...
  H5::DataSpace const              dirSpace = dirSet.getSpace();
  hsize_t                          dim[2];
  ...
  H5::FloatType const dirType = dirSet.getFloatType();
...
HDF5ImageIO::CanReadFile(const char * FileNameToRead)
{
  ...
  try
  {
    ...
    htri_t const ishdf5 = H5Fis_hdf5(FileNameToRead);
    ...
    H5::H5File const h5file(FileNameToRead, H5F_ACC_RDONLY);
...
HDF5ImageIO::ReadImageInformation()
{
  try
  {
    ...
    *(m_VoxelDataSet) = m_H5File->openDataSet(VoxelDataName);
    ...
    H5::DataSpace const imageSpace = imageSet.getSpace();
    //
    ...
    {
      hsize_t const nDims = imageSpace.getSimpleExtentNdims();
    ...
    MetaDataGroupName += "/";
    H5::Group const metaGroup(m_H5File->openGroup(MetaDataGroupName));
    for (unsigned int i = 0; i < metaGroup.getNumObjs(); ++i)
    {
      H5std_string const name = metaGroup.getObjnameByIdx(i);

      std::string localMetaDataName(MetaDataGroupName);
      localMetaDataName += name;
      H5::DataSet const   metaDataSet = m_H5File->openDataSet(localMetaDataName);
      H5::DataType const  metaDataType = metaDataSet.getDataType();
      H5::DataSpace const metaDataSpace = metaDataSet.getSpace();
      ...
      else
      {
        ...
        {
          std::string const val = this->ReadString(localMetaDataName);
...
HDF5ImageIO::SetupStreaming(H5::DataSpace * imageSpace, H5::DataSpace * slabSpace)
{
  ...
  //
  int const numComponents = this->GetNumberOfComponents();
...
{
  ImageIORegion const            regionToRead = this->GetIORegion();
  ImageIORegion::SizeType const  size = regionToRead.GetSize();
  ImageIORegion::IndexType const start = regionToRead.GetIndex();

  H5::DataType const voxelType = m_VoxelDataSet->getDataType();
...
#  error The selected version of HDF5 library does not support setting backwards compatibility at run-time.\
  Please use a different version of HDF5, e.g. the one bundled with ITK (by setting ITK_USE_SYSTEM_HDF5 to OFF).
    ...
    std::string const typeVal(ComponentToString(this->GetComponentType()));
    ...
    H5::DataSpace const imageSpace(numDims, dims.get());
    H5::PredType const  dataType = ComponentToPredType(this->GetComponentType());

    // set up properties for chunked, compressed writes.
    // in this case, set the chunk size to be the N-1 dimension
    // region
    H5::DSetCreatPropList const plist;
    ...
    *(m_VoxelDataSet) = m_H5File->createDataSet(VoxelDataName, dataType, imageSpace, plist);
    ...
    for (; it != end; ++it)
    {
      ...
      // std::string
      {
        ...
        {
          std::string const val = stdStringObj->GetMetaDataObjectValue();
          this->WriteString(objName, val);
          continue;
        }
      }
