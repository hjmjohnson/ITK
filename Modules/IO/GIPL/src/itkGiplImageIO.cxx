...
GiplImageIO::CanReadFile(const char * filename)
{
  ...
  bool const extensionFound = CheckExtension(filename);
...
GiplImageIO::CanWriteFile(const char * name)
{
  ...
  bool const extensionFound = CheckExtension(name);
...
GiplImageIO::ReadImageInformation()
{
  ...
  for (char & i : line1)
  {
    if (m_IsCompressed)
    {
      gzread(m_Internal->m_GzFile, (char *)&i, static_cast<unsigned int>(sizeof(char)));
    }
    else
    {
      m_Ifstream.read((char *)&i, sizeof(char));
    }
  }
  ...
  for (float & i : matrix)
  {
    if (m_IsCompressed)
    {
      gzread(m_Internal->m_GzFile, (char *)&i, static_cast<unsigned int>(sizeof(float)));
    }
    else
    {
      m_Ifstream.read((char *)&i, sizeof(float));
    }

    if (m_ByteOrder == IOByteOrderEnum::BigEndian)
    {
      ByteSwapper<float>::SwapFromSystemToBigEndian(&i);
    }
    else if (m_ByteOrder == IOByteOrderEnum::LittleEndian)
    {
      ByteSwapper<float>::SwapFromSystemToLittleEndian(&i);
    }
  }
