...
ReadImage(const std::string & fileName)
{
  ...
  itk::MetaImageIO::Pointer const io = itk::MetaImageIO::New();
...
itkMetaImageIOMetaDataTest(int argc, char * argv[])
{
  ...
  }
  {
    ...
    double const      value(7.891011);
    itk::EncapsulateMetaData<double>(dict, key, value);
  }
  {
    // Add float
    std::string const key("float");
    float const       value(1.23456);
    itk::EncapsulateMetaData<float>(dict, key, value);
  }
  {
    // Add long
    std::string const key("long");
    long const        value(-31415926);
    itk::EncapsulateMetaData<int>(dict, key, value);
  }
  {
    // Add unsigned long
    std::string const   key("unsigned_long");
    unsigned long const value(27182818);
    itk::EncapsulateMetaData<unsigned long>(dict, key, value);
  }
  {
    // Add long long
    std::string const key("long_long");
    long long const   value(-8589934592ll);
    itk::EncapsulateMetaData<long long>(dict, key, value);
  }
  {
    // Add unsigned long long
    std::string const        key("unsigned_long_long");
    unsigned long long const value(8589934592ull);
    itk::EncapsulateMetaData<unsigned long long>(dict, key, value);
  }
  {
    // Add int
    std::string const key("int");
    int const         value(-3141592);
    itk::EncapsulateMetaData<int>(dict, key, value);
  }
  {
    // Add unsigned int
    std::string const  key("unsigned_int");
    unsigned int const value(2718281);
    itk::EncapsulateMetaData<unsigned int>(dict, key, value);
  }
  {
    // Add short
    std::string const key("short");
    short const       value(-16384);
    itk::EncapsulateMetaData<short>(dict, key, value);
  }
  {
    // Add short
    std::string const  key("unsigned_short");
    unsigned int const value(8192);
    itk::EncapsulateMetaData<unsigned short>(dict, key, value);
  }
  {
    // Add char
    std::string const key("char");
    char const        value('c');
  ...
  ImageType::Pointer const randImage2 = ReadImage<ImageType>(argv[1]);
