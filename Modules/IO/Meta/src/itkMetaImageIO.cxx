...
MetaImageIO::ReadImageInformation()
{
  ...
  int const dictFields = m_MetaImage.GetNumberOfAdditionalReadFields();
  for (int f = 0; f < dictFields; ++f)
  {
    std::string const key(m_MetaImage.GetAdditionalReadFieldName(f));
    std::string const value(m_MetaImage.GetAdditionalReadFieldValue(f));
...
MetaImageIO::Write(const void * buffer)
{
  ...
  int const nChannels = this->GetNumberOfComponents();
  ...
  if (numberOfDimensions == 3)
  {
    ...
    AnatomicalOrientation const coordOrient(dir);
                                               ...
                                               const ImageIORegion & largestPossibleRegion)
{
  ...
  else if (pasteRegion != largestPossibleRegion)
  {
    ...
    Pointer const headerImageIOReader = Self::New();
