...
ComputeCG(TImageType::Pointer img)
{
  ...
  while (!it.IsAtEnd())
  {
    ...
    sumMass += value;
    TImageType::IndexType const indexPosition = it.GetIndex();
...
itkShrinkImagePreserveObjectPhysicalLocations(int, char *[])
{
  ...
  PyramidFilterType::Pointer const MyPyramid = MakeTwoLevelPyramid(image);
  TImageType::Pointer const        ReallySmallImage = MyPyramid->GetOutput(0);
  TImageType::Pointer const        SmallImage = MyPyramid->GetOutput(1);

  itk::ShrinkImageFilter<TImageType, TImageType>::Pointer const Shrinkfilter =
    itk::ShrinkImageFilter<TImageType, TImageType>::New();
  Shrinkfilter->SetInput(image);
  Shrinkfilter->SetShrinkFactors(4);
  Shrinkfilter->Update();
  TImageType::Pointer const ShrinkSmallImage = Shrinkfilter->GetOutput();

  itk::DiscreteGaussianImageFilter<TImageType, TImageType>::Pointer const smoother =
  ...
  TImageType::Pointer const GaussianImage = smoother->GetOutput();

  itk::ShrinkImageFilter<TImageType, TImageType>::Pointer const smootherShrinkfilter =
    itk::ShrinkImageFilter<TImageType, TImageType>::New();
  smootherShrinkfilter->SetInput(GaussianImage);
  smootherShrinkfilter->SetShrinkFactors(4);
  smootherShrinkfilter->Update();
  TImageType::Pointer const GaussianShrinkSmallImage = smootherShrinkfilter->GetOutput();
