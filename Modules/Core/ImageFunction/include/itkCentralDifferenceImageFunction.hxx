...
CentralDifferenceImageFunction<TInputImage, TCoordRep, TOutputType>::SetInputImage(const TInputImage * inputData)
{
  if (inputData != this->m_Image)
  {
    ...
    {
      SizeValueType const nComponents = OutputConvertType::GetNumberOfComponents();
      if (nComponents > 0)
      {
  ...
  OutputTypeSpecializationStructType<OutputType>) const
{
  ...
  for (unsigned int dim = 0; dim < MaxDims; ++dim)
  {
    PointValueType const offset = static_cast<PointValueType>(0.5) * spacing[dim];
    ...
    PointValueType const delta = neighPoint2[dim] - neighPoint1[dim];
  ...
  OutputTypeSpecializationStructType<Type>) const
{
  ...
  for (unsigned int nc = 0; nc < numberComponents; ++nc)
  {
    for (unsigned int dim = 0; dim < MaxDims; ++dim)
    {
      ...
      {
        OutputValueType const left = InputPixelConvertType::GetNthComponent(nc, neighPixels[dim][0]);
        OutputValueType const right = InputPixelConvertType::GetNthComponent(nc, neighPixels[dim][1]);
