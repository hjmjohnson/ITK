  ...
  unsigned int                splineOrder) const
{
  ...
  switch (splineOrder)
  {
    case 3:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        double const w = x[n] - static_cast<double>(EvaluateIndex[n][1]);
    ...
    case 1:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        double const w = x[n] - static_cast<double>(EvaluateIndex[n][0]);
        weights[n][1] = w;
        weights[n][0] = 1.0 - w;
      }
      break;
    ...
    case 4:
    {
      ...
      {
        /* x */
        double const w = x[n] - static_cast<double>(EvaluateIndex[n][2]);
        double const w2 = w * w;
        double const t = (1.0 / 6.0) * w2;
        weights[n][0] = 0.5 - w;
        weights[n][0] *= weights[n][0];
        weights[n][0] *= (1.0 / 24.0) * weights[n][0];
        double const t0 = w * (t - 11.0 / 24.0);
        double const t1 = 19.0 / 96.0 + w2 * (0.25 - t);
        weights[n][1] = t1 + t0;
        weights[n][3] = t1 - t0;
    ...
    case 5:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        ...
        w2 -= w;
        double const w4 = w2 * w2;
        w -= 0.5;
        double const t = w2 * (w2 - 3.0);
  ...
  unsigned int                splineOrder) const
{
  ...
  switch (derivativeSplineOrder)
  {
    ...
    case 1:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        double const w = x[n] + 0.5 - static_cast<double>(EvaluateIndex[n][1]);
        // w2 = w;
        double const w1 = 1.0 - w;
    ...
    case 2:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        double const w = x[n] + .5 - static_cast<double>(EvaluateIndex[n][2]);
        double const w2 = 0.75 - w * w;
        double const w3 = 0.5 * (w - w2 + 1.0);
        double const w1 = 1.0 - w2 - w3;
    ...
    case 3:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        double const w = x[n] + 0.5 - static_cast<double>(EvaluateIndex[n][2]);
        double const w4 = (1.0 / 6.0) * w * w * w;
        double const w1 = (1.0 / 6.0) + 0.5 * w * (w - 1.0) - w4;
        double const w3 = w + w1 - 2.0 * w4;
        double const w2 = 1.0 - w1 - w3 - w4;
    ...
    case 4:
    {
      for (unsigned int n = 0; n < ImageDimension; ++n)
      {
        double const w = x[n] + .5 - static_cast<double>(EvaluateIndex[n][3]);
        double const t2 = w * w;
        double const t = (1.0 / 6.0) * t2;
        double       w1 = 0.5 - w;
        w1 *= w1;
        w1 *= (1.0 / 24.0) * w1;
        double const t0 = w * (t - 11.0 / 24.0);
        double const t1 = 19.0 / 96.0 + t2 * (0.25 - t);
        double const w2 = t1 + t0;
        double const w4 = t1 - t0;
        double const w5 = w1 + t0 + 0.5 * w;
        double const w3 = 1.0 - w1 - w2 - w4 - w5;
                                                      ...
                                                      vnl_matrix<double> &        weightsDerivative) const
{
  ...
  for (unsigned int p = 0; p < m_MaxNumberInterpolationPoints; ++p)
  {
    ...
    for (unsigned int n = 1; n < ImageDimension; ++n)
    {
      ...
      coefficientIndex[n] = (evaluateIndex)[n][indx];
      double const tmpW = (weights)[n][indx];
      w *= tmpW;
      w1 *= tmpW;
    }
    double const tmpV = m_Coefficients->GetPixel(coefficientIndex);
    value += w * tmpV;
