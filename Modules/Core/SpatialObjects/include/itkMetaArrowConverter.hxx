...
MetaArrowConverter<VDimension>::MetaObjectToSpatialObject(const MetaObjectType * mo) -> SpatialObjectPointer
{
  ...
  ArrowSpatialObjectPointer const arrowSO = ArrowSpatialObjectType::New();

  this->MetaObjectToSpatialObjectBase(mo, arrowSO);

  float const lengthInObjectSpace = metaArrow->Length();
...
{
  ArrowSpatialObjectConstPointer const arrowSO = dynamic_cast<const ArrowSpatialObjectType *>(spatialObject);
  if (arrowSO.IsNull())
  {
  ...
  float const metaLength = arrowSO->GetLengthInObjectSpace();
