...
MetaEllipseConverter<VDimension>::MetaObjectToSpatialObject(const MetaObjectType * mo) -> SpatialObjectPointer
{
  ...
  EllipseSpatialObjectPointer const ellipseSO = EllipseSpatialObjectType::New();
...
{
  EllipseSpatialObjectConstPointer const ellipseSO = dynamic_cast<const EllipseSpatialObjectType *>(so);
  if (ellipseSO.IsNull())
  {
