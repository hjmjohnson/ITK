...
{
  FieldType const field(itksys::SystemTools::LowerCase(name), value);
...
{
  std::string const charname = this->TranslateEnumToChar(name);
...
DTITubeSpatialObjectPoint<TPointDimension>::AddField(DTITubeSpatialObjectPointFieldEnum name, float value)
{
  ...
  if (!charname.empty())
  {
    FieldType const field(itksys::SystemTools::LowerCase(charname).c_str(), value);
    m_Fields.push_back(field);
  }
  else
  {
