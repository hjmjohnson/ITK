...
MetaContourConverter<VDimension>::MetaObjectToSpatialObject(const MetaObjectType * mo) -> SpatialObjectPointer
{
  ...
  ContourSpatialObjectPointer const contourSO = ContourSpatialObjectType::New();
...
{
  ContourSpatialObjectConstPointer const contourSO = dynamic_cast<const ContourSpatialObjectType *>(so);
  if (contourSO.IsNull())
  {
