...
MetaLineConverter<VDimension>::MetaObjectToSpatialObject(const MetaObjectType * mo) -> SpatialObjectPointer
{
  ...
  LineSpatialObjectPointer const lineSO = LineSpatialObjectType::New();
...
{
  LineSpatialObjectConstPointer const lineSO = dynamic_cast<const LineSpatialObjectType *>(spatialObject);
  if (lineSO.IsNull())
  {
