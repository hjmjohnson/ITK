  ...
  -> MetaObjectType *
{
  ...
  typename MeshType::ConstPointer const mesh = meshSO->GetMesh();
  ...
  while (it_cells != cells->End())
  {
    unsigned int const celldim = (*it_cells)->Value()->GetNumberOfPoints();
    ...
    CellGeometryEnum const geom = (*it_cells)->Value()->GetType();
