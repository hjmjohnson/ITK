...
MetaGroupConverter<VDimension>::MetaObjectToSpatialObject(const MetaObjectType * mo) -> SpatialObjectPointer
{
  ...
  GroupSpatialObjectPointer const groupSO = GroupSpatialObjectType::New();
...
{
  GroupSpatialObjectConstPointer const groupSO = dynamic_cast<const GroupSpatialObjectType *>(so);
  if (groupSO.IsNull())
  {
