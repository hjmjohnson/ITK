...
public:
  ...
  MetaObjectToSpatialObject(const MetaObjectType * mo) override
  {
    ...
    DummySpatialObjectPointer const dummySO = DummySpatialObjectType::New();
  ...
  {
    DummySpatialObjectConstPointer const dummySO = dynamic_cast<const DummySpatialObjectType *>(spatialObject);
    if (dummySO.IsNull())
    {
...
itkNewMetaObjectTypeTest(int, char *[])
{
  ...
  GroupType::Pointer const group(GroupType::New());

  DummyType::Pointer const dummy(DummyType::New());
  ...
  DummyConverterType::Pointer const dummyConverter(DummyConverterType::New());
  ...
  unsigned int const transformPrecision = 6;
  ...
  SpatialObjectType::Pointer const myScene = converter->CreateSpatialObjectScene(metaScene);
  ...
  {
    std::string const childType((*obj)->GetTypeName());
    ...
    DummyType::Pointer const p = dynamic_cast<DummyType *>(obj->GetPointer());
    if (p.IsNull())
    {
