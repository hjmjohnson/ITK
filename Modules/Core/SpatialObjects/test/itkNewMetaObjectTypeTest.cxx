...
public:
  ...
  MetaObjectToSpatialObject(const MetaObjectType * mo) override
  {
    ...
    if (dummyMO == nullptr)
    {
      itkExceptionStringMacro("Can't convert MetaObject to MetaDummy");
    }
  ...
  SpatialObjectToMetaObject(const SpatialObjectType * spatialObject) override
  {
    ...
    if (dummySO.IsNull())
    {
      itkExceptionStringMacro("Can't downcast SpatialObject to DummySpatialObject");
    }
...
itkNewMetaObjectTypeTest(int, char *[])
{
  ...
  for (auto obj = mySceneChildren->begin(); obj != mySceneChildren->end(); ++obj)
  {
    ...
    if (itk::Math::Absolute(value - Pi) > 0.00001)
    {
