...
{
  // Observe the filter
  unsigned long const progressTag = filter->AddObserver(ProgressEvent(), m_CallbackCommand);
  unsigned long const startTag = filter->AddObserver(StartEvent(), m_CallbackCommand);
...
ProgressAccumulator::ReportProgress(Object * who, const EventObject & event)
{
  ...
  if (typeid(event) == typeid(pe))
  {
    ...
    for (it = m_FilterRecord.begin(); it != m_FilterRecord.end(); ++it)
    {
      float const progress = it->Filter->GetProgress();
