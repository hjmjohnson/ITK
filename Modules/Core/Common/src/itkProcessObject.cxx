...
{
  DataObjectPointerArraySizeType const nb = this->GetNumberOfIndexedInputs();
  if (nb > 0)
  {
...
ProcessObject::SetOutput(const DataObjectIdentifierType & name, DataObject * output)
{
  ...
  // is not enough.
  DataObjectIdentifierType const key = name;
  ...
  DataObjectPointerMap::const_iterator const it = m_Outputs.find(key);
  ...
  if (!m_Outputs[key])
  {
    ...
    DataObjectPointer const newOutput = this->MakeOutput(key);
...
{
  DataObjectIdentifierType const       baseName = "_";
  DataObjectPointerArraySizeType const baseSize = baseName.size();
  ...
  DataObjectIdentifierType const idxStr = name.substr(baseSize);
...
ProcessObject::IncrementProgress(float increment)
{
  ...
  uint32_t const integerIncrement = progressFloatToFixed(increment);

  uint32_t const oldProgress = m_Progress.fetch_add(integerIncrement);

  uint32_t const updatedProgress = m_Progress;
...
ProcessObject::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  Indent const indent2 = indent.GetNextIndent();
  if (!m_Inputs.empty())
  {
...
ProcessObject::SetMultiThreader(MultiThreaderType * threader)
{
  if (this->m_MultiThreader != threader)
  {
    ...
    {
      ThreadIdType const oldDefaultNumber = m_MultiThreader->GetNumberOfWorkUnits();
      this->m_MultiThreader = threader;
      ThreadIdType const newDefaultNumber = m_MultiThreader->GetNumberOfWorkUnits();
