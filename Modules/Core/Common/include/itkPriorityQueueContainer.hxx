...
{
  ElementIdentifierType const location = m_Interface.GetLocation(element);
  ...
  const ElementWrapperType & element)
{
  ...
  if (location != m_ElementNotFound)
  {
    ...
    ElementIdentifierType const tsize = this->Size();
  ...
  const ElementIdentifierType & identifier)
{
  ...
  ElementIdentifierType const queueSize = this->Size();
