...
{
  VectorType const v01 = iB - iA;
  VectorType const v02 = iC - iA;
  VectorType const v12 = iC - iB;
...
{
  CrossVectorType const cross;
  VectorType            w = cross(iB - iA, iC - iA);
  CoordinateType const  l2 = w.GetSquaredNorm();
...
TriangleHelper<TPoint>::Cotangent(const PointType & iA, const PointType & iB, const PointType & iC) -> CoordinateType
{
  ...
  CoordinateType const v21_l2 = v21.GetSquaredNorm();
  ...
  CoordinateType const v23_l2 = v23.GetSquaredNorm();
  if (Math::NotAlmostEquals(v23_l2, CoordinateType{}))
  {
    v23 /= std::sqrt(v23_l2);
  }

  CoordinateType const bound(0.999999);

  CoordinateType const cos_theta = std::clamp(v21 * v23, -bound, bound);
...
TriangleHelper<TPoint>::ComputeArea(const PointType & iP1, const PointType & iP2, const PointType & iP3)
  -> CoordinateType
{
  CoordinateType const a = iP2.EuclideanDistanceTo(iP3);
  CoordinateType const b = iP1.EuclideanDistanceTo(iP3);
  CoordinateType const c = iP2.EuclideanDistanceTo(iP1);

  CoordinateType const s = 0.5 * (a + b + c);
  ...
  -> CoordinateType
{
  ...
  if (!TriangleType::IsObtuse(iP1, iP2, iP3))
  {
    ...
    CoordinateType const cot_theta_210 = TriangleType::Cotangent(iP3, iP2, iP1);
    CoordinateType const cot_theta_021 = TriangleType::Cotangent(iP1, iP3, iP2);
