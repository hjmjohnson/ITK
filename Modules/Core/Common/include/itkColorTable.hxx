...
ColorTable<TComponent>::UseDiscreteColors()
{
  ...
  typename NumericTraits<TComponent>::RealType const realMax(1.0 * scale + shift);
...
ColorTable<TComponent>::UseGrayColors(unsigned int n)
{
  ...
  {
    typename NumericTraits<TComponent>::RealType const realGray(minimum + i * delta);
...
ColorTable<TComponent>::UseHeatColors(unsigned int n)
{
  ...
  for (i = 0; i < n / 2.0; ++i)
  {
    //
    // avoid overflow
    typename NumericTraits<TComponent>::RealType const realR(((i + 1) / (n / 2.0 + 1)) * scale + shift);
  ...
  for (i = 0; i < n / 2; ++i)
  {
    typename NumericTraits<TComponent>::RealType const rdouble(1.0 * scale + shift);
    TComponent                                         r(NumericTraits<TComponent>::max());
