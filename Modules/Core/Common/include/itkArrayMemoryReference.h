 ...
 *
 *=========================================================================*/
#ifndef itkArrayMemoryReference_h
#define itkArrayMemoryReference_h
...
class ITK_TEMPLATE_EXPORT ArrayMemoryReference : public vnl_vector<TValue>
{
public:
  /** The element type stored at each location in the Array. */
  using ValueType = TValue;
  using Self = ArrayMemoryReference;
...
public:
  ...
  ArrayMemoryReference();

  /** Copy constructor.  Uses VNL copy constructor with correct
   *  setting for memory management.                          */
  ArrayMemoryReference(const ArrayMemoryReference &);

  /** Construct from a VnlVectorType */
  explicit ArrayMemoryReference(const VnlVectorType &);

  /** Constructor with size. Size can only be changed by assignment.
   * \note This constructor may not initialize its elements.
   */
  explicit ArrayMemoryReference(SizeValueType dimension);

  /** Constructor with size and initial value for each element. */
  explicit ArrayMemoryReference(SizeValueType dimension, const ValueType & value);
   ...
   * manages memory. */
  ArrayMemoryReference(const ValueType * datain, SizeValueType sz);


  /** Constructor to initialize an array from another of any data type */
  template <typename TArrayValue>
  ArrayMemoryReference(const ArrayMemoryReference<TArrayValue> & r)
  {
...
#  pragma warning disable 444 // destructor for base class "itk::Array<>" is not virtual
  ...
  ~ArrayMemoryReference() override;


  void
  Swap(ArrayMemoryReference & other)
  {
    using std::swap;
...
template <typename TValue>
std::ostream &
operator<<(std::ostream & os, const ArrayMemoryReference<TValue> & arr)
{
  os << "[";
...
template <>
ITKCommon_EXPORT std::ostream & operator<<<double>(std::ostream & os, const ArrayMemoryReference<double> & arr);
template <>
ITKCommon_EXPORT std::ostream & operator<<<float>(std::ostream & os, const ArrayMemoryReference<float> & arr);


template <typename T>
inline void
swap(ArrayMemoryReference<T> & a, ArrayMemoryReference<T> & b)
{
  a.Swap(b);
}
...
#  include "itkArrayMemoryReference.hxx"
#endif
