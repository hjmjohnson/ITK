...
{
  Detail::FloatIEEE<T> const x1f(x1);
  Detail::FloatIEEE<T> const x2f(x2);
...
{
  Detail::FloatIEEE<T> const representInput(x);
  Detail::FloatIEEE<T> const representOutput(representInput.asInt + ulps);
                                                                                  ...
                                                                                  itk::NumericTraits<T>::epsilon())
{
  ...
  typename Detail::FloatIEEE<T>::IntType const ulps = std::abs(FloatDifferenceULP(x1, x2));
  return ulps <= maxUlps;
}
