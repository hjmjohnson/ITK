...
{
  double const x = position[0] - m_Origin[0];
  double const y = position[1] - m_Origin[1];
  double const z = position[2] - m_Origin[2];

  double const k = std::pow(m_MajorRadius - std::sqrt(x * x + y * y), 2.0) + z * z;
