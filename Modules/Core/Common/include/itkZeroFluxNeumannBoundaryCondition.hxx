  ...
  const RegionType & outputRequestedRegion) const -> RegionType
{
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    ...
    else
    {
      ...
      if (requestIndex[i] < outputIndex[i])
      {
        ...
        OffsetValueType const crop = outputIndex[i] - requestIndex[i];
      ...
      {
        // How much do we need to adjust
        OffsetValueType const crop = requestIndex[i] + static_cast<OffsetValueType>(requestSize[i]) - outputIndex[i] -
  ...
  RegionType const inputRequestedRegion(requestIndex, requestSize);
...
ZeroFluxNeumannBoundaryCondition<TInputImage, TOutputImage>::GetPixel(const IndexType &   index,
  ...
  -> OutputPixelType
{
  RegionType const imageRegion = image->GetLargestPossibleRegion();
  ...
  for (unsigned int i = 0; i < ImageDimension; ++i)
  {
    IndexValueType const lowerIndex = imageIndex[i];
    IndexValueType const upperIndex = imageIndex[i] + static_cast<IndexValueType>(imageSize[i]) - 1;
