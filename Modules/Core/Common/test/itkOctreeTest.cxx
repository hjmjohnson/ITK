...
itkOctreeTest(int, char *[])
{
  ...
  try
  {
    ...
    while (!ri.IsAtEnd())
    {
      unsigned int const val = rand() % 16384;
  ...
  ImageType::Pointer const            output = octree->GetImage();
  ...
  try
  {
    ...
    {
      unsigned int const x = ri.Get();
      unsigned int const y = ri2.Get();
      unsigned int const mapped = id.Evaluate(&x);
