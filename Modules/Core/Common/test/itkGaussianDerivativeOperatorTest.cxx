...
TestGaussianOperator(double variance, double error, unsigned int width, unsigned int order, double spacing)
{
  ...
  double const total = std::accumulate(op.Begin(), op.End(), 0.0);
...
itkGaussianDerivativeOperatorTest(int argc, char * argv[])
{
  ...
  if (argc == 6)
  {
    double const       variance = std::stod(argv[1]);
    double const       error = std::stod(argv[2]);
    unsigned int const width = std::stoi(argv[3]);
    unsigned int const order = std::stoi(argv[4]);
    double const       spacing = std::stod(argv[5]);
