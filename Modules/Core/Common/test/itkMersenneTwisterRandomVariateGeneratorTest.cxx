...
itkMersenneTwisterRandomVariateGeneratorTest(int, char *[])
{
  ...
  Twister::IntegerType const seed = 1234;
  ...
  {
    Twister::IntegerType const actual = twister->GetIntegerVariate();
    if (actual != i)
    {
  ...
  {
    double const v = twister->GetNormalVariate();
    sum += v;
    sum2 += v * v;
  }
  double const mean = sum / static_cast<double>(count);
  double const variance = sum2 / static_cast<double>(count) - mean * mean;
