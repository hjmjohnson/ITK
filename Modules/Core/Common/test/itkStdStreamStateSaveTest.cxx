...
itkStdStreamStateSaveTest(int, char *[])
{
  ...
  std::streamsize const         defaultPrecision = std::cout.precision();
  std::streamsize const         defaultWidth = std::cout.width();
  const char                    defaultFill = std::cout.fill();
  std::ios_base::fmtflags const defaultFlags = std::cout.flags();
  ...
  int const originalInt = 10;
  {
    itk::StdStreamStateSave const sstreamState(stream);

    // Change some representative state variables
    stream.precision(14);
    stream.width(25);
    int const anInt = originalInt;
    stream.fill('%');
