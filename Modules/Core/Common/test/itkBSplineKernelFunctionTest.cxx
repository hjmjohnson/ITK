...
#define TEST_BSPLINE_KERNEL(ORDERNUM)                                               \
  ...
  // Testing derivative spline order = 0
  {
    ...
    double const xx = -0.25;
    double const expectedValue = 0.0;
    double const results = derivFunction->Evaluate(xx);
  ...
  // Testing derivative spline order = 1
  {
    ...
    {
      double const expectedValue = function->Evaluate(xx + 0.5) - function->Evaluate(xx - 0.5);
      double const results = derivFunction->Evaluate(xx);
