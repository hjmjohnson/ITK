...
ColorTableTestSpecialConditionChecker(typename itk::ColorTable<TComponent>::Pointer colors, unsigned int numberOfColors)
{
  ...
  RGBPixelType const pixel = colors->GetColor(numberOfColors);
  if (pixel != zeroPixel)
  {
    std::cerr << "Test failed!" << std::endl;
    std::cerr << "Error in itk::ColorTable::GetColor" << std::endl;
  ...
  bool const tf = colors->SetColor(numberOfColors, 0, 0, 0, "NoMatterTheName");
  if (tf != false)
  {
    std::cerr << "Test failed!" << std::endl;
  ...
  std::string const name = colors->GetColorName(numberOfColors);
  if (!name.empty())
  {
    std::cerr << "Test failed!" << std::endl;
    std::cerr << "Error in itk::ColorTable::GetColorName" << std::endl;
...
int
itkColorTableTest(int argc, char * argv[])
{
  if (argc != 2)
  {
  ...
  testStatus |= ColorTableTestExpectedConditionChecker<unsigned char>(colors, colorId, pixel, name);
