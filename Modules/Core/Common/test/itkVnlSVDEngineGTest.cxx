...
// recomposition) across float/double/complex. Scavenged from the vxl
// core/vnl/algo/tests/test_svd.cxx so the coverage runs in ITK CI now that
// vnl_svd is backed by the native C++ svdc engine (vnl_svd_native.h) rather
// than the deleted LINPACK Fortran.
...
{
  vnl_random rng(9667566);
  nullvector<float>(5e-7, rng);
  nullvector<double>(5e-15, rng);
  nullvector<std::complex<float>>(5e-7, rng);
  nullvector<std::complex<double>>(5e-15, rng);
}
