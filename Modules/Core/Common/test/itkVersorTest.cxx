...
TestCreateRotationMatrixFromAngles(const double alpha, const double beta, const double gamma)
{
  ...
  R(2, 1) = sa * cb;
  R(2, 2) = ca * cb;
  itk::Matrix<double, 3, 3>::InternalMatrixType const test = R.GetVnlMatrix() * R.GetTranspose();
  ...
  for (double const center : centers)
  {
         ...
         alpha += small_degree_steps)
    {
           ...
           beta += small_degree_steps)
      {
        ...
        {
          itk::Matrix<double, 3, 3> const MR = TestCreateRotationMatrixFromAngles(alpha, beta, gamma);
          itk::Versor<double> const       VR = TestCreateRotationVersorFromAngles(alpha, beta, gamma);
          ...
          VFROMMR.Set(MR);
          itk::Matrix<double, 3, 3> const VRMatrix = VR.GetMatrix();
...
itkVersorTest(int, char *[])
{
...

  {
    ...
    VersorType qa;
    qa.SetIdentity();
    MatrixType const ma = qa.GetMatrix();
...

  {
    ...
    xa[1] = 1.5;
    xa[2] = 0.5;
    ValueType const angle = std::atan(1.0) / 3.0; // 15 degrees in radians
    qa.Set(xa, angle);

    xa.Normalize();

    ValueType const cosangle = std::cos(angle / 2.0);
    ValueType const sinangle = std::sin(angle / 2.0);
...

  {
    ...
    ValueType const angle = std::atan(1.0) * 30.0 / 45.0;
    ValueType const sin2a = std::sin(angle / 2.0);
    ...
    VersorType qa;
    qa.Set(xa, angle);
    ValueType const cos2a = std::cos(angle / 2.0);
...

  {
    ...
    xa[1] = 2.5;
    xa[2] = 2.5;
    ValueType const angle = 8.0 * std::atan(1.0) / 3.0; // 120 degrees in radians
...

  {
    ...
    VectorType const      x1 = x1Init;

    ValueType const angle1 = std::atan(1.0) / 3.0; // 15 degrees in radians

    VersorType v1;
    v1.Set(x1, angle1);

    // Get the components and scale them
    ValueType const scale = 5.5;
...

  {
    ...
    ValueType const angle2 = std::atan(1.0) / 1.0; // 45 degrees in radians
    ...
    VersorType const v2r = v2.GetReciprocal();
    ...
    VersorType const v3 = v1 * v2;
    VersorType const v4 = v3 * v2r;
