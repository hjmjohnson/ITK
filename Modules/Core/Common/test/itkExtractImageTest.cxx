...
ExtractImageInPlaceTest()
{
  ...
  ImageType::SizeType const size = { { 32, 32, 32 } };
  source->SetSize(size);

  source->UpdateLargestPossibleRegion();


  ImageType::IndexType const extractIndex = { { 16, 16, 16 } };
  ImageType::SizeType const  extractSize = { { 8, 8, 8 } };
  ImageType::SizeType const  zeroSize = { { 0, 0, 0 } };
...
itkExtractImageTest(int, char *[])
{
  ...
  ShortImage::RegionType const setRegion = extract->GetExtractionRegion();
  ...
  {
    LineImage::PixelType const linePixelValue = iteratorLineIn.Get();
