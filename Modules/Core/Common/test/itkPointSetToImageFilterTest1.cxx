...
itkPointSetToImageFilterTest1(int argc, char * argv[])
{
  ...
  BinaryImageType::SpacingType::ValueType const spacingValue = 1.0;
  ...
  BinaryImageType::PointType const origin{ { { -125, -125 } } };
  ...
  typename BinaryImageType::ValueType const insideValue =
  ...
  typename BinaryImageType::ValueType const outsideValue{};
  filter->SetOutsideValue(outsideValue);
  ITK_TEST_SET_GET_VALUE(outsideValue, filter->GetOutsideValue());

  typename BinaryImageType::SizeType const size = { { 250, 250 } };
  filter->SetSize(size);
  ...
  BinaryImageType::Pointer const binaryImage = filter->GetOutput();
