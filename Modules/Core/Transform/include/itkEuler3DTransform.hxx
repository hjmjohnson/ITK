...
Euler3DTransform<TParametersValueType>::ComputeMatrixParameters()
{
  if (m_ComputeZYX)
  {
    ...
    double const C = std::cos(m_AngleY);
    ...
    {
      m_AngleX = ScalarType{};
      double const x = this->GetMatrix()[1][1];
      double const y = -this->GetMatrix()[0][1];
      m_AngleZ = std::atan2(y, x);
    }
  }
  else
  {
    ...
    double const A = std::cos(m_AngleX);
    ...
    {
      m_AngleZ = ScalarType{};
      double const x = this->GetMatrix()[0][0];
      double const y = this->GetMatrix()[1][0];
      m_AngleY = std::atan2(y, x);
    }
  }
  this->ComputeMatrix();
                                                                               ...
                                                                               JacobianType &         jacobian) const
{
  ...
  unsigned int const blockOffset = 3;
