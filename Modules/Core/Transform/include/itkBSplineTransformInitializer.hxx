...
BSplineTransformInitializer<TTransform, TImage>::InitializeTransform() const
{
  ...
  for (unsigned int d = 0; d < cornerPoints->GetNumberOfPoints(); ++d)
  {
    ...
    RealType const distance = corner.SquaredEuclideanDistanceTo(bbox->GetMinimum());
  ...
  for (unsigned int d = 0; d < SpaceDimension; ++d)
  {
    ...
    for (unsigned int i = 0; i < SpaceDimension; ++i)
    {
      PointIdentifier const oppositeCornerId =
      ...
      double const theta = angle(vectorAxis.GetVnlVector(), vector.GetVnlVector());
