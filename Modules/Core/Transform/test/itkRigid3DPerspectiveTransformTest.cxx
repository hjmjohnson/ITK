...
itkRigid3DPerspectiveTransformTest(int, char *[])
{
  ...
  // Test exceptions
  {
    ...
    typename TransformType::InputVectorType const vector = itk::MakeVector(1.0, 4.0, 9.0);
  ...
  // Exercise basic object methods and test Set/Get macros
  {
    ...
    typename TransformType::OffsetType const fixedOffset{};

    transform->SetFixedOffset(fixedOffset);
    ITK_TEST_SET_GET_VALUE(fixedOffset, transform->GetFixedOffset());

    typename TransformType::InputPointType const centerOfRotation{};
  ...
  /* Create a Rigid 3D transform with a rotation */
  {
    ...
    TransformType::OffsetType const ioffset{};

    rigid->SetOffset(ioffset);

    TransformType::OffsetType const offset = rigid->GetOffset();
    std::cout << "pure Translation test:  ";
    std::cout << offset << std::endl;
    ...
    VersorType::ValueType const angle = 30.0f * std::atan(1.0f) / 45.0f;
    axis[0] = 1.0f;
    axis[1] = 1.0f;
    axis[2] = 1.0f;
