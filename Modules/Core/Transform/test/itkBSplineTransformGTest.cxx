           ...
           double                                                                        tolerance = 1e-15)
{
  ...
  for (unsigned int i = 0; i < VDimension; ++i)
  {
    ...
    typename ImageType::Pointer const coeffImage1 = bspline1->GetCoefficientImages()[i];
    typename ImageType::Pointer const coeffImage2 = bspline2->GetCoefficientImages()[i];
...
TEST(ITKBSplineTransform, Copying_Clone)
{
  ...
  SizeType const imageSize = itk::MakeSize(10, 10);
  DirectionType  imageDirection; // filled with zeros
  imageDirection(0, 1) = -1;
  imageDirection(1, 0) = 1;

  VectorType const imageSpacing = itk::MakeVector(1.1, 1.2);

  PointType const imageOrigin = itk::MakePoint(0.9, 0.8);
