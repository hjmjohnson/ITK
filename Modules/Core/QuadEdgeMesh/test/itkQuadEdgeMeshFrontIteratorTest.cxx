...
itkQuadEdgeMeshFrontIteratorTest(int, char *[])
{
  ...
  int const numPts = mesh->GetNumberOfPoints();
  ...
  int const numCells = mesh->GetNumberOfCells();
  ...
  for (FrontIterator it = mesh->BeginFront(); it != mesh->EndFront(); ++it)
  {
    ...
    PointIdentifier const origin = edge->GetOrigin();
    PointIdentifier const destination = edge->GetDestination();
  ...
  size_t const numberOfPoints = visitedSet.size();
  int const    computedNumberOfPoints = mesh->ComputeNumberOfPoints();
