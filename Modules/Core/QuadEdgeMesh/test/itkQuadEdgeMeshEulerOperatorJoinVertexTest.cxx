...
itkQuadEdgeMeshEulerOperatorJoinVertexTest(int argc, char * argv[])
{
  ...
  std::cout << "     "
            << "Test QE Input and Sym isolated.";
  ...
  {
    joinVertex->SetInput(mesh);
    PointIdentifier const id_org = qe->GetOrigin();
    PointIdentifier const id_dest = qe->GetDestination();

    QEType *                             qe_output = joinVertex->Evaluate(qe);
    JoinVertexType::EdgeStatusType const status = joinVertex->GetEdgeStatus();
    ...
    PointIdentifier const old_id = joinVertex->GetOldPointID();
