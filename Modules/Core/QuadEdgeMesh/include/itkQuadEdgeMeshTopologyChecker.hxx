...
QuadEdgeMeshTopologyChecker<TMesh>::ValidateEulerCharacteristic() const
{
  ...
  PointIdentifier const numPoints = m_Mesh->ComputeNumberOfPoints();
  // Number of USED edges
  CellIdentifier const numEdges = m_Mesh->ComputeNumberOfEdges();
  // Number of USED faces
  CellIdentifier const numFaces = m_Mesh->ComputeNumberOfFaces();
  ...
  OffsetValueType const twiceGenus = OffsetValueType(2) - OffsetValueType(numBounds) - OffsetValueType(numFaces) +
  ...
  CellsContainerConstIterator const cellEnd = m_Mesh->GetCells()->End();
