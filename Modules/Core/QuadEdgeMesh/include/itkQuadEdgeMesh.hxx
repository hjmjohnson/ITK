...
{
  bool const      SplitingOrigin = a->IsInOnextRing(b);
  ...
  if (SplitingOrigin)
  {
    ...
    PointIdentifier const orgId = a->GetOrigin();
    ...
    newOrigin.SetEdge(b);
    PointIdentifier const newOriginId = this->AddPoint(newOrigin);
  ...
  else
  {
    ...
    PointIdentifier const oldOriginId = b->GetOrigin();
    PointIdentifier const orgId = a->GetOrigin();
    ...
    FaceRefType const aLeftFace = a->GetLeft();
    FaceRefType const bLeftFace = b->GetLeft();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::SetCell(CellIdentifier itkNotUsed(cId), CellAutoPointer & cell)
{
  ...
  else if ((pe = dynamic_cast<PolygonCellType *>(cell.GetPointer())) != nullptr)
  {
    ...
    PointIdInternalIterator const pend = pe->InternalPointIdsEnd();
    while (pit != pend)
    {
  ...
  {
    PointIdentifier const numPoint = cell->GetNumberOfPoints();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::FindFirstUnusedPointIndex() -> PointIdentifier
{
  ...
  PointIdentifier const maxpid = this->GetNumberOfPoints();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::SqueezePointsIds()
{
  ...
  PointsContainerPointer const points = this->GetPoints();
  PointsContainerConstIterator last = points->End();
  --last;

  // Check if there is any data
  PointDataContainerPointer const pointData = this->GetPointData();
  bool const                      HasPointData = (pointData->Size() != 0);
...
{
  PointIdentifier const pid = this->FindFirstUnusedPointIndex();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::LightWeightDeleteEdge(QEPrimal * e)
{
  ...
  CellIdentifier const LineIdent = e->GetIdent();
...
{
  CellsContainerPointer const cells = this->GetCells();
  CellType *                  c;
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::GetEdge() const -> QEPrimal *
{
  ...
  CellsContainerConstIterator const cit = edgeCells->Begin();
  ...
  -> QEPrimal *
{
  ...
  if (initialEdge)
  {
    ...
    typename QEPrimal::IteratorGeom const end = initialEdge->EndGeomOnext();
    while (it != end)
    {
                                                        ...
                                                        const PointIdentifier & pid1) const -> EdgeCellType *
{
  ...
  {
    CellIdentifier const LineIdent = EdgeGeom->GetIdent();
    if (LineIdent != m_NoPoint)
    {
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::AddFace(const PointIdList & points) -> QEPrimal *
{
  ...
  for (size_t i = 0; i < N; ++i)
  {
    PointIdentifier const pid0 = points[i];
    PointIdentifier const pid1 = points[(i + 1) % N];
  ...
  -> QEPrimal *
{
  ...
  {
    PointIdentifier const pid0 = points[i];
    PointIdentifier const pid1 = points[(i + 1) % numberOfPoints];
    QEPrimal *            edge = this->FindEdge(pid0, pid1);
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::AddFace(QEPrimal * entry)
{
  ...
  CellIdentifier const fid = this->FindFirstUnusedCellIndex();
  ...
  typename QEPrimal::IteratorGeom const end = entry->EndGeomLnext();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::ClearCellsContainer()
{
  if (m_EdgeCellsContainer->GetReferenceCount() == 1)
  {
    ...
    CellsContainerIterator const EdgeEnd = m_EdgeCellsContainer->End();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::ComputeNumberOfPoints() const -> PointIdentifier
{
  ...
  PointsContainerConstIterator const pointEnd = points->End();
...
QuadEdgeMesh<TPixel, VDimension, TTraits>::ComputeNumberOfFaces() const -> CellIdentifier
{
  ...
  CellsContainerConstIterator const cellEnd = this->GetCells()->End();
