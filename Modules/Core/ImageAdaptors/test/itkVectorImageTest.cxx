                       ...
                       const unsigned int                                                componentToExtract)
{
...

  {
    ...
    CovariantVectorType const input{ 0.0 };
    CovariantVectorType       output1;
    vectorImageAdaptor->TransformLocalVectorToPhysicalVector(input, output1);
    CovariantVectorType const output2 = vectorImageAdaptor->TransformLocalVectorToPhysicalVector(input);
  ...
  }
  {
    ...
    CovariantVectorType const output2 = vectorImageAdaptor->TransformPhysicalVectorToLocalVector(input);
    constexpr double          diff_tolerance = 1e-13;
    if ((input - output1).GetSquaredNorm() > diff_tolerance || (input - output2).GetSquaredNorm() > diff_tolerance)
    {
      std::cerr << "[FAILED]  ";
  ...
  {
    PixelType const pixel = adaptIt.Get();
...
testVectorImageBasicMethods()
{
  ...
  typename VectorImageType::SizeType const size = { { 11, 9, 7 } };
  ...
  typename VectorImageType::IndexType const idx = { { 1, 1, 1 } };

  typename VectorImageType::ConstPointer const cimage(image);
  ...
  */
  ...
  typename VectorImageType::PixelType const temp2 = cimage->GetPixel(idx);
...
itkVectorImageTest(int, char * argv[])
{
...

  {
    ...
    // Using image of VariableLengthVector< PixelType >
    {
      ...
      start.Fill(0);
      size.Fill(50);
      VariableLengthVectorImageType::RegionType const region{ start, size };
      image->SetRegions(region);
      image->Allocate();
      image->FillBuffer(f);

      clock.Stop();
      double const timeTaken = clock.GetMean();
    ...
    // Using image of FixedArray< PixelType, VectorLength >
    {
      ...
      start.Fill(0);
      size.Fill(50);
      FixedArrayImageType::RegionType const region{ start, size };
      image->SetRegions(region);
      image->Allocate();
      image->FillBuffer(f);
    ...
    // Using VectorImage< PixelType, Dimension >
    {
      ...
      start.Fill(0);
      size.Fill(50);
      VectorImageType::RegionType const region{ start, size };
    ...
    // Test with Region and Linear iterators...
    {
      ...
      size.Fill(4);
      size[Dimension - 1] = 2;
      VectorImageType::RegionType const subRegion(start, size);
      ...
      while (!cit.IsAtEnd())
      {
        itk::VariableLengthVector<PixelType> const value = cit.Get();
        ++cit;
        if (ctr == midCtr)
        {
          if (value != f)
  ...
  // Test IO support.
  {
    ...
    auto                              image = VectorImageType::New();
    VectorImageType::IndexType const  start{};
...

  {
    ...
    VectorImageType::Pointer const vectorImage = reader->GetOutput();
...

  {
...

    {
      ...
      cNit -= offset;
      itk::VariableLengthVector<PixelType> const pixel = cNit.GetCenterPixel();
      ...
      if constexpr (Dimension == 3)
      {
        ...
        ShapedNeighborhoodIteratorType::Iterator const shit = sNit.Begin();
        p[0] = p[3] = 10;
        p[1] = p[4] = 20;
        p[2] = p[5] = 30;
        shit.Set(p);
