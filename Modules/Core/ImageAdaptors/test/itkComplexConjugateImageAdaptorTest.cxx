...
itkComplexConjugateImageAdaptorTest(int, char *[])
{
  ...
  ImageType::SizeType const size = { { 10, 10 } };
  image->SetRegions(size);
  image->Allocate();
  ImageType::RegionType const region = image->GetLargestPossibleRegion();
  ...
  for (iter.GoToBegin(); !iter.IsAtEnd(); ++iter)
  {
    ...
    PixelType const pixel(static_cast<float>(rand()) / randMax, static_cast<float>(rand()) / randMax);
    iter.Set(pixel);
  }
  ...
  {
    PixelType const imageValue = iterA.Get();
    PixelType const adaptedValue = iterB.Get();
    ...
    PixelType const newValue = iterB.Get();
