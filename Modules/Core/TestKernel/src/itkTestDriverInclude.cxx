                     ...
                     double             directionTolerance)
{
  ...
  typename ImageType::SizeType const baselineSize = baselineReader->GetOutput()->GetLargestPossibleRegion().GetSize();
  typename ImageType::SizeType const testSize = testReader->GetOutput()->GetLargestPossibleRegion().GetSize();
  ...
  itk::SizeValueType const status = diff->GetNumberOfPixelsWithDifferences();

  if (!reportErrors)
  {
    // The measurement errors should be reported for both success and errors
    // to facilitate setting tight tolerances of tests.
    std::string const shortFilename = itksys::SystemTools::GetFilenameName(baselineImageFilename);
...
{
  itk::ImageIOBase::Pointer const iobase =
  ...
  itk::IOComponentEnum const componentType = iobase->GetComponentType();
...
RegressionTestBaselines(char * baselineFilename)
{
  ...
  int                          x = 0;
  std::string::size_type const suffixPos = originalBaseline.rfind(".");
  std::string                  suffix;
