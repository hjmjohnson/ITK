                                                                                ...
                                                                                int            extent[6])
{
  ...
  for (int z = extent[4]; z <= extent[5]; ++z)
  {
    ...
    for (int k = 0; k < n; ++k)
    {
      Point2DType & p2D1 = xylist[static_cast<size_type>(2 * k)];
...
TriangleMeshToBinaryImageFilter<TInputMesh, TOutputImage>::RasterizeTriangles()
{
  ...
  const int    zInc = extent[3] - extent[2] + 1;
  const int    zSize = extent[5] - extent[4] + 1;
  Point1DArray zymatrix(static_cast<size_type>(zInc * zSize));
  PointVector  coords;
  ...
  for (int z = extent[4]; z <= extent[5]; ++z)
  {
    for (int y = extent[2]; y <= extent[3]; ++y)
    {
      ...
      for (int i = 0; i < n; ++i)
      {
        auto x1 = static_cast<int>(std::ceil(nlist[static_cast<size_type>(2 * i)]));
