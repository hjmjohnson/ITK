...
TriangleMeshCurvatureCalculator<TInputMesh>::ComputeGaussCurvature(const InputMeshType * inputMesh)
{
  ...
  double const pi2 = itk::Math::twopi;
  for (unsigned int k = 0; k < numberOfPoints; ++k)
  {
    K[k] = pi2;
  }

  CellsContainerConstPointer const outCells = inputMesh->GetCells();
  ...
  while (cellsItr != outCells->End())
  {
    ...
    double const alpha0 = itk::Math::pi - angle(e1.GetVnlVector(), e2.GetVnlVector());
    double const alpha1 = itk::Math::pi - angle(e2.GetVnlVector(), e0.GetVnlVector());
    double const alpha2 = itk::Math::pi - angle(e0.GetVnlVector(), e1.GetVnlVector());

    // Surface area
    double const A = static_cast<double>(
