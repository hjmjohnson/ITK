...
{
  IdentifierType const nextNewPointID = m_OutputMesh->GetNumberOfPoints();
...
{
  CoordinateType const p0[] = { x0, x1, x2, x3, x4, x5 };
  PointType            newPoint;
  unsigned int         i;
  unsigned int const   end = (PointDimension < 6 ? PointDimension : 6);
...
AutomaticTopologyMeshSource<TOutputMesh>::AddLine(const IdentifierArrayType & pointIDs) -> IdentifierType
{
  ...
  else
  {
    ...
    {
      IdentifierType const pointID = pointIDs[i];
    ...
    *cellIDPlusOne = cellID + 1;
    ...
    for (i = 0; i < pointIdsEnd; ++i)
    {
      IdentifierType const boundaryID = vertexArray[i];
