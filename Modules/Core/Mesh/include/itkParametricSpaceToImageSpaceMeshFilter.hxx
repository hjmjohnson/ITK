...
ParametricSpaceToImageSpaceMeshFilter<TInputMesh, TOutputMesh>::GenerateData()
{
  ...
  OutputPointsContainerPointer const outPoints = OutputPointsContainer::New();

  outPoints->Reserve(inputMesh->GetNumberOfPoints());

  const InputPointDataContainer *       inData = inputMesh->GetPointData();
  OutputPointDataContainerPointer const outData = OutputPointDataContainer::New();
  ...
  typename InputPointsContainer::ConstIterator const inputPointEnd = inPoints->End();
