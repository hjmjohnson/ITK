...
SimplexMesh<TPixelType, VDimension, TMeshTraits>::~SimplexMesh()
{
  ...
  GeometryMapPointer const  geometryMap = this->GetGeometryData();
  GeometryMapIterator       pointDataIterator = geometryMap->Begin();
  GeometryMapIterator const pointDataEnd = geometryMap->End();
  ...
  -> CellIdentifier
{
  ...
  CellIdentifier const edgeId = m_LastCellId;
