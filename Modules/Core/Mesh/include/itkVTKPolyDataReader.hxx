...
VTKPolyDataReader<TOutputMesh>::VTKPolyDataReader()
{
  //
  // Create the output
  //
...
VTKPolyDataReader<TOutputMesh>::GenerateData()
{
  ...
  for (CellIdentifier i = 0; i < itk::Math::CastWithRangeCheck<CellIdentifier>(numberOfPolygons); ++i)
  {
    ...
    {
      itkExceptionMacro("Error reading file: " << m_FileName
                                               << "Point ids must be < number of points: " << numberOfPoints
...
template <typename TOutputMesh>
void
VTKPolyDataReader<TOutputMesh>::PrintSelf(std::ostream & os, Indent indent) const
{
