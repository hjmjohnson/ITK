...
SphereMeshSource<TOutputMesh>::GenerateData()
{
  ...
  IdentifierType const numpts = m_ResolutionX * m_ResolutionY + 2;

  // calculate the steps using resolution
  double const ustep = itk::Math::pi / (m_ResolutionX + 1);
  double const vstep = 2.0 * itk::Math::pi / m_ResolutionY;
  double const ubeg = (-itk::Math::pi / 2.0) + ustep;
  double const vbeg = -itk::Math::pi;
  ...
  for (unsigned int ii = 0; ii + 1 < m_ResolutionX; ++ii)
  {
    ...
    {
      IdentifierType const jn = (jj + 1) % m_ResolutionY;
