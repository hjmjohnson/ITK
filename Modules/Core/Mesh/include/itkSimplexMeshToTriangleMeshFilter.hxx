...
SimplexMeshToTriangleMeshFilter<TInputMesh, TOutputMesh>::CreateTriangles()
{
  ...
  typename TInputMesh::ConstPointer const           inputMesh = this->GetInput(0);
  typename InputPointsContainer::ConstPointer const points = inputMesh->GetPoints();
  ...
  while (pointsIt != points->End())
  {
    ...
    CellIdentifier const newId1 = FindCellId(n[0], pointsIt.Index(), n[1]);
    CellIdentifier const newId2 = FindCellId(n[1], pointsIt.Index(), n[2]);
    CellIdentifier const newId3 = FindCellId(n[2], pointsIt.Index(), n[0]);
