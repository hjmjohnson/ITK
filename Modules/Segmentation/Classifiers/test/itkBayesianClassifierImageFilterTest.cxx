...
itkBayesianClassifierImageFilterTest(int argc, char * argv[])
{
  ...
  ReaderType::OutputImageType::Pointer const inputImage = reader->GetOutput();

  char *             outputFilename = argv[2];
  unsigned int const numberOfClasses = std::stoi(argv[3]);
  unsigned int const numberOfSmoothingIterations = std::stoi(argv[4]);
  bool const         testPriors = std::stoi(argv[5]);
