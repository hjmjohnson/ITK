...
ScalarImageKmeansImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  for (unsigned int k = 0; k < numberOfClasses; ++k)
  {
    classLabels[k] = label;
    label += labelInterval;
    MembershipFunctionPointer const membershipFunction = MembershipFunctionType::New();
  ...
  typename ClassifierType::MembershipFunctionVectorObjectPointer const membershipFunctionsObject =
  ...
  LabelIterator const end = membershipSample->End();
  ...
  if (m_ImageRegionDefined)
  {
    ...
    if (m_UseNonContiguousLabels)
    {
      OutputPixelType const outsideLabel = labelInterval * numberOfClasses;
      while (!exIt.IsAtEnd())
      {
