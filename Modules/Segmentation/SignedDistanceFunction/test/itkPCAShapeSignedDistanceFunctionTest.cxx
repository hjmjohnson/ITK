...
itkPCAShapeSignedDistanceFunctionTest(int, char *[])
{
  ...
  ImageType::SizeType const imageSize = { { ImageWidth, ImageHeight } };

  ImageType::IndexType const startIndex{};

  ImageType::RegionType const region{ startIndex, imageSize };
  ...
  unsigned int const            numberOfShapeParameters = shape->GetNumberOfShapeParameters();
  unsigned int const            numberOfPoseParameters = shape->GetNumberOfPoseParameters();
  unsigned int const            numberOfParameters = numberOfShapeParameters + numberOfPoseParameters;
  ...
  unsigned int const numberOfRotationParameters = Dimension * (Dimension - 1) / 2;
  unsigned int const startIndexOfTranslationParameters = numberOfShapeParameters + numberOfRotationParameters;
  ...
  for (meanImageIt.GoToBegin(); !meanImageIt.IsAtEnd(); ++meanImageIt)
  {
    ...
    double const angle = parameters[numberOfShapeParameters];
...
#define TEST_INITIALIZATION_ERROR(ComponentName, badComponent, goodComponent) \
  ...
  ImageType::RegionType const badRegion(badSize);
