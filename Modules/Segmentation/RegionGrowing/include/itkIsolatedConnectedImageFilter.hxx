...
IsolatedConnectedImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  if (m_FindUpperThreshold)
  {
    ...
    while (lower + m_IsolatedValueTolerance < guess)
    {
      ...
      typename SeedsContainerType::const_iterator const li = m_Seeds2.end();
      while (si != li)
      {
  ...
  typename SeedsContainerType::const_iterator const li1 = m_Seeds1.end();
  while (si1 != li1)
  {
  ...
  typename SeedsContainerType::const_iterator const li2 = m_Seeds2.end();
  while (si2 != li2)
  {
