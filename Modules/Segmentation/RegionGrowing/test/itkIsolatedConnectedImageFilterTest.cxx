...
itkIsolatedConnectedImageFilterTest(int argc, char * argv[])
{
  ...
  itk::ImageFileReader<myImage>::Pointer const input = itk::ImageFileReader<myImage>::New();
  ...
  FilterType::InputImagePixelType const lower = 0;
...
#endif
  FilterType::InputImagePixelType const upper = 255;
  filter->SetUpper(upper);
  ITK_TEST_SET_GET_VALUE(upper, filter->GetUpper());

  FilterType::OutputImagePixelType const replaceValue = 255;
  filter->SetReplaceValue(replaceValue);
  ITK_TEST_SET_GET_VALUE(replaceValue, filter->GetReplaceValue());

  FilterType::InputImagePixelType const isolatedValueTolerance = 1;
