...
itkConfidenceConnectedImageFilterTest(int argc, char * argv[])
{
  ...
  // Test the GetMacros
  double const doubleMultiplier = filter->GetMultiplier();
  std::cout << "filter->GetMultiplier(): " << doubleMultiplier << std::endl;

  unsigned int const uintNumberOfIterations = filter->GetNumberOfIterations();
  std::cout << "filter->GetNumberOfIterations(): " << uintNumberOfIterations << std::endl;

  PixelType const pixelReplaceValue = filter->GetReplaceValue();
