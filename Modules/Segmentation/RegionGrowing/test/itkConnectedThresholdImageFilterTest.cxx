...
itkConnectedThresholdImageFilterTest(int argc, char * argv[])
{
  ...
  itk::ImageFileReader<ImageType>::Pointer const imageReader = itk::ImageFileReader<ImageType>::New();

  std::string const inputImageFilename = argv[1];
  ...
  itk::SimpleFilterWatcher const watcher(connectedThresholdFilter);
  ...
  ConnectedThresholdImageFilterType::InputImagePixelType const lowerThreshold = std::stoi(argv[5]);
  connectedThresholdFilter->SetLower(lowerThreshold);
  ITK_TEST_SET_GET_VALUE(lowerThreshold, connectedThresholdFilter->GetLower());

  ConnectedThresholdImageFilterType::InputImagePixelType const upperThreshold = std::stoi(argv[6]);
  connectedThresholdFilter->SetUpper(upperThreshold);
  ITK_TEST_SET_GET_VALUE(upperThreshold, connectedThresholdFilter->GetUpper());

  ConnectedThresholdImageFilterType::OutputImagePixelType const replaceValue = 255;
  ...
  if (argc > 7)
  {
    ConnectedThresholdImageFilterType::ConnectivityEnum const conenctivity =
  ...
  itk::ImageFileWriter<ImageType>::Pointer const writer = itk::ImageFileWriter<ImageType>::New();

  std::string const outputImageFilename = argv[2];
