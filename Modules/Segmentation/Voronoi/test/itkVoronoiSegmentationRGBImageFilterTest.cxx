...
SetUpInputImage()
{
  ...
  ImageType::SizeType const size = { { width, height } };
...
TestNoPrior(ImageType::Pointer inputImage)
{
  ...
  double const percentCorrect = CheckResults(filter->GetOutput());
...
TestWithPrior(ImageType::Pointer inputImage)
{
  ...
  BinaryObjectImage::SizeType const size = { { width, height } };
...
itkVoronoiSegmentationRGBImageFilterTest(int, char *[])
{
  ...
  ImageType::Pointer const inputImage = VoronoiSegRGBTest::SetUpInputImage();

  // test without prior
  std::cout << "[Running test without prior]" << std::endl;
  int const noPriorTestResult = VoronoiSegRGBTest::TestNoPrior(inputImage);
  ...
  int const priorTestResult = VoronoiSegRGBTest::TestWithPrior(inputImage);
