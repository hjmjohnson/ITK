  ...
  IndexList *    PixelPool)
{
  ...
  { // normal case some scanlines
    double const offset = static_cast<double>(intbeginy) - beginy;
    endx += offset * rightDx;
...
VoronoiSegmentationImageFilterBase<TInputImage, TOutputImage, TBinaryPriorImage>::MakeSegmentBoundary()
{
  ...
  for (int i = 0; i < m_NumberOfSeeds; ++i)
  {
    if (m_Label[i] == 2)
    {
      NeighborIdIterator const nitend = m_WorkingVD->NeighborIdsEnd(i);
