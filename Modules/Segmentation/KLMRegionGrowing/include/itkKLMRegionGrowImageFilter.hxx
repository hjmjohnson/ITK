  ...
  -> LabelImagePointer
{
  ...
  for (unsigned int iregion = 0; iregion < m_InitialNumberOfRegions; ++iregion)
  {
    ...
    RegionLabelType const newRegionLabel = m_RegionsPointer[iregion]->GetRegionLabel();
...
KLMRegionGrowImageFilter<TInputImage, TOutputImage>::InitializeKLM()
{
  ...
  for (unsigned int idim = 0; idim < InputImageDimension; ++idim)
  {
    ...
    for (unsigned int iborder = 0; iborder < numBorderThisDim; ++iborder)
    {
      ...
      KLMSegmentationBorderPtr const pcurrentBorder = m_BordersPointer[borderCounter];
      ...
      KLMSegmentationRegionPtr const pRegion1 = m_RegionsPointer[intRegion1Index];
      KLMSegmentationRegionPtr const pRegion2 = m_RegionsPointer[intRegion2Index];
...
KLMRegionGrowImageFilter<TInputImage, TOutputImage>::ResolveRegions()
{
  ...
  {
    RegionLabelType const labelValue = m_RegionsPointer[iregion]->GetRegionLabel();

    newLabelValue = remapLabelsVec[labelValue - 1];
    double const                  newAreaValue = m_RegionsPointer[labelValue - 1]->GetRegionArea();
    MeanRegionIntensityType const newMeanValue = m_RegionsPointer[labelValue - 1]->GetMeanRegionIntensity();
