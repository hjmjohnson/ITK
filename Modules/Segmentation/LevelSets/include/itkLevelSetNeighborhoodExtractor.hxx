...
LevelSetNeighborhoodExtractor<TLevelSet>::GenerateDataFull()
{
  ...
  SizeValueType const totalPixels = m_InputLevelSet->GetBufferedRegion().GetNumberOfPixels();
...
LevelSetNeighborhoodExtractor<TLevelSet>::GenerateDataNarrowBand()
{
  ...
  NodeType     node;
  double const maxValue = m_NarrowBandwidth / 2.0;

  SizeValueType const totalPixels = m_InputNarrowBand->Size();
  SizeValueType       updateVisits = totalPixels / 10;
  if (updateVisits < 1)
  {
    updateVisits = 1;
...
LevelSetNeighborhoodExtractor<TLevelSet>::CalculateDistance(IndexType & index)
{
  ...
  bool const inside = (centerValue <= 0.0);
