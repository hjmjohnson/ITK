...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::InitializeActiveLayerValues()
{
  ...
  unsigned int const center = shiftedIt.Size() / 2;
  ...
  for (typename LayerType::ConstIterator activeIt = m_Layers[0]->Begin(); activeIt != m_Layers[0]->End(); ++activeIt)
  {
    ...
    ValueType const distance = shiftedIt.GetCenterPixel() / length;
...
{
  StatusType const past_end = static_cast<StatusType>(m_Layers.size()) - 1;
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::ComputeInitialThreadBoundaries()
{
  ...
  for (unsigned int i = 0; i < m_NumOfWorkUnits - 1; ++i)
  {
    ...
    float const cutOff = 1.0 * (i + 1) * m_ZCumulativeFrequency[m_ZSize - 1] / m_NumOfWorkUnits;
  ...
  const ThreadRegionType & ThreadRegion)
{
  ...
  for (unsigned int i = 0; i < 2 * static_cast<unsigned int>(m_NumberOfLayers) + 1; ++i)
  {
    ...
    typename LayerType::Iterator const layerEnd = m_Layers[i]->End();
    ...
    while (layerIt != layerEnd)
    {
      ...
      unsigned int const k = this->GetThreadNumber(nodePtr->m_Index[m_SplitAxis]);
      if (k != ThreadId)
      {
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::DeallocateData()
{
  ...
  if (!m_Layers.empty())
  {
    for (unsigned int i = 0; i < 2 * static_cast<unsigned int>(m_NumberOfLayers) + 1; ++i)
    {
      ...
      LayerPointerType const layerPtr = m_Layers[i];
  ...
  if (m_Data != nullptr)
  {
    ...
    for (ThreadIdType ThreadId = 0; ThreadId < m_NumOfWorkUnits; ++ThreadId)
    {
      ...
      for (unsigned int i = 0; i < 2 * static_cast<unsigned int>(m_NumberOfLayers) + 1; ++i)
      {
        ...
        LayerPointerType const layerPtr = m_Data[ThreadId].m_Layers[i];
        while (!layerPtr->Empty())
        {
          nodePtr = layerPtr->Front();
          layerPtr->PopFront();
      ...
      for (unsigned int i = 0; i < 2 * static_cast<unsigned int>(m_NumberOfLayers) + 1; ++i)
      {
        for (ThreadIdType tid = 0; tid < m_NumOfWorkUnits; ++tid)
        {
          ...
          LayerNodeType *        nodePtr;
          LayerPointerType const layerPtr = m_Data[ThreadId].m_LoadTransferBufferLayers[i][tid];
      ...
      for (unsigned int i = 0; i < m_NumOfWorkUnits; ++i)
      {
        ...
        {
          LayerPointerType const layerPtr =
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::Iterate()
{
  ...
  typename TOutputImage::RegionType const reqRegion = m_OutputImage->GetRequestedRegion();
  ...
  while (!(this->Halt()))
  {
    ...
    if (this->m_NumOfWorkUnits == 1)
    {
      if (iter != 0)
      {
        ...
        unsigned int const count = this->m_Data[0].m_Count;
        if (count != 0)
        {
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::ThreadedCalculateChange(ThreadIdType ThreadId)
  -> TimeStepType
{
  typename FiniteDifferenceFunctionType::Pointer const df = this->GetDifferenceFunction();
  ...
  typename LayerType::Iterator const layerEnd = m_Data[ThreadId].m_Layers[0]->End();
  ...
  for (; layerIt != layerEnd; ++layerIt)
  {
        ...
        Math::NotExactlyEquals((centerValue = outputIt.GetCenterPixel()), ValueType{}))
    {
      ...
      for (unsigned int i = 0; i < static_cast<unsigned int>(ImageDimension); ++i)
      {
        ValueType const forwardValue = outputIt.GetPixel(center + m_NeighborList.GetStride(i));
        ValueType const backwardValue = outputIt.GetPixel(center - m_NeighborList.GetStride(i));
  ...
  TimeStepType const timeStep = df->ComputeGlobalTimeStep((void *)m_Data[ThreadId].globalData);
                                                                                       ...
                                                                                       ThreadIdType         ThreadId)
{
  ...
  unsigned int const N = (2 * static_cast<unsigned int>(m_NumberOfLayers) + 1) - 2;
  ...
  ThreadIdType         ThreadId)
{
  ...
  unsigned int const Neighbor_Size = m_NeighborList.GetSize();
                                                                                  ...
                                                                                  LayerPointerType ToListPtr)
{
  ...
  typename LayerType::Iterator const layerEnd = FromListPtr->End();
  ...
  ThreadIdType       ThreadId)
{
  ...
  typename LayerType::Iterator const layerEnd = InputList->End();
  while (layerIt != layerEnd)
  {
    auto nodePtr = layerIt.GetPointer();
    ++layerIt;
    ...
    for (unsigned int i = 0; i < neighbor_Size; ++i)
    {
      ...
      if (neighbor_status == SearchForStatus)
      {
        ...
        unsigned int const tmpId = this->GetThreadNumber(n_index[m_SplitAxis]);
  ...
  ThreadIdType       ThreadId)
{
  ...
  while (!InputList->Empty())
  {
    ...
    IndexType const center_index = nodePtr->m_Index;
    ...
    for (unsigned int i = 0; i < neighbor_size; ++i)
    {
      ...
      StatusType const neighbor_status = m_StatusImage->GetPixel(n_index);
  ...
  ThreadIdType       ThreadId)
{
  ...
  ValueType const delta = (InOrOut == 1) ? -m_ConstantGradientValue : m_ConstantGradientValue;

  unsigned int const                 Neighbor_Size = m_NeighborList.GetSize();
  typename LayerType::Iterator       toIt = m_Data[ThreadId].m_Layers[to]->Begin();
  typename LayerType::Iterator const toEnd = m_Data[ThreadId].m_Layers[to]->End();

  while (toIt != toEnd)
  {
    IndexType const centerIndex = toIt->m_Index;

    StatusType const centerStatus = m_StatusImage->GetPixel(centerIndex);
    ...
    for (unsigned int i = 0; i < Neighbor_Size; ++i)
    {
      IndexType const  nIndex = centerIndex + m_NeighborList.GetNeighborhoodOffset(i);
      StatusType const nStatus = m_StatusImage->GetPixel(nIndex);
      // If this neighbor is in the "from" list, compare its absolute value
      ...
      if (nStatus == from)
      {
        ValueType const value_temp = m_OutputImage->GetPixel(nIndex);
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::CheckLoadBalance()
{
  ...
  for (unsigned int i = 0; i < m_NumOfWorkUnits; ++i)
  {
    NodeCounterType const count = m_Data[i].m_Layers[0]->Size();
    total += count;
    if (min > count)
    {
      min = count;
  ...
  {
    // compute m_Boundary[i]
    float const cutOff = 1.0f * (i + 1) * m_ZCumulativeFrequency[m_ZSize - 1] / m_NumOfWorkUnits;
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::ThreadedLoadBalance1(ThreadIdType ThreadId)
{
  ...
  for (i = 0; i < 2 * static_cast<unsigned int>(m_NumberOfLayers) + 1; ++i)
  {
    ...
    typename LayerType::Iterator const layerEnd = m_Data[ThreadId].m_Layers[i]->End();
    ...
    while (layerIt != layerEnd)
    {
      ...
      ThreadIdType const tmpId = this->GetThreadNumber(nodePtr->m_Index[m_SplitAxis]);
...
ParallelSparseFieldLevelSetImageFilter<TInputImage, TOutputImage>::SignalNeighborsAndWait(ThreadIdType ThreadId)
{
  ...
  ThreadIdType const lastThreadId = m_NumOfWorkUnits - 1;
  if (lastThreadId == 0)
  {
