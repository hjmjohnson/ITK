  ...
  -> MeasureType
{
  ...
  typename NodeContainerType::ConstIterator const end = this->GetActiveRegion()->End();
  ...
  while (iter != end)
  {
    ...
    {
      double const value = this->m_ShapeFunction->Evaluate(point);
      if (value > 0.0)
      {
        counter += 1.0;
      }
  ...
  MeasureType const output = counter * m_Weights[0];
  return output;
}
