...
itkReinitializeLevelSetImageFilterTest(int, char *[])
{
  ...
  */
  ...
  double const    maxAbsDifference = calculator->GetMaximum();
  IndexType const maxAbsDifferenceIndex = calculator->GetIndexOfMaximum();
  ...
  calculator->Compute();
  double const minValue = calculator->GetMinimum();
  double const maxValue = calculator->GetMaximum();
  ...
  NodeContainerPointer const nodes = reinitializer->GetOutputNarrowBand();
  ...
  NodeContainerPointer const nodes2 = reinitializer->GetOutputNarrowBand();
  ContainerIterator          nodeIter = nodes2->Begin();
  ContainerIterator const    nodeEnd = nodes2->End();

  while (nodeIter != nodeEnd)
  {
    ImageType::IndexType const nodeIndex = nodeIter.Value().GetIndex();
    double const               product = image->GetPixel(nodeIndex) * reinitializer->GetOutput()->GetPixel(nodeIndex);
    if (product < 0.0)
    {
