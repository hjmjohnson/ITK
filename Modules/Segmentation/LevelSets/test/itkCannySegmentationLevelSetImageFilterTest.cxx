...
{
  float const dis = (x - static_cast<float>(V_WIDTH) / 2.1) * (x - static_cast<float>(V_WIDTH) / 2.1) /
                      ((0.2f * V_WIDTH) * (0.2f * V_WIDTH)) +
                    (y - static_cast<float>(V_HEIGHT) / 2.0) * (y - static_cast<float>(V_HEIGHT) / 2.0) /
                      ((0.2f * V_HEIGHT) * (0.2f * V_HEIGHT)) +
                    (z - static_cast<float>(V_DEPTH) / 2.0) * (z - static_cast<float>(V_DEPTH) / 2.0) /
...
evaluate_float_function(itk::Image<float, 3> * im, float (*f)(float, float, float))
{
  ...
  for (int z = 0; z < V_DEPTH; ++z)
  {
    ...
    for (int y = 0; y < V_HEIGHT; ++y)
    {
      ...
      {
        idx[0] = x;
        float const val = f(static_cast<float>(x), static_cast<float>(y), static_cast<float>(z));
...
itkCannySegmentationLevelSetImageFilterTest(int, char *[])
{
  ...
  CSIFTN::ImageType::Pointer const     inputImage = CSIFTN::ImageType::New();
  CSIFTN::SeedImageType::Pointer const seedImage = CSIFTN::SeedImageType::New();
  ...
  itk::CannySegmentationLevelSetImageFilter<::CSIFTN::SeedImageType, ::CSIFTN::ImageType>::Pointer const filter =
  ...
  itk::CannySegmentationLevelSetImageFilter<::CSIFTN::SeedImageType, ::CSIFTN::ImageType, double>::Pointer const
