...
itkShapePriorMAPCostFunctionTest(int, char *[])
{
  ...
  while (!iter.IsAtEnd())
  {
    ...
    if (itk::Math::Absolute(value) < activeRegionThreshold)
    {
      NodeType node;
      node.SetIndex(index);
      node.SetValue(value);
  ...
  {
    if (itk::Math::Absolute(parameters[j] - optimizer->GetCurrentPosition()[j]) > 0.5)
    {
...
#define TEST_INITIALIZATION_ERROR(ComponentName, badComponent, goodComponent) \
  ...
  CostFunctionType::ParametersType badParameters(shape->GetNumberOfShapeParameters() - 1, 2.0);
