...
itkMultiLabelSTAPLEImageFilterTest(int, char *[])
{
  ...
  // Create two images
  ImageTypePointer const inputImageA = ImageType::New();
  ImageTypePointer const inputImageB = ImageType::New();
  ImageTypePointer const inputImageC = ImageType::New();
  ...
  FilterTypePointer const filter = FilterType::New();

  ITK_EXERCISE_BASIC_OBJECT_METHODS(filter, MultiLabelSTAPLEImageFilter, ImageToImageFilter);

  // Get the Smart Pointer to the Filter Output
  ImageTypePointer const outputImage = filter->GetOutput();
  ...
  unsigned int const maximumNumberOfIterations = 100;
  ...
  typename FilterType::WeightsType const terminationUpdateThreshold = 1e-5;
  ...
  typename FilterType::OutputPixelType const labelForUndecidedPixels = 255;
  ...
  typename FilterType::PriorProbabilitiesType::ValueType const priorProbabilitiesVal(0.0);
