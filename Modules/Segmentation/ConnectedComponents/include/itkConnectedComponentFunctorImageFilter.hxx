...
ConnectedComponentFunctorImageFilter<TInputImage, TOutputImage, TFunctor, TMaskImage>::GenerateData()
{
  ...
  if (typename TMaskImage::ConstPointer const mask = this->GetMaskImage())
  {
  ...
  while (!oit.IsAtEnd())
  {
    ...
    InputPixelType const  value = it.Get();
    OutputPixelType const originalLabel = label;
    ...
    if (label != OutputPixelType{})
    {
      ...
      for (isIt = init.Begin(), osIt = onit.Begin(); !osIt.IsAtEnd(); ++isIt, ++osIt)
      {
        ...
        OutputPixelType const neighborLabel = osIt.Get();
  ...
  while (!oit.IsAtEnd())
  {
    OutputPixelType const label = oit.Get();
