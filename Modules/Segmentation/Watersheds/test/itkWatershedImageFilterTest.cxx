...
itkWatershedImageFilterTest(int, char *[])
{
  ...
  itk::EquivalencyTable::Pointer const table = itk::EquivalencyTable::New();

  itk::watershed::EquivalenceRelabeler<LongImageType2D::PixelType, Dimension>::Pointer const eq =
  ...
  itk::WatershedMiniPipelineProgressCommand::Pointer const wmppc = itk::WatershedMiniPipelineProgressCommand::New();

  ITK_EXERCISE_BASIC_OBJECT_METHODS(wmppc, WatershedMiniPipelineProgressCommand, Command);

  double const count = 2.0;
  wmppc->SetCount(count);
  ITK_TEST_SET_GET_VALUE(count, wmppc->GetCount());

  unsigned int const numberOfFilters = 2;
  ...
  itk::watershed::BoundaryResolver<PixelType, Dimension>::Pointer const br =
  ...
  itk::watershed::Boundary<PixelType, 1>::Pointer const boundaryA = itk::watershed::Boundary<PixelType, 1>::New();
  if (boundaryA.IsNull())
  {
    std::cerr << "Test failed!" << std::endl;
  ...
  itk::watershed::Boundary<PixelType, 1>::Pointer const boundaryB = itk::watershed::Boundary<PixelType, 1>::New();
  if (boundaryB.IsNull())
  {
    std::cerr << "Test failed!" << std::endl;
    std::cout << "Null itk::watershed::Boundary." << std::endl;
  ...
  itk::WatershedImageFilter<ImageType2D>::Pointer const watershedFilter = itk::WatershedImageFilter<ImageType2D>::New();

  ITK_EXERCISE_BASIC_OBJECT_METHODS(watershedFilter, WatershedImageFilter, ImageToImageFilter);

  itk::SimpleFilterWatcher const watchIt(watershedFilter, "WatershedImageFilter");

  double const threshold = .05;
  watershedFilter->SetThreshold(threshold);
  ITK_TEST_SET_GET_VALUE(threshold, watershedFilter->GetThreshold());

  double const level = 1.0;
