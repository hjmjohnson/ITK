...
IsolatedWatershedImageFilter<TInputImage, TOutputImage>::GenerateInputRequestedRegion()
{
  ...
  if (this->GetInput())
  {
    InputImagePointer const image = const_cast<TInputImage *>(this->GetInput());
...
IsolatedWatershedImageFilter<TInputImage, TOutputImage>::GenerateData()
{
  ...
  {
    ProgressReporter const progress(this, 0, region.GetNumberOfPixels(), 100, cumulatedProgress, progressWeight);
  ...
  IdentifierType const seed1Label = m_Watershed->GetOutput()->GetPixel(m_Seed1);
  IdentifierType const seed2Label = m_Watershed->GetOutput()->GetPixel(m_Seed2);
