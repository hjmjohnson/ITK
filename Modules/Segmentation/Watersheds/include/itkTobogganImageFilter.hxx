...
TobogganImageFilter<TInputImage, TOutputImage>::GenerateInputRequestedRegion()
{
  ...
  if (this->GetInput())
  {
    InputImagePointer const image = const_cast<typename Superclass::InputImageType *>(this->GetInput());
    ...
    if (outIt.Get() == z)
    {
        ...
        for (unsigned int Dimension = 0; Dimension < ImageDimension; ++Dimension)
        {
          for (int t = 1; t >= -1; t = t - 2)
          {
            ...
            if (outputImage->GetRequestedRegion().IsInside(NeighborIndex))
            {
              ...
              // it's minimum
              OutputImagePixelType const NeighborClass = outputImage->GetPixel(NeighborIndex);
              // See if we've already touched it
              if (NeighborClass != 1)
              {
                InputImagePixelType const NeighborValue = inputImage->GetPixel(NeighborIndex);
      ...
      if (MinimumNeighborClass == 1)
      {
        ...
        while (!OpenList.empty())
        {
          ...
          IndexType const SeedIndex = OpenList.back();
          OpenList.pop_back();
          Visited.push_back(SeedIndex);
          itkDebugMacro("Flood fill, looking at " << SeedIndex);
          // Look at the neighbors
          InputImagePixelType const SeedValue = inputImage->GetPixel(SeedIndex);
          for (unsigned int Dimension = 0; Dimension < ImageDimension; ++Dimension)
          {
            for (int t = -1; t <= 1; t = t + 2)
            {
