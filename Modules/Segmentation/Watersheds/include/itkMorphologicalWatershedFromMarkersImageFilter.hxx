...
MorphologicalWatershedFromMarkersImageFilter<TInputImage, TLabelImage>::GenerateData()
{
  ...
  if (m_MarkWatershedLine)
  {
    ...
    for (markerIt.GoToBegin(), statusIt.GoToBegin(), outputIt.GoToBegin(), inputIt.GoToBegin(); !markerIt.IsAtEnd();
         ++markerIt, ++outputIt)
    {
      LabelImagePixelType const markerPixel = markerIt.GetCenterPixel();
      if (markerPixel != bgLabel)
      {
        IndexType const idx = markerIt.GetIndex();

        // move the iterators to the right place
        OffsetType const shift = idx - statusIt.GetIndex();
        statusIt += shift;
        inputIt += shift;
    ...
    while (!fah.empty())
    {
      ...
      InputImagePixelType const currentValue = fah.begin()->first;
      ...
      {
        IndexType const idx = currentQueue.front();
        currentQueue.pop();

        // move the iterators to the right place
        OffsetType const shift = idx - outputIt.GetIndex();
        outputIt += shift;
        statusIt += shift;
        inputIt += shift;
        ...
        {
          LabelImagePixelType const o = noIt.Get();
          if (o != wsLabel)
          {
        ...
        if (!collision)
        {
          ...
          for (niIt = inputIt.Begin(), nsIt = statusIt.Begin(); niIt != inputIt.End(); niIt++, nsIt++)
          {
            if (!nsIt.Get())
            {
              ...
              InputImagePixelType const GrayVal = niIt.Get();
  ...
  else
  {
    ...
    for (markerIt.GoToBegin(), outputIt.GoToBegin(), inputIt.GoToBegin(); !markerIt.IsAtEnd(); ++markerIt, ++outputIt)
    {
      ...
      {
        IndexType const  idx = markerIt.GetIndex();
        OffsetType const shift = idx - inputIt.GetIndex();
        inputIt += shift;
    ...
    while (!fah.empty())
    {
      ...
      while (!currentQueue.empty())
      {
        ...
        LabelImagePixelType const currentMarker = outputIt.GetCenterPixel();
