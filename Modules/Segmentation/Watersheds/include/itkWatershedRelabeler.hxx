...
{
  typename ImageType::Pointer const img = static_cast<ImageType *>(this->MakeOutput(0).GetPointer());
...
Relabeler<TScalar, TImageDimension>::GenerateData()
{
  ...
  typename ImageType::Pointer const input = this->GetInputImage();
  typename ImageType::Pointer const output = this->GetOutputImage();

  typename SegmentTreeType::Pointer const tree = this->GetInputSegmentTree();
  ...
  ScalarType const max = tree->Back().saliency;
...
Relabeler<TScalar, VImageDimension>::GenerateInputRequestedRegion()
{
  ...
  typename ImageType::Pointer const inputPtr = this->GetInputImage();
  typename ImageType::Pointer const outputPtr = this->GetOutputImage();
...
Relabeler<TScalar, TImageDimension>::GraftNthOutput(unsigned int idx, ImageType * graft)
{
  ...
  {
    OutputImagePointer const output = this->GetOutputImage();
