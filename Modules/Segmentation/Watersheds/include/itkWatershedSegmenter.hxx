...
Segmenter<TInputImage>::GenerateData()
{
  ...
  typename InputImageType::Pointer const  input = this->GetInputImage();
  typename OutputImageType::Pointer const output = this->GetOutputImage();
  typename BoundaryType::Pointer const    boundary = this->GetBoundary();
  ...
  ImageRegionType const largestPossibleRegion = this->GetLargestPossibleRegion();
...
Segmenter<TInputImage>::CollectBoundaryInformation(flat_region_table_t & flatRegions)
{
  ...
  for (idx.first = 0; idx.first < ImageDimension; (idx.first)++)
  {
    for (idx.second = 0; idx.second < 2; (idx.second)++)
    {
      ...
      typename BoundaryType::face_t::Pointer const face = boundary->GetFace(idx);
      typename BoundaryType::flat_hash_t *         flats = boundary->GetFlatHash(idx);
      ImageRegionType const                        region = face->GetRequestedRegion();
      ...
      while (!faceIt.IsAtEnd())
      {
        ...
        typename flat_region_table_t::iterator const flrt_it = flatRegions.find(labelIt.Get());
        if (faceIt.Get().flow != NULL_FLOW && flrt_it != flatRegions.end())
        {
          // Have we already entered this
          // flat region into the boundary?
          typename BoundaryType::flat_hash_t::iterator const flats_it = flats->find(labelIt.Get());
...
Segmenter<TInputImage>::InitializeBoundary()
{
  ...
  for (idx.first = 0; idx.first < ImageDimension; ++(idx.first))
  {
    for (idx.second = 0; idx.second < 2; ++(idx.second))
    {
      ...
      typename BoundaryType::face_t::Pointer const face = this->GetBoundary()->GetFace(idx);
      if (face != nullptr)
      {
        face->FillBuffer(fps);
      }
                                            ...
                                            InputPixelType        max)
{
  ...
  for (idx.first = 0; idx.first < ImageDimension; ++(idx.first))
  {
    for (idx.second = 0; idx.second < 2; ++(idx.second))
    {
      ...
      unsigned int const nCenter = searchIt.Size() / 2;
      searchIt.GoToBegin();
      labelIt.GoToBegin();
      ...
      while (!searchIt.IsAtEnd())
      {
        ...
        if (Math::AlmostEquals(searchIt.GetPixel(nCenter), searchIt.GetPixel(cPos)))
        {
          ...
          {
            unsigned int const nPos = m_Connectivity.index[i];
...
Segmenter<TInputImage>::GenerateConnectivity()
{
  ...
  ConstNeighborhoodIterator<InputImageType> const it(
    rad, this->GetInputImage(), this->GetInputImage()->GetRequestedRegion());
  unsigned int const nSize = it.Size();
  unsigned int const nCenter = nSize >> 1;
  ...
  {
    unsigned int const stride = it.GetStride(d);
...
{
  unsigned int         nPos = 0;
  InputPixelType const maxValue = Max;
  ...
  unsigned int const                        nSize = searchIt.Size();
  unsigned int const                        nCenter = nSize >> 1;
  ...
  for (searchIt.GoToBegin(), labelIt.GoToBegin(); !searchIt.IsAtEnd(); ++searchIt, ++labelIt)
  {
    ...
    InputPixelType const currentValue = searchIt.GetPixel(nCenter);
    unsigned int         i = 0;
  ...
  for (searchIt.GoToBegin(), labelIt.GoToBegin(); !searchIt.IsAtEnd(); ++searchIt, ++labelIt)
  {
    typename flat_region_table_t::iterator const flatPtr = flatRegions.find(labelIt.GetPixel(nCenter));
  ...
  for (ImageRegionIterator<OutputImageType> it(output, region); !it.IsAtEnd(); ++it)
  {
    if (it.Get() == NULL_LABEL)
    {
      ...
      {                                     // is found.
        ...
        {
          unsigned int const nPos = m_Connectivity.index[ii];
...
Segmenter<TInputImage>::UpdateSegmentTable(InputImageTypePointer input, ImageRegionType region)
{
  ...
  typename OutputImageType::Pointer const  output = this->GetOutputImage();
  typename SegmentTableType::Pointer const segments = this->GetSegmentTable();
  ...
  IdentifierType const hoodCenter = searchIt.Size() >> 1;

  edge_table_hash_t edgeHash;
  for (searchIt.GoToBegin(), labelIt.GoToBegin(); !searchIt.IsAtEnd(); ++searchIt, ++labelIt)
  {
    IdentifierType const segment_label = labelIt.GetPixel(hoodCenter);
    ...
    edge_table_t const                     tempEdgeTable;
    ...
    for (unsigned int i = 0; i < m_Connectivity.size; ++i)
    {
      ...
      if (labelIt.GetPixel(nPos) != segment_label && labelIt.GetPixel(nPos) != NULL_LABEL)
      {
        ...
        typename edge_table_t::iterator const edge_ptr = edge_table_entry_ptr->second.find(labelIt.GetPixel(nPos));
       ...
       ++edge_table_entry_ptr)
  {
    ...
    IdentifierType const listsz = static_cast<IdentifierType>(edge_table_entry_ptr->second.size());
...
Segmenter<TInputImage>::MergeFlatRegions(flat_region_table_t & regions, EquivalencyTable::Pointer eqTable)
{
  ...
  {
    typename flat_region_table_t::iterator const a = regions.find(it->first);
    typename flat_region_table_t::iterator const b = regions.find(it->second);
                                     ...
                                     EquivalencyTable::Pointer eqTable)
{
  ...
  while (!it.IsAtEnd())
  {
    IdentifierType const temp = eqTable->Lookup(it.Get());
    if (temp != it.Get())
    {
      it.Set(temp);
    }
                                  ...
                                  InputPixelType        threshold)
{
  ...
  if (NumericTraits<InputPixelType>::is_integer)
  {
    ...
    while (!dIt.IsAtEnd())
    {
      InputPixelType const tmp = sIt.Get();
      ITK_GCC_PRAGMA_PUSH
      ITK_GCC_SUPPRESS_Wfloat_equal
      if (tmp < threshold)
      {
...
Segmenter<TInputImage>::UpdateOutputInformation()
{
  ...
  typename InputImageType::Pointer const  inputPtr = this->GetInputImage();
  typename OutputImageType::Pointer const outputPtr = this->GetOutputImage();
