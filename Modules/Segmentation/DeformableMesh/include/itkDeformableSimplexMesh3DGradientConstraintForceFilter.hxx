...
DeformableSimplexMesh3DGradientConstraintForceFilter<TInputMesh, TOutputMesh>::NextVoxel(const double * pp,
                                                                                         ...
                                                                                         double *       z)
{
  double const dp[3]{ pp[0], pp[1], pp[2] };
  ...
  const GradientImageType * gradientImage)
{
  ...
  for (std::vector<ImageVoxel *>::iterator it = m_Positive.begin(); it != m_Positive.end(); ++it)
  {
    ...
    double const mag = std::sqrt(dot_product(vec_for.GetVnlVector(), vec_for.GetVnlVector()));
    if (mag > max)
    {
      max = mag;
      coord2 = coord3;
  ...
  double const mag = dot_product(vec_for.GetVnlVector(), data->normal.GetVnlVector());
  if (mag > 0)
  {
