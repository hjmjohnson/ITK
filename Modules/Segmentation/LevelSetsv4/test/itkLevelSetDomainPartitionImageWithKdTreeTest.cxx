...
itkLevelSetDomainPartitionImageWithKdTreeTest(int, char *[])
{
  ...
  IdentifierType const numberOfLevelSetFunctions = 10;
  ...
  for (unsigned int i = 0; i < numberOfLevelSetFunctions; ++i)
  {
    ...
    InputImageType::RegionType const region1{ index, size };
  ...
  TreeType::Pointer const kdtree = treeGenerator->GetOutput();
  ...
  typename DomainPartitionSourceType::NeighborsIdType const numberOfNeighbors = 3;
  ...
  ListType                          ll;
  ListImageType::ConstPointer const listImage = partitionSource->GetListDomain();
  ...
  while (!It.IsAtEnd())
  {
    ...
    while (it != ll.end())
    {
      IdentifierType const id = index[0] / 10;
      if (*it != id)
      {
        flag = false;
        break;
