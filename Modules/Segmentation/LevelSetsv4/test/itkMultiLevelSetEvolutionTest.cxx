...
itkMultiLevelSetEvolutionTest(int, char *[])
{
  ...
  while (!it1.IsAtEnd())
  {
    ...
    it1.Set(std::sqrt(static_cast<float>((idx[0] - 2) * (idx[0] - 2) + (idx[1] - 2) * (idx[1] - 2))) - 1.5);

    it2.Set(std::sqrt(static_cast<float>((idx[0] - 5) * (idx[0] - 5) + (idx[1] - 5) * (idx[1] - 5))) - 2.5);
    ++it1;
    ++it2;
  }
