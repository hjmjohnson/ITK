  ...
  const LevelSetInputIndexType & inputIndex)
{
  ...
  {
    InputPixelType const pixel = this->m_Input->GetPixel(inputIndex);
...
LevelSetEquationChanAndVeseInternalTerm<TInput, TLevelSetContainer>::ComputeProduct(
  ...
  LevelSetOutputRealType &       prod)
{
  LevelSetOutputRealType const value = this->m_CurrentLevelSetPointer->Evaluate(inputIndex);
  ...
  const LevelSetOutputRealType & newValue)
{
  ...
  InputPixelType const input = this->m_Input->GetPixel(inputIndex);
