...
UpdateMalcolmSparseLevelSet<VDimension, TEquationContainer>::Update()
{
  ...
  LevelSetLabelMapPointer const outputLabelMap = this->m_OutputLevelSet->GetModifiableLabelMap();
...
UpdateMalcolmSparseLevelSet<VDimension, TEquationContainer>::FillUpdateContainer()
{
  ...
  TermContainerPointer const termContainer = this->m_EquationContainer->GetEquation(this->m_CurrentLevelSetId);
...
UpdateMalcolmSparseLevelSet<VDimension, TEquationContainer>::EvolveWithUnPhasedPropagation()
{
  ...
  while (nodeIt != nodeEnd)
  {
    ...
    if (update != LevelSetOutputType{})
    {
      ...
      {
        LevelSetOutputType const tempValue = i.Get();
        if (tempValue * newValue == -1)
        {
          LevelSetInputType const tempIndex = neighIt.GetIndex(i.GetNeighborhoodOffset());
                                                                                         ...
                                                                                         const bool iContraction)
{
  ...
  while (nodeIt != nodeEnd)
  {
    ...
    LevelSetOutputType const oldValue = LevelSetType::ZeroLayer();
    LevelSetOutputType       newValue;

    LevelSetOutputType const update = upIt->second;
    LevelSetInputType const  currentIdx = nodeIt->first;
    LevelSetInputType const  inputIndex = currentIdx + this->m_Offset;
    ...
    if (Math::NotAlmostEquals(update, LevelSetOutputRealType{}))
    {
      ...
      for (typename NeighborhoodIteratorType::Iterator i = neighIt.Begin(); !i.IsAtEnd(); ++i)
      {
        ...
        if (tempValue * newValue == -1)
        {
          LevelSetInputType const tempIdx = neighIt.GetIndex(i.GetNeighborhoodOffset());
...
UpdateMalcolmSparseLevelSet<VDimension, TEquationContainer>::CompactLayersToSinglePixelThickness()
{
  ...
  while (nodeIt != nodeEnd)
  {
    ...
    LevelSetOutputRealType const oldValue = LevelSetType::ZeroLayer();
    for (typename NeighborhoodIteratorType::Iterator i = neighIt.Begin(); !i.IsAtEnd(); ++i)
    {
      LevelSetOutputType const tempValue = i.Get();
