...
LevelSetEquationTermContainer<TInputImage, TLevelSetContainer>::AddTerm(const TermIdType & iId, TermType * iTerm)
{
  if (iTerm)
  {
    ...
    typename RequiredDataType::const_iterator const dEnd = termRequiredData.end();
  ...
  -> LevelSetOutputRealType
{
  ...
  while (term_it != term_end)
  {
    LevelSetOutputRealType const temp_val = (term_it->second)->Evaluate(iP);

    LevelSetOutputRealType const abs_temp_value = itk::Math::abs(temp_val);
  ...
  -> LevelSetOutputRealType
{
  ...
  while (term_it != term_end)
  {
    LevelSetOutputRealType const temp_val = (term_it->second)->Evaluate(iP, iData);
                                                                                    ...
                                                                                    LevelSetDataType & ioData)
{
  ...
  typename RequiredDataType::const_iterator const dEnd = m_RequiredData.end();

  auto tIt = m_Container.begin();

  LevelSetPointer const levelset = (tIt->second)->GetModifiableCurrentLevelSetPointer();
