...
{
  InputType const mapIndex = inputIndex - this->m_DomainOffset;
...
LevelSetSparseImage<TOutput, VDimension>::GetAsLabelObject()
{
  ...
  while (lIt != lEnd)
  {
    LayerIdType const        id = *lIt;
    LabelObjectPointer const labelObject = this->m_LabelMap->GetLabelObject(id);
    SizeValueType const      numberOfLines = labelObject->GetNumberOfLines();
