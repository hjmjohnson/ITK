...
BinaryImageToLevelSetImageAdaptor<TInput, WhitakerSparseLevelSetImage<TOutput, TInput::ImageDimension>>::Initialize()
{
  ...
  LevelSetLabelObjectPointer const innerPart = LevelSetLabelObjectType::New();
  ...
  PropagateToOuterLayers(LayerIdType layerToBeScanned, LayerIdType outputLayer, LayerIdType testValue)
{
  ...
  {
    LevelSetInputType const idx = nodeIt->first;
    ...
    for (typename NeighborhoodIteratorType::Iterator it = neighIt.Begin(); !it.IsAtEnd(); ++it)
    {
      ...
      if (it.Get() == testValue)
      {
        LevelSetInputType const tempIndex = neighIt.GetIndex(it.GetNeighborhoodOffset());
  ...
  LevelSetLabelObjectPointer const ObjectPlus2 = LevelSetLabelObjectType::New();
...
{
  LevelSetLabelObjectPointer const labelObject = this->m_LabelMap->GetLabelObject(LevelSetType::MinusThreeLayer());
  ...
  if (!layer0.empty())
  {
    LevelSetLabelObjectPointer const ZeroSet = LevelSetLabelObjectType::New();
    ZeroSet->SetLabel(LevelSetType::ZeroLayer());

    LevelSetLayerConstIterator       nodeIt = layer0.begin();
    LevelSetLayerConstIterator const nodeEnd = layer0.end();
  ...
  FindPlusOneMinusOneLayer()
{
  ...
  LevelSetLabelObjectPointer const ObjectMinus1 = LevelSetLabelObjectType::New();
  ...
  LevelSetLabelObjectPointer const ObjectPlus1 = LevelSetLabelObjectType::New();
...
{
  LevelSetLabelObjectPointer const labelObject = this->m_LabelMap->GetLabelObject(LevelSetType::MinusOneLayer());
  ...
  LevelSetLabelObjectPointer const ObjectZero = LevelSetLabelObjectType::New();
...
BinaryImageToLevelSetImageAdaptor<TInput, MalcolmSparseLevelSetImage<TInput::ImageDimension>>::CreateMinimalInterface()
{
  ...
  while (nodeIt != nodeEnd)
  {
    LevelSetInputType const currentIdx = nodeIt->first;
    ...
    {
      LayerIdType const tempValue = i.Get();
