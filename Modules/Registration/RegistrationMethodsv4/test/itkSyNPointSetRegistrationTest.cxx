...
itkSyNPointSetRegistrationTest(int itkNotUsed(argc), char * itkNotUsed(argv)[])
{
  ...
  double const varianceForUpdateField = 5;
  double const varianceForTotalField = 0.0;
  ...
  PointType::ValueType const tolerance = 0.1;
  ...
  for (unsigned int n = 0; n < movingPoints->GetNumberOfPoints(); ++n)
  {
    ...
    PointType const transformedFixedPoint =
  ...
  unsigned int const numberOfPoints = movingPoints->GetNumberOfPoints();
  if (numberOfPoints > 0)
  {
