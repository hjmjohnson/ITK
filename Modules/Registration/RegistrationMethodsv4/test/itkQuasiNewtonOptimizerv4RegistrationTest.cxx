...
{
  std::string const metricString = argv[2];
  ...
  field->Allocate();
  // Fill it with 0's
  typename DisplacementTransformType::OutputVectorType const zeroVector{};
  ...
  typename RegistrationParameterScalesFromShiftType::Pointer const shiftScaleEstimator =
  ...
  std::string const outfilename(argv[5]);
  ...
  std::string const defout = name + std::string("_def") + ext;
...
itkQuasiNewtonOptimizerv4RegistrationTest(int argc, char * argv[])
{
  ...
  unsigned int const Dimension = std::stoi(argv[1]);
