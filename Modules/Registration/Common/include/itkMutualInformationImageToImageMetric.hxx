  ...
  SpatialSampleContainer & samples) const
{
  ...
  typename SpatialSampleContainer::const_iterator const end = samples.end();
  ...
  SizeValueType const dryRunTolerance = this->GetFixedImageRegion().GetNumberOfPixels();
  for (iter = samples.begin(); iter != end; ++iter)
  {
    // Get sampled index
    FixedImageIndexType const index = randIter.GetIndex();
    ...
    MovingImagePointType const mappedPoint = this->m_Transform->TransformPoint(iter->FixedImagePointValue);
  ...
  -> MeasureType
{
  ...
  typename SpatialSampleContainer::const_iterator const aend = m_SampleA.end();
  typename SpatialSampleContainer::const_iterator       biter;
  typename SpatialSampleContainer::const_iterator const bend = m_SampleB.end();
  ...
  double const threshold = -0.5 * nsamp * std::log(m_MinProbability);
...
{
  value = MeasureType{};
  unsigned int const numberOfParameters = this->m_Transform->GetNumberOfParameters();
...
{
  MovingImagePointType const mappedPoint = this->m_Transform->TransformPoint(point);
