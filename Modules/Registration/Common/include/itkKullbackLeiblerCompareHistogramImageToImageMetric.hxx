  ...
  HistogramType & histogram) const -> MeasureType
{
  ...
  HistogramIteratorType const measured_end = histogram.End();

  HistogramIteratorType       training_it = this->GetTrainingHistogram()->Begin();
  HistogramIteratorType const training_end = this->GetTrainingHistogram()->End();

  while (measured_it != measured_end)
  {
    // Every bin gets epsilon added to it
    double const TrainingFreq = training_it.GetFrequency() + m_Epsilon;
    double const MeasuredFreq = measured_it.GetFrequency() + m_Epsilon;
  ...
  HistogramFrequencyType const totalTrainingFreq = this->GetTrainingHistogram()->GetTotalFrequency();
  HistogramFrequencyType const totalMeasuredFreq = histogram.GetTotalFrequency();

  // The actual number of total frequency is a bit larger
  // than the number of counts because we add m_Epsilon to every bin
  double const AdjustedTotalTrainingFreq =
    totalTrainingFreq + this->GetHistogramSize()[0] * this->GetHistogramSize()[1] * m_Epsilon;
  double const AdjustedTotalMeasuredFreq =
