  ...
  const TransformParametersType & parameters) const -> MeasureType
{
  ...
  PointIterator const pointEnd = fixedPointSet->GetPoints()->End();

  PointDataIterator       pointDataItr = fixedPointSet->GetPointData()->Begin();
  PointDataIterator const pointDataEnd = fixedPointSet->GetPointData()->End();

  MeasureType measure{};

  this->m_NumberOfPixelsCounted = 0;
  double const lambdaSquared = std::pow(this->m_Lambda, 2);
