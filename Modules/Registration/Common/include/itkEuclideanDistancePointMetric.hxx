...
{
  MovingPointSetConstPointer const movingPointSet = this->GetMovingPointSet();
...
{
  FixedPointSetConstPointer const fixedPointSet = this->GetFixedPointSet();
  ...
  MovingPointIterator const pointEnd = movingPointSet->GetPoints()->End();
  ...
  while (pointItr != pointEnd)
  {
    ...
    typename Superclass::OutputPointType const transformedPoint = this->m_Transform->TransformPoint(inputPoint);
    ...
    if (!closestPoint)
    {
      ...
      FixedPointIterator const pointEnd2 = fixedPointSet->GetPoints()->End();
