  ...
  -> MeasureType
{
  ...
  //
  MovingImageConstPointer const movingImage = this->m_MovingImage;
  if (!movingImage)
  {
  ...
  while (!fi.IsAtEnd())
  {
    ...
    //
    //
    OutputPointType const transformedPoint = this->m_Transform->TransformPoint(fixedInputPoint);
                                                                           ...
                                                                           DerivativeType & derivative) const
{
  ...
  {
    double const areaSum = static_cast<double>(fixedArea) + static_cast<double>(movingArea);
    for (unsigned int par = 0; par < ParametersDimension; ++par)
    {
