...
HistogramImageToImageMetric<TFixedImage, TMovingImage>::Initialize()
{
  ...
  if (!m_LowerBoundSetByUser || !m_UpperBoundSetByUser)
  {
    ...
    FixedImageConstPointerType const         pFixedImage = this->m_FixedImage;
    ...
    while (!fiIt.IsAtEnd())
    {
      FixedImagePixelType const value = fiIt.Value();
    ...
    MovingImageConstPointerType const         pMovingImage = this->m_MovingImage;
    ...
    while (!miIt.IsAtEnd())
    {
      MovingImagePixelType const value = miIt.Value();
                                                                      ...
                                                                      DerivativeType &                derivative) const
{
  ...
  for (unsigned int i = 0; i < ParametersDimension; ++i)
  {
    ...
    MeasureType const e0 = EvaluateMeasure(*pHistogram2);
    ...
    MeasureType const e1 = EvaluateMeasure(*pHistogram2);
...
{
  FixedImageConstPointerType const fixedImage = this->m_FixedImage;
  ...
  while (!ti.IsAtEnd())
  {
    ...
    if (fixedRegion.IsInside(index) && (!m_UsePaddingValue || (m_UsePaddingValue && ti.Get() > m_PaddingValue)))
    {
      ...
      OutputPointType const transformedPoint = this->m_Transform->TransformPoint(inputPoint);
                                                                      ...
                                                                      HistogramType & source) const
{
  ...
  typename HistogramType::Iterator const sourceEnd = source.End();
  typename HistogramType::Iterator       targetIt = target.Begin();
  typename HistogramType::Iterator const targetEnd = target.End();

  while (sourceIt != sourceEnd && targetIt != targetEnd)
  {
    typename HistogramType::AbsoluteFrequencyType const freq = sourceIt.GetFrequency();
