...
main(int argc, char * argv[])
{
  ...
  TransformType::Pointer const transformLow = TransformType::New();
  ...
  TransformType::Pointer const transformHigh = TransformType::New();
  ...
  for (unsigned int k = 0; k < SpaceDimension; ++k)
  {
    ...
    ResamplerType::Pointer const upsampler = ResamplerType::New();

    using FunctionType = itk::BSplineResampleImageFunction<ParametersImageType, double>;
    FunctionType::Pointer const function = FunctionType::New();

    using IdentityTransformType = itk::IdentityTransform<double, SpaceDimension>;
    IdentityTransformType::Pointer const identity = IdentityTransformType::New();
    ...
    DecompositionType::Pointer const decomposition = DecompositionType::New();

    decomposition->SetSplineOrder(SplineOrder);
    decomposition->SetInput(upsampler->GetOutput());
    decomposition->Update();

    ParametersImageType::Pointer const newCoefficients = decomposition->GetOutput();
  ...
  DifferenceFilterType::Pointer const difference = DifferenceFilterType::New();

  WriterType::Pointer const writer2 = WriterType::New();
  ...
  DisplacementFieldType::Pointer const field = DisplacementFieldType::New();
  ...
  FieldWriterType::Pointer const fieldWriter = FieldWriterType::New();
