...
itkCenteredTransformInitializerTest(int, char *[])
{
  ...
  std::cout << "Running tests with itk::Image" << std::endl;
  {
    ...
    RegionType const fixedRegion{ fixedIndex, size };

    RegionType const movingRegion{ movingIndex, size };
    ...
    DirectionType const fixedDirection = (x * y * z).GetMatrix();
    DirectionType const movingDirection = (z * y * x).GetMatrix();
