             ...
             int                                                      scaleFactor)
{
  ...
  double const angle = 10 * nPI / 180.0;
...
{
  typename TransformInitializerType::TransformType::Pointer const transform =
  ...
  typename OutputVectorType::RealValueType const tolerance = 0.1;
  unsigned int                                   failed = 0;

  while (mitr != movingLandmarks.end())
  {
    typename TransformInitializerType::LandmarkPointType const transformedPoint = transform->TransformPoint(*fitr);
...
itkLandmarkBasedTransformInitializerTest(int, char *[])
{
...

  {
    ...
    FixedImageType::Pointer const  fixedImage = CreateTestImage<Dimension>();
    MovingImageType::Pointer const movingImage = CreateTestImage<Dimension>();
...

  {
    ...
    ImageType::Pointer const fixedImage = CreateTestImage<Dimension>();
    ImageType::Pointer const movingImage = CreateTestImage<Dimension>();
    ...
    double const       fixedLandMarkInit[numLandmarks][3] = {
    ...
    double const movingLandmarkInit[numLandmarks][3] = {
    ...
    double const weights[numLandmarks] = { 10, 1, 10, 1, 1, 1, 0.001, 0.001 };
...

  {
    ...
    double const           weights[numLandmarks] = { 1, 3, 0.01, 0.5 };
