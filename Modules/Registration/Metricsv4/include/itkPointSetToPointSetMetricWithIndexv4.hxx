...
PointSetToPointSetMetricWithIndexv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::Initialize()
{
  ...
  if (this->HasLocalSupport())
  {
    if (!this->m_UserHasSetVirtualDomain)
    {
      ...
      typename DisplacementFieldType::ConstPointer const field = displacementTransform->GetDisplacementField();
  ...
  PointSetToPointSetMetricWithIndexv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::GetValue() const
{
  ...
  std::function<void(SizeValueType)> const       sumNeighborhoodValues =
    [this, &threadValues, &ranges, &virtualTransformedPointSet, &fixedTransformedPointSet](SizeValueType rangeIndex) {
      ...
      for (PointIdentifier index = ranges[rangeIndex].first; index < ranges[rangeIndex].second; ++index)
      {
        if (this->IsInsideVirtualDomain(virtualTransformedPointSet[index]))
        {
          ...
          {
            bool const doesPointDataExist = this->m_FixedPointSet->GetPointData(index, &pixel);
  ...
  TransformMovingPointSet() const
{
  ...
  if (update)
  {
    ...
    typename MovingTransformType::InverseTransformBasePointer const inverseTransform =
    ...
    while (It != this->m_MovingPointSet->GetPoints()->End())
    {
      if (this->m_CalculateValueAndDerivativeInTangentSpace)
      {
        PointType const point = inverseTransform->TransformPoint(It.Value());
  ...
  TransformFixedAndCreateVirtualPointSet() const
{
  ...
  if (update)
  {
    ...
    InverseTransformBasePointer const inverseTransform = this->m_FixedTransform->GetInverseTransform();
...
const typename PointSetToPointSetMetricWithIndexv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::
  ...
  PointSetToPointSetMetricWithIndexv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::CreateRanges()
    const
{
  PointIdentifier const nPoints = this->m_FixedTransformedPointSet->GetNumberOfPoints();
  ...
  {
    PointIdentifier const endRange = (p * nPoints) / static_cast<double>(nWorkUnits);
