...
LabeledPointSetToPointSetMetricv4<TFixedPointSet, TMovingPointSet, TInternalComputationValueType>::Initialize()
{
  ...
  {
    typename PointSetMetricType::Pointer const metric =
    ...
    FixedPointSetPointer const  fixedPointSet = this->GetLabeledFixedPointSet(*it);
    MovingPointSetPointer const movingPointSet = this->GetLabeledMovingPointSet(*it);
  ...
  GetLocalNeighborhoodValue(const PointType & point, const LabelType & label) const
{
  ...
  {
    unsigned int const labelIndex = labelIt - this->m_CommonPointSetLabels.begin();
    MeasureType const  value = this->m_PointSetMetricClones[labelIndex]->GetLocalNeighborhoodValue(point, label);
    return value;
  }
}
