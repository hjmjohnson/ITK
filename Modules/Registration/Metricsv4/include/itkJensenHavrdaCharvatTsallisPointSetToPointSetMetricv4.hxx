                            ...
                            bool                  calcDerivative) const
{
   ...
   * first term only
   */
  typename PointSetType::PointIdentifier const numberOfMovingPoints =
  ...
  if (calcValue)
  {
    RealType const realOne = NumericTraits<RealType>::OneValue();
  ...
  if (calcDerivative)
  {
    RealType const probabilityStarFactor = std::pow(probabilityStar, static_cast<RealType>(2.0 - this->m_Alpha));
    ...
    {
      RealType const gaussian = this->m_MovingDensityFunction->GetGaussian(neighbors[n])->Evaluate(samplePoint);
      ...
      {
        typename GaussianType::CovarianceMatrixType const Ci =
      ...
      DerivativeValueType const factor = this->m_Prefactor1 * gaussian / probabilityStarFactor;
      for (unsigned int i = 0; i < PointDimension; ++i)
      {
