                                    ...
                                    const ThreadIdType              threadId) const
{
  ...
  FixedImagePixelType const diff = fixedImageValue - movingImageValue;
  ...
  {
    MeasureType const diffC = DefaultConvertPixelTraits<FixedImagePixelType>::GetNthComponent(nc, diff);
  ...
  for (unsigned int par = 0; par < this->GetCachedNumberOfLocalParameters(); ++par)
  {
    ...
    for (unsigned int nc = 0; nc < nComponents; ++nc)
    {
      MeasureType const diffValue = DefaultConvertPixelTraits<FixedImagePixelType>::GetNthComponent(nc, diff);
