...

  {
    ...
    if (this->m_UseSampledPointSet) // Analysis region defined by SampledPointSet
    {
      if (this->m_UseVirtualSampledPointSet) // Sparse points defined in VirtualSpace
      {
        ...
        typename Superclass::FixedSampledPointSetType::PointsContainerConstIterator const end =
                                            ...
                                            TMetricTraits>::DerivativeBufferManager::CheckAndReduceIfNecessary()
{
  if (m_CurrentFillSize == m_MaxBufferSize)
  {
    ...
    std::unique_lock<std::mutex> const FirstTryLockHolder(*this->m_ParentJointPDFDerivativesMutexPtr, std::try_to_lock);
    ...
    else if (m_MaxBufferSize < 5000)
    {
      ...
      std::unique_lock<std::mutex> const SecondTryLockHolder(*this->m_ParentJointPDFDerivativesMutexPtr,
                                                             std::try_to_lock);
