...
JointHistogramMutualInformationGetValueAndDerivativeThreader<
  TDomainPartitioner,
  TImageToImageMetric,
  TJointHistogramMetric>::ProcessPoint(const VirtualIndexType &,
                                       ...
                                       const ThreadIdType threadId) const
{
  ...
  InternalComputationValueType const jointPDFValue =
    this->m_JointHistogramMIPerThreadVariables[threadId].JointPDFInterpolator->Evaluate(jointPDFpoint);
  SizeValueType const                 ind = 1;
  InternalComputationValueType const  dJPDF = this->ComputeJointPDFDerivative(jointPDFpoint, threadId, ind);
  typename MarginalPDFType::PointType mind;
  mind[0] = jointPDFpoint[ind];
  InternalComputationValueType const movingImagePDFValue =
    this->m_JointHistogramMIPerThreadVariables[threadId].MovingImageMarginalPDFInterpolator->Evaluate(mind);
  InternalComputationValueType const dMmPDF = this->ComputeMovingImageMarginalPDFDerivative(mind, threadId);
...
JointHistogramMutualInformationGetValueAndDerivativeThreader<
  ...
  TJointHistogramMetric>::ComputeMovingImageMarginalPDFDerivative(const MarginalPDFPointType & margPDFpoint,
                                                                  const ThreadIdType           threadId) const
{
  InternalComputationValueType const offset = 0.5 * this->m_JointAssociate->m_JointPDFSpacing[0];
  InternalComputationValueType const eps = this->m_JointAssociate->m_JointPDFSpacing[0];
  MarginalPDFPointType               leftpoint = margPDFpoint;
  leftpoint[0] -= offset;
  MarginalPDFPointType rightpoint = margPDFpoint;
  rightpoint[0] += offset;
  ...
  InternalComputationValueType const delta = rightpoint[0] - leftpoint[0];
  if (delta > InternalComputationValueType{})
  {
    InternalComputationValueType const deriv =
...
JointHistogramMutualInformationGetValueAndDerivativeThreader<
  TDomainPartitioner,
  TImageToImageMetric,
  TJointHistogramMetric>::ComputeJointPDFDerivative(const JointPDFPointType & jointPDFpoint,
                                                    const ThreadIdType        threadId,
                                                    const SizeValueType       ind) const
{
  InternalComputationValueType const offset = 0.5 * this->m_JointAssociate->m_JointPDFSpacing[ind];
  InternalComputationValueType const eps = this->m_JointAssociate->m_JointPDFSpacing[ind];
  ...
  InternalComputationValueType const delta = rightpoint[ind] - leftpoint[ind];
  InternalComputationValueType       deriv{};
  if (delta > InternalComputationValueType{})
  {
