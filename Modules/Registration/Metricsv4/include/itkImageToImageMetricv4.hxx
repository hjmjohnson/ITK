  ...
  Initialize()
{
  ...
  if (!this->m_UserHasSetVirtualDomain)
  {
    ...
    VirtualImagePointer const image = VirtualImageType::New();
  ...
  MapFixedSampledPointSetToVirtual()
{
  ...
  typename PointsContainer::ConstPointer const points = this->m_FixedSampledPointSet->GetPoints();
  if (points.IsNull())
  {
  ...
  typename FixedTransformType::InverseTransformBasePointer const inverseTransform =
  ...
  {
    typename FixedSampledPointSetType::PointType const point = inverseTransform->TransformPoint(fixedIt.Value());
  ...
  GetNumberOfDomainPoints() const
{
  ...
  {
    typename VirtualImageType::RegionType const region = this->GetVirtualRegion();
