                                                   ...
                                                   ImageToImageMetricv4TestMetricType::DerivativeType & truthDerivative)
{
  ...
  while (!itFixed.IsAtEnd() && !itMoving.IsAtEnd())
  {
    ...
    {
      ImageToImageMetricv4TestMetricType::FixedImageGradientImageType::ConstPointer const fixedGradientImage =
    ...
    else
    {
      ...
      FixedGradientCalculatorPointer const fixedGradientCalculator = metric->GetFixedImageGradientCalculator();
    ...
    {
      ImageToImageMetricv4TestMetricType::MovingImageGradientImageType::ConstPointer const movingGradientImage =
...
{
  bool const origGlobalWarningValue = itk::Object::GetGlobalWarningDisplay();
  itk::Object::SetGlobalWarningDisplay(true);

  using DimensionSizeType = unsigned int;
  constexpr DimensionSizeType imageSize = 4;

  ImageToImageMetricv4TestImageType::SizeType const   size = { { imageSize, imageSize } };
  ImageToImageMetricv4TestImageType::IndexType const  index = { { 0, 0 } };
  ImageToImageMetricv4TestImageType::RegionType const region{ index, size };
  auto spacing = itk::MakeFilled<ImageToImageMetricv4TestImageType::SpacingType>(1.0);
  ImageToImageMetricv4TestImageType::PointType const origin{};
  ...
  ImageToImageMetricv4TestMetricPointer const metric = ImageToImageMetricv4TestMetricType::New();
  ...
  for (itk::ThreadIdType numberOfThreads = 1; numberOfThreads < 6; ++numberOfThreads)
  {
    ...
    for (signed char useMovingFilter = 1; useMovingFilter >= 0; --useMovingFilter)
    {
      for (signed char useFixedFilter = 1; useFixedFilter >= 0; --useFixedFilter)
      {
        ...
        bool const useFixedImageGradientFilter = useFixedFilter == 1;
        ITK_TEST_SET_GET_BOOLEAN(metric, UseFixedImageGradientFilter, useFixedImageGradientFilter);

        bool const useMovingImageGradientFilter = useMovingFilter == 1;
  ...
  bool const useVirtualSampledPointSet = false;
  ITK_TEST_SET_GET_BOOLEAN(metric, UseVirtualSampledPointSet, useVirtualSampledPointSet);

  bool const useFloatingPointCorrection = false;
  ...
  PointSetType::Pointer const pset(PointSetType::New());
  ...
  bool const useSampledPointSet = true;
  ...
  typename ImageToImageMetricv4TestMetricType::DerivativeValueType const floatingPointCorrectionResolution = 1.0;
  ...
  itk::ThreadIdType const thread = 0;
