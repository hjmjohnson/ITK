                                                                                 ...
                                                                                 itk::SizeValueType vecdim)
{
  ...
  typename ImageType::RegionType const imageRegion = image->GetBufferedRegion();

  // only display the first slice
  itk::SizeValueType const dim0 = imageRegion.GetSize()[0];
  itk::SizeValueType const dim1 = imageRegion.GetSize()[1];
...
ANTSNeighborhoodCorrelationImageToImageMetricv4Test_PrintImage(ImageType * imageP)
{
  ...
  ImageConstPointerType const image = imageP;
...
itkANTSNeighborhoodCorrelationImageToImageMetricv4Test(int, char ** const)
{
  ...
  MetricTypePointer const metric = MetricType::New();
  ...
  MetricTypePointer const metricSparse = MetricType::New();
