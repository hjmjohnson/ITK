                                        ...
                                        bool                       doGradientFilter)
{
  ...
  itk::SizeValueType const   ImageSize = 100;
  itk::OffsetValueType const boundary = 6;
  ...
  typename TImage::PointType const origin{};
  ...
  typename TImage::Pointer const fixedImage = fixedImageSource->GetOutput();
  ...
  typename CyclicShiftFilterType::OffsetValueType const maxImageShift = boundary - 1;
  imageShift.Fill(maxImageShift);
  imageShift[0] = maxImageShift / 2;
  shiftFilter->SetInput(fixedImage);
  shiftFilter->SetShift(imageShift);
  shiftFilter->Update();
  typename TImage::Pointer const movingImage = shiftFilter->GetOutput();
  ...
  else
  {
    ...
    typename PointSetType::Pointer const      pset(PointSetType::New());
  ...
  typename TMetric::MeasureType const initialValue = metric->GetValue();

  // scales estimator
  using RegistrationParameterScalesFromPhysicalShiftType = itk::RegistrationParameterScalesFromPhysicalShift<TMetric>;
  typename RegistrationParameterScalesFromPhysicalShiftType::Pointer const shiftScaleEstimator =
