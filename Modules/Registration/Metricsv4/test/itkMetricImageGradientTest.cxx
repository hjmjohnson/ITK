...
public:
  ...
  itkOverrideGetNameOfClassMacro(VanillaImageToImageMetricv4);
                                  ...
                                  std::string &                outputPath)
{
  ...
  typename ImageType::RegionType const region{ virtualIndex, size };
  auto                                 spacing = itk::MakeFilled<typename ImageType::SpacingType>(1.0);
  typename ImageType::PointType const  origin{};
  ...
  it.GoToBegin();
  unsigned int const imageBorder = 20;
  while (!it.IsAtEnd())
  {
    it.Set(0);
  ...
  resample->Update();
  typename ImageType::Pointer const movingImage = resample->GetOutput();
  ...
  typename TTransform::Pointer const movingTransform = transform->GetInverseTransform().GetPointer();
  ...
  typename ImageType::PointType const mappedPoint = movingTransform->TransformPoint(virtualPoint);
  ...
  for (unsigned int i = 0; i < 2; ++i)
  {
    ...
    bool const b = metric->TransformAndEvaluateMovingPoint(virtualPoint, mappedMovingPoint, mappedMovingPixelValue);
    ...
    if (b)
    {
      ...
      vnl_vector_ref<double> const p2 = mappedMovingImageGradient.GetVnlVector();
      vnl_vector_ref<double> const p1 = mappedMovingImageGradientGroundtruth.GetVnlVector();

      double const norm1 = p1.two_norm();
      double const norm2 = p2.two_norm();
      if (norm1 > 0 && norm2 > 0)
      {
        double const correlation = dot_product(p2, p1) / (norm1 * norm2);
        sumc += correlation;
      }
...
itkMetricImageGradientTest(int argc, char * argv[])
{
  ...
  std::string const commandName(argv[0]);
  ...
  for (dimensionality = 2; dimensionality <= 3; ++dimensionality)
  {
    ...
    for (rotationDegrees = 0.0; rotationDegrees < maxDegrees; rotationDegrees += degreeStep)
    {
      ...
      if (dimensionality == 3)
      {
        ...
        auto transform = TransformType::New();
        transform->SetIdentity();
        double const angleRad = itk::Math::pi * rotationDegrees / 180;
