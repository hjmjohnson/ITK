...
itkJensenHavrdaCharvatTsallisPointSetMetricTestRun()
{
  ...
  unsigned int const numberOfAlphaValues = 6;
  float const        alphaValues[] = { 1.0f, 1.2f, 1.4f, 1.6f, 1.8f, 2.0f };

  unsigned int const evaluationKNeighborhood = 50;
  auto               useAnisotropicCovariances = false;
  unsigned int const covarianceKNeighborhood = 5;
  ...
  for (unsigned int i = 0; i < numberOfAlphaValues; ++i)
  {
    ...
    typename PointSetMetricType::RealType const pointSetSigma = 1.0;
    ...
    typename PointSetMetricType::RealType const kernelSigma = 10.0;
