...
public:
  ...
  Execute(const itk::Object * object, const itk::EventObject & event) override
  {
    ...
    std::string const  ext = itksys::SystemTools::GetFilenameExtension(this->m_OutputFileNameBase);
    std::string const  name = itksys::SystemTools::GetFilenameWithoutExtension(this->m_OutputFileNameBase);
    std::string const  path = itksys::SystemTools::GetFilenamePath(this->m_OutputFileNameBase);
...
itkJointHistogramMutualInformationImageToImageRegistrationTest(int argc, char * argv[])
{
  ...
  MovingImageType::Pointer const movingImage = movingImageReader->GetOutput();
  ...
  RegistrationParameterScalesFromShiftType::Pointer const shiftScaleEstimator =
  ...
  std::string const outfilename(argv[3]);
  std::string const ext = itksys::SystemTools::GetFilenameExtension(outfilename);
  std::string const name = itksys::SystemTools::GetFilenameWithoutExtension(outfilename);
  std::string const path = itksys::SystemTools::GetFilenamePath(outfilename);
  std::string const defout = path + std::string("/") + name + std::string("_def") + ext;
