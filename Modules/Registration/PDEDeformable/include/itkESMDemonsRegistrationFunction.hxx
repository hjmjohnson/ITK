...
ESMDemonsRegistrationFunction<TFixedImage, TMovingImage, TDisplacementField>::InitializeIteration()
{
  ...
  this->m_MovingImageWarperOutput = this->m_MovingImageWarper->GetOutput();
  ...
  const FloatOffsetType &  itkNotUsed(offset)) -> PixelType
{
  ...
  MovingPixelType movingPixValue = m_MovingImageWarperOutput->GetPixel(index);
  ...
  if ((this->m_UseGradientType == GradientEnum::Symmetric) || (this->m_UseGradientType == GradientEnum::WarpedMoving))
  {
    ...
    for (unsigned int dim = 0; dim < ImageDimension; ++dim)
    {
      ...
      if (movingPixValue == NumericTraits<MovingPixelType>::max())
      {
        ...
        {
          // backward difference
          warpedMovingGradient[dim] -= static_cast<double>(m_MovingImageWarperOutput->GetPixel(tmpIndex));
