...
{
  MovingImageConstPointer const movingPtr = this->GetMovingImage();
  FixedImageConstPointer const  fixedPtr = this->GetFixedImage();
...
{
  typename Superclass::InputImageType::ConstPointer const inputPtr = this->GetInput();
...
PDEDeformableRegistrationFilter<TFixedImage, TMovingImage, TDisplacementField>::GenerateInputRequestedRegion()
{
  ...
  MovingImagePointer const movingPtr = const_cast<MovingImageType *>(this->GetMovingImage());
  if (movingPtr)
  {
  ...
  DisplacementFieldPointer const inputPtr = const_cast<DisplacementFieldType *>(this->GetInput());
  DisplacementFieldPointer const outputPtr = this->GetOutput();
  FixedImagePointer const        fixedPtr = const_cast<FixedImageType *>(this->GetFixedImage());
...
{
  DisplacementFieldPointer const field = this->GetOutput();
  ...
  for (unsigned int j = 0; j < ImageDimension; ++j)
  {
    ...
    oper.SetDirection(j);
    double const variance = itk::Math::sqr(m_StandardDeviations[j]);
...
PDEDeformableRegistrationFilter<TFixedImage, TMovingImage, TDisplacementField>::SmoothUpdateField()
{
  ...
  DisplacementFieldPointer const field = this->GetUpdateBuffer();
  ...
  for (unsigned int j = 0; j < ImageDimension; ++j)
  {
    ...
    double const variance = itk::Math::sqr(this->GetUpdateFieldStandardDeviations()[j]);
