...
TemporalProcessObject::EnlargeOutputRequestedTemporalRegion(TemporalDataObject * output)
{
  ...
  TemporalRegion const outBufTempRegion = output->GetBufferedTemporalRegion();
  SizeValueType const  outReqStart = outReqTempRegion.GetFrameStart();
  SizeValueType        outReqDuration = outReqTempRegion.GetFrameDuration();
  SizeValueType const  outReqEnd = outReqDuration + outReqStart;
  SizeValueType const  outBufStart = outBufTempRegion.GetFrameStart();
  SizeValueType const  outBufEnd = outBufTempRegion.GetFrameDuration() + outBufStart;
  ...
  SizeValueType const remainder = outReqDuration % m_UnitOutputNumberOfFrames;
  if (remainder > 0)
  {
...
TemporalProcessObject::GenerateOutputRequestedTemporalRegion(TemporalDataObject * output)
{
  ...
  TemporalRegion const outputRequest = output->GetRequestedTemporalRegion();
  ...
  if (!outputRequest.GetFrameDuration() &&
      output->GetLargestPossibleTemporalRegion().GetFrameDuration() == ITK_INFINITE_FRAME_DURATION)
  {
  ...
  {
    output->SetRequestedTemporalRegion(output->GetLargestPossibleTemporalRegion());
  }
}
...
TemporalProcessObject::GenerateInputRequestedTemporalRegion()
{
  ...
  TemporalRegion const outReqTempRegion = output->GetRequestedTemporalRegion();
  ...
  // to be loaded.
  SizeValueType const inputDuration = m_FrameSkipPerOutput * (numInputRequests - 1) + m_UnitInputNumberOfFrames;

  // Compute the start of the input requested temporal region based on
  // m_InputStencilCurrentFrameIndex and FrameSkipPerOutput
  OffsetValueType const inputStart =
...
TemporalProcessObject::UpdateOutputInformation()
{
  ...
  OffsetValueType const scannableDuration = inputLargestRegion.GetFrameDuration() - m_UnitInputNumberOfFrames + 1;
  SizeValueType const   outputDuration =
  ...
  OffsetValueType const outputStart =
...
TemporalProcessObject::GenerateData()
{
  ...
  TemporalRegion const fullOutputRequest = output->GetRequestedTemporalRegion();
  ...
  for (const auto & inputTemporalRegionRequest : this->SplitRequestedTemporalRegion())
  {
    ...
    OffsetValueType const spareFrames = output->GetNumberOfBuffers() - (OffsetValueType)bufferedDuration;
...
TemporalProcessObject::SplitRequestedTemporalRegion()
{
  ...
  TemporalRegion const unbufferedRegion = outputObject->GetUnbufferedRequestedTemporalRegion();
  ...
  {
    regionStartFrame = unbufferedRegion.GetFrameStart() * m_FrameSkipPerOutput - m_InputStencilCurrentFrameIndex;
  }
  else if (this->m_FrameSkipPerOutput < 0)
  {
    regionStartFrame = unbufferedRegion.GetFrameStart() * m_FrameSkipPerOutput + unbufferedRegion.GetFrameDuration() +
                       1 - (this->m_UnitOutputNumberOfFrames - m_InputStencilCurrentFrameIndex);
  }
