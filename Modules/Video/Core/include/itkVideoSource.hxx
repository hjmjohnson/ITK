...
{
  typename OutputVideoStreamType::Pointer const output =
...
VideoSource<TOutputVideoStream>::GenerateOutputRequestedTemporalRegion(TemporalDataObject * output)
{
  ...
  SizeValueType const requestDuration = this->GetOutput()->GetRequestedTemporalRegion().GetFrameDuration();
  ...
  if (resetNumFrames)
  {
    SizeValueType const frameStart = this->GetOutput()->GetRequestedTemporalRegion().GetFrameStart();
    SizeValueType const numFrames = this->GetOutput()->GetRequestedTemporalRegion().GetFrameDuration();
...
VideoSource<TOutputVideoStream>::AllocateOutputs()
{
  ...
  TemporalRegion const unbufferedRegion = output->GetUnbufferedRequestedTemporalRegion();
  ...
  SizeValueType const numFrames = unbufferedRegion.GetFrameDuration();
  ...
  SizeValueType const startFrame = unbufferedRegion.GetFrameStart();
  for (SizeValueType i = startFrame; i < startFrame + numFrames; ++i)
  {
    ...
    typename OutputFrameType::Pointer const frame = output->GetFrame(i);
  ...
  typename TOutputVideoStream::SpatialRegionType & splitRegion)
{
  ...
  SizeValueType const     currentFrame = outputPtr->GetRequestedTemporalRegion().GetFrameStart();
  ...
  typename TOutputVideoStream::SizeType::SizeValueType const range = requestedRegionSize[splitAxis];
...
{
  int const workUnitID = ((MultiThreaderBase::WorkUnitInfo *)(arg))->WorkUnitID;
  int const threadCount = ((MultiThreaderBase::WorkUnitInfo *)(arg))->NumberOfWorkUnits;
  ...
  typename TOutputVideoStream::SpatialRegionType splitRegion;
  int const total = str->Filter->SplitRequestedSpatialRegion(workUnitID, threadCount, splitRegion);
