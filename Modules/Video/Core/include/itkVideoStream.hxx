                                                              ...
                                                              typename TFrameType::RegionType region)
{
  ...
  SizeValueType const bufDur = m_BufferedTemporalRegion.GetFrameDuration();
...
VideoStream<TFrameType>::InitializeEmptyFrames()
{
  ...
  SizeValueType const numFrames = m_RequestedTemporalRegion.GetFrameDuration();
  if (numFrames == 0)
  {
    return;
  }
  ...
  SizeValueType const startFrame = m_RequestedTemporalRegion.GetFrameStart();
  for (SizeValueType i = startFrame; i < startFrame + numFrames; ++i)
  {
    if (!m_DataObjectBuffer->BufferIsFull(i))
    {
      FramePointer const                        newFrame = FrameType::New();
      FrameType *                               newFrameRawPointer = newFrame.GetPointer();
      typename BufferType::ElementPointer const element =
...
VideoStream<TFrameType>::SetFrame(SizeValueType frameNumber, FramePointer frame)
{
  ...
  typename BufferType::ElementPointer const dataObject = dataObjectRawPointer;
...
VideoStream<TFrameType>::GetFrame(SizeValueType frameNumber) -> FrameType *
{
  ...
  // Fetch the frame
  typename BufferType::ElementPointer const element = m_DataObjectBuffer->GetBufferContents(frameNumber);
  FramePointer const                        frame = dynamic_cast<FrameType *>(element.GetPointer());
  return frame;
}
...
{
  typename BufferType::ElementPointer const element = m_DataObjectBuffer->GetBufferContents(frameNumber);
  FrameConstPointer const                   frame = dynamic_cast<FrameType *>(element.GetPointer());
  return frame;
}
...
VideoStream<TFrameType>::SetAllLargestPossibleSpatialRegions(typename TFrameType::RegionType region)
{
  ...
  SizeValueType const startFrame = m_LargestPossibleTemporalRegion.GetFrameStart();

  // If the largest region is infinite, use the largest of the requested or
  // buffered region
  if (numFrames == ITK_INFINITE_FRAME_DURATION)
  {
    SizeValueType const bufEnd = m_BufferedTemporalRegion.GetFrameStart() + m_BufferedTemporalRegion.GetFrameDuration();
    SizeValueType const reqEnd =
...
{
  SizeValueType const numFrames = m_BufferedTemporalRegion.GetFrameDuration();
