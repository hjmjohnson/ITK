...
{
  size_t const currentSize = this->m_PointerVector.size();
  ...
  if (n > currentSize)
  {
    ...
    {
      ElementPointer const newPointer = nullptr;
...
{
  OffsetValueType const moddedOffset = itk::Math::abs(offset) % this->GetNumberOfBuffers();
