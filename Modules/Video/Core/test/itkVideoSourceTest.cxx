...
protected:
  ...
  ThreadedGenerateData(const OutputFrameSpatialRegionType & outputRegionForThread, int threadId) override
  {
    ...
    typename OutputVideoStreamType::TemporalRegionType const requestedTemporalRegion =
      video->GetRequestedTemporalRegion();
    SizeValueType const startFrame = requestedTemporalRegion.GetFrameStart();
    SizeValueType const frameDuration = requestedTemporalRegion.GetFrameDuration();
...
CreateEmptyFrame()
{
  ...
  FrameType::IndexType const startLR{};
  sizeLR[0] = 50;
  sizeLR[1] = 40;
...
itkVideoSourceTest(int, char *[])
{
  ...
  VideoType::Pointer const videoOut = videoSource->GetOutput();
  ...
  VideoType::Pointer const output = videoSource->GetOutput();
  output->SetRequestedTemporalRegion(requestedRegion);
  output->InitializeEmptyFrames();
  FrameType::RegionType const spatialRegion = frame->GetRequestedRegion();
  ...
  SizeValueType const frameStart = requestedRegion.GetFrameStart();
  SizeValueType const numFrames = requestedRegion.GetFrameDuration();
  for (SizeValueType i = frameStart; i < frameStart + numFrames; ++i)
  {
    frame = videoSource->GetOutput()->GetFrame(i);
    FrameType::RegionType const         region = frame->GetRequestedRegion();
  ...
  unsigned int const  newNumBuffers = 25;
