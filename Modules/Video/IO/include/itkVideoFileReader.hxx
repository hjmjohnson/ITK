...
VideoFileReader<TOutputVideoStream>::UpdateOutputInformation()
{
  ...
  VideoStreamPointer const output = this->GetOutput();
...
VideoFileReader<TOutputVideoStream>::InitializeVideoIO()
{
  ...
  IOComponentEnum const ioType = ImageIOBase::MapPixelType<typename ConvertPixelTraits::ComponentType>::CType;
...
VideoFileReader<TOutputVideoStream>::TemporalStreamingGenerateData()
{
  ...
  FrameOffsetType const frameNum = requestedTemporalRegion.GetFrameStart();

  // Figure out if we need to skip frames
  FrameOffsetType const currentIOFrame = m_VideoIO->GetCurrentFrame();
  ...
  if (this->m_PixelConversionNeeded)
  {
    ...
    size_t const bufferSize = m_VideoIO->GetImageSizeInBytes();
...
VideoFileReader<TOutputVideoStream>::DoConvertBuffer(const void * inputData, FrameOffsetType frameNumber)
{
  ...
  unsigned int const numberOfPixels = this->GetOutput()->GetFrame(frameNumber)->GetPixelContainer()->Size();
  bool const isVectorImage(strcmp(this->GetOutput()->GetFrame(frameNumber)->GetNameOfClass(), "VectorImage") == 0);
