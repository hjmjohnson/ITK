...
VideoFileWriter<TInputVideoStream>::Write()
{
  ...
  // frame as output
  SizeValueType const frameStart = m_OutputTemporalRegion.GetFrameStart();
  SizeValueType const numFrames = m_OutputTemporalRegion.GetFrameDuration();
...
VideoFileWriter<TInputVideoStream>::TemporalStreamingGenerateData()
{
  ...
  SizeValueType const frameNum = output->GetRequestedTemporalRegion().GetFrameStart();
...
VideoFileWriter<TInputVideoStream>::InitializeOutputParameters()
{
  ...
  SizeValueType const frameNum = this->GetInput()->GetRequestedTemporalRegion().GetFrameStart();
