...
public:
  ...
  {
    itkExceptionStringMacro("itk::OpenCVVideoCapture::open(filename) -> If you just want to read from a file, use "
                            "cv::VideoCapture since there is nothing to be gained using itk's version.");
  }

  virtual bool
  open(int)
  {
    itkExceptionStringMacro("itk::OpenCVVideoCapture::open(device) -> If you just want to read from a device, use "
                            "cv::VideoCapture since there is nothing to be gained using itk's version.");
  }
  ...
  virtual bool
  retrieve(cv::Mat & image, int channel = 0);
