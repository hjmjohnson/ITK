...
itkFrameAverageVideoFilterTest(int argc, char * argv[])
{
  ...
  SizeValueType const numInputFrames = 50;
  ...
  for (unsigned int i = outputStart; i < outputStart + outputDuration; ++i)
  {
    ...
    // Check the results
    OutputPixelType const expectedVal = (OutputPixelType)(i + (i + 1)) / 2.0;
    OutputPixelType const actualVal = filter->GetOutput()->GetFrame(i)->GetPixel(checkPx);
  ...
  for (unsigned int i = outputStart; i < outputStart + outputDuration; ++i)
  {
    OutputPixelType const expectedVal = (OutputPixelType)(i + (i + 1) + (i + 2)) / 3.0;
    OutputPixelType const actualVal = filter->GetOutput()->GetFrame(i)->GetPixel(checkPx);
    double const          eps = 0.00001;
    if (expectedVal < actualVal - eps || expectedVal > actualVal + eps)
    {
