...
bool DICOMParser::IsValidRepresentation(
  DICOMSource& source, doublebyte rep, quadbyte& len, VRTypes& mytype)
{
  switch (rep)
  {
  ...
  DICOMSource& source, doublebyte& group, doublebyte& element, DICOMParser::VRTypes& mytype)
{
  ...
  //   dicom_stream::cout << "representation = " << representation << dicom_stream::dec << ", length
  //   = " << length
  //                      << dicom_stream::endl;
  ...
  if (iter != Implementation->Map.end())
  {
    ...
    bool doSwap =
      (this->ToggleByteSwapImageData ^ source.GetPlatformIsBigEndian()) && callbackType == VR_OW;
...
{
  // dicom_stream::cout << dicom_stream::dec << "ParseSequence(), len = " << len <<
  // dicom_stream::endl;
  ...
  while (DataBuffer.Tell() < len)
  {
    ...
    {
      dicom_stream::cerr
        << "DICOMParser:: sequence missing data element tag.  Skipping rest of sequence."
        << dicom_stream::endl;
      return;
    }

    itemLength = DataBuffer.ReadQuadByte();
    // dicom_stream::cout << "itemLength = " << dicom_stream::hex << itemLength << dicom_stream::dec
    // << dicom_stream::endl;

    if (static_cast<unsigned int>(itemLength) == 0xFFFFFFFFul)
    {
      // undetermined length.  punt for now
      dicom_stream::cerr << "DICOMParser:: sequence of undetermined length.  Skipping sequence."
                         << dicom_stream::endl;
      return;
    }
    else
