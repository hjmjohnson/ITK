...
public:
  ...
  typedef dicom_stl::map<dicom_stl::string, dicom_stl::vector<dicom_stl::string>, ltstdstr>
  ...
  typedef dicom_stl::map<dicom_stl::string, dicom_stl::string, ltstdstr>
    SeriesUIDToSeriesDescriptionMapType;
  ...
  typedef dicom_stl::map<dicom_stl::string, ContoursVectorType, ltstdstr>
    SeriesUIDToContoursMapType;
  ...
  typedef dicom_stl::map<dicom_stl::string, InstanceUIDVectorType, ltstdstr>
    SeriesUIDToReferencedInstanceUIDMapType;
...
void DICOMAppHelper::RegisterCallbacks(DICOMParser* parser)
{
  ...
  NumberOfContourPointsCB->SetCallbackFunction(
    this, &DICOMAppHelper::NumberOfContourPointsCallback);
  ...
  ReferencedInstanceUIDCB->SetCallbackFunction(
    this, &DICOMAppHelper::ReferencedInstanceUIDCallback);
  ...
  NumberOfSeriesInStudyCB->SetCallbackFunction(
    this, &DICOMAppHelper::NumberOfSeriesInStudyCallback);
  parser->AddDICOMTagCallback(0x0020, 0x1000, DICOMParser::VR_IS, NumberOfSeriesInStudyCB);

  NumberOfStudyRelatedSeriesCB->SetCallbackFunction(
    this, &DICOMAppHelper::NumberOfStudyRelatedSeriesCallback);
...
void DICOMAppHelper::DefaultCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char*, quadbyte)
{
#ifdef DEBUG_DICOM_APP_HELPER
...
void DICOMAppHelper::InstanceUIDCallback(DICOMParser* parser, doublebyte, doublebyte,
  DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  ...
  // to filenames
  this->Implementation->InstanceUIDToFileNameMap.insert(
    dicom_stl::pair<const dicom_stl::string, dicom_stl::string>(
      this->InstanceUID, parser->GetFileName()));
}

void DICOMAppHelper::ReferencedInstanceUIDCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  ...
  // with the contours
  this->Implementation->SeriesUIDToReferencedInstanceUIDMap[this->CurrentSeriesUID].push_back(
    newStdString);
}

void DICOMAppHelper::ContourImageSequenceCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char*, quadbyte)
{
  ...
  DICOMParser* parser, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  ...
  // UIDs to SeriesUIDs
  this->Implementation->InstanceUIDToSeriesUIDMap.insert(
    dicom_stl::pair<const dicom_stl::string, dicom_stl::string>(this->InstanceUID, newStdString));

  // Put the series UID into the internal database mapping series UIDs
  // to InstanceUIDs
  DICOMAppHelperImplementation::SeriesUIDToInstanceUIDMapType::iterator iiter =
    this->Implementation->SeriesUIDToInstanceUIDMap.find(newStdString);
  if (iiter == this->Implementation->SeriesUIDToInstanceUIDMap.end())
  {
    ...
    this->Implementation->SeriesUIDToInstanceUIDMap.insert(
  ...
  // Put the series UID into the internal database mapping series UIDs
  // to contours
  DICOMAppHelperImplementation::SeriesUIDToContoursMapType::iterator citer =
    this->Implementation->SeriesUIDToContoursMap.find(newStdString);
  if (citer == this->Implementation->SeriesUIDToContoursMap.end())
  {
    DICOMAppHelperImplementation::ContoursVectorType newVector;

    this->Implementation->SeriesUIDToContoursMap.insert(
      dicom_stl::pair<const dicom_stl::string, DICOMAppHelperImplementation::ContoursVectorType>(
        newStdString, newVector));
  }
  // Put the series UID into the internal database mapping series UIDs
  // to referenced instance UIDs
  DICOMAppHelperImplementation::SeriesUIDToReferencedInstanceUIDMapType::iterator riter =
    this->Implementation->SeriesUIDToReferencedInstanceUIDMap.find(newStdString);
  if (riter == this->Implementation->SeriesUIDToReferencedInstanceUIDMap.end())
  {
    ...
    this->Implementation->SeriesUIDToReferencedInstanceUIDMap.insert(
      dicom_stl::pair<const dicom_stl::string, DICOMAppHelperImplementation::InstanceUIDVectorType>(
        newStdString, newVector));
  }
...
void DICOMAppHelper::SeriesDescriptionCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (len == 0)
  {
  ...
  // to InstanceUIDs
  DICOMAppHelperImplementation::SeriesUIDToSeriesDescriptionMapType::iterator iiter =
    this->Implementation->SeriesUIDToSeriesDescriptionMap.find(this->CurrentSeriesUID);
  if (iiter ==
    this->Implementation->SeriesUIDToSeriesDescriptionMap.end()) // if not found we insert
  {
    this->Implementation->SeriesUIDToSeriesDescriptionMap.insert(
      dicom_stl::pair<const dicom_stl::string, dicom_stl::string>(
        this->CurrentSeriesUID, newStdString));
  }
...
void DICOMAppHelper::OutputSeries()
{
  ...
  for (DICOMAppHelperImplementation::SeriesUIDToInstanceUIDMapType::iterator iter =
         this->Implementation->SeriesUIDToInstanceUIDMap.begin();
       iter != this->Implementation->SeriesUIDToInstanceUIDMap.end(); iter++)
  {
    ...
    {
      DICOMAppHelperImplementation::InstanceUIDToSliceOrderingMapType::iterator sn_iter =
        Implementation->InstanceUIDToSliceOrderingMap.find(*v_iter);
  ...
  DICOMParser* parser, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  ...
  if (it == Implementation->InstanceUIDToSliceOrderingMap.end())
  {
    ...
    // insert into the map
    this->Implementation->InstanceUIDToSliceOrderingMap.insert(
      dicom_stl::pair<const dicom_stl::string, DICOMOrderingElements>(this->InstanceUID, ord));
  }
  else
  {
  ...
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  ...
  if (it == Implementation->InstanceUIDToSliceOrderingMap.end())
  {
    ...
    if (val)
    {
      sscanf((char*)(val), "%f\\%f\\%f", &ord.ImagePositionPatient[0], &ord.ImagePositionPatient[1],
  ...
  else
  {
    ...
    {
      // file found, add new values
      sscanf((char*)(val), "%f\\%f\\%f", &(*it).second.ImagePositionPatient[0],
  ...
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  ...
  if (it == Implementation->InstanceUIDToSliceOrderingMap.end())
  {
    ...
    if (val)
    {
      sscanf((char*)(val), "%f\\%f\\%f\\%f\\%f\\%f", &ord.ImageOrientationPatient[0],
        &ord.ImageOrientationPatient[1], &ord.ImageOrientationPatient[2],
        &ord.ImageOrientationPatient[3], &ord.ImageOrientationPatient[4],
  ...
  else
  {
    ...
    if (val)
    {
      sscanf((char*)(val), "%f\\%f\\%f\\%f\\%f\\%f", &(*it).second.ImageOrientationPatient[0],
...
void DICOMAppHelper::BitsAllocatedCallback(DICOMParser* parser, doublebyte, doublebyte,
  DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (len == 0)
  {
...
void DICOMAppHelper::WidthCallback(DICOMParser* parser, doublebyte, doublebyte,
  DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
...
void DICOMAppHelper::HeightCallback(DICOMParser* parser, doublebyte, doublebyte,
  DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  unsigned short uival = 0;
...
void DICOMAppHelper::PixelRepresentationCallback(DICOMParser* parser, doublebyte, doublebyte,
  DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
...
void DICOMAppHelper::ROIContourSequenceCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char*, quadbyte)
{
...
void DICOMAppHelper::ContourSequenceCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char*, quadbyte)
{
...
void DICOMAppHelper::ContourGeometricTypeCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char*, quadbyte)
{
...
void DICOMAppHelper::ContourDataCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* data, quadbyte len)
{
  ...
  {
    dicom_stream::cerr << "DICOMAppHelper:: Found contour data tag (0x3006, 0x0050) without a "
                          "matching contour sequence tag (0x3006, 0x0040)."
                       << dicom_stream::endl;
  }
  ...
  // points.
  else if (this->Implementation->SeriesUIDToContoursMap[this->CurrentSeriesUID].back().size() ==
    0) // Note the test for equality
  {
    dicom_stream::cerr << "DICOMAppHelper:: Found contour data tag (0x3006, 0x0050) without a "
                          "matching number of contour points tag (0x3006, 0x0046)."
                       << dicom_stream::endl;
  }
  else
  {
    ...
    DICOMAppHelperImplementation::ContourType& contour =
      this->Implementation->SeriesUIDToContoursMap[this->CurrentSeriesUID].back();
...
void DICOMAppHelper::NumberOfContourPointsCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* data, quadbyte)
{
  ...
  {
    dicom_stream::cerr
      << "DICOMAppHelper:: Found number of contour points tag (0x3006, 0x0046) without a matching "
         "contour sequence tag (0x3006, 0x0040)."
      << dicom_stream::endl;
  }
  ...
  {
    dicom_stream::cerr << "DICOMAppHelper:: Found number of contour points tag (0x3006, 0x0046) "
                          "without a matching contour geometric type tag (0x3006, 0x0042)."
                       << dicom_stream::endl;
  }
  else
  {
...
void DICOMAppHelper::PatientIDCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::PatientSexCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::PatientAgeCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::PatientDOBCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::StudyDescriptionCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::BodyPartCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
  ...
  // Put the series description into the internal database mapping series UIDs
  // to InstanceUIDs
  DICOMAppHelperImplementation::SeriesUIDToBodyPartMapType::iterator iiter =
    this->Implementation->SeriesUIDToBodyPartMap.find(this->CurrentSeriesUID);
  if (iiter == this->Implementation->SeriesUIDToBodyPartMap.end()) // if not found we insert
  {
    this->Implementation->SeriesUIDToBodyPartMap.insert(
      dicom_stl::pair<const dicom_stl::string, dicom_stl::string>(
        this->CurrentSeriesUID, newStdString));
  }
...
void DICOMAppHelper::NumberOfSeriesInStudyCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::NumberOfStudyRelatedSeriesCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::StudyDateCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::ModalityCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::ManufacturerCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::InstitutionCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::ModelCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
...
void DICOMAppHelper::ScanOptionsCallback(
  DICOMParser*, doublebyte, doublebyte, DICOMParser::VRTypes, unsigned char* val, quadbyte len)
{
  if (val)
  {
  ...
  // Put the series description into the internal database mapping series UIDs
  // to InstanceUIDs
  DICOMAppHelperImplementation::SeriesUIDToScanOptionsMapType::iterator iiter =
    this->Implementation->SeriesUIDToScanOptionsMap.find(this->CurrentSeriesUID);
  if (iiter == this->Implementation->SeriesUIDToScanOptionsMap.end()) // if not found we insert
  {
    this->Implementation->SeriesUIDToScanOptionsMap.insert(
      dicom_stl::pair<const dicom_stl::string, dicom_stl::string>(
        this->CurrentSeriesUID, newStdString));
  }
...
dicom_stl::string DICOMAppHelper::GetFileName(const dicom_stl::string& instanceUID)
{
  dicom_stl::string ret("");

  DICOMAppHelperImplementation::InstanceUIDToFileNameMapType::iterator it =
    this->Implementation->InstanceUIDToFileNameMap.find(instanceUID);
  ...
  dicom_stl::vector<dicom_stl::pair<int, dicom_stl::string>>& v, bool ascending)
{
  ...
  DICOMAppHelperImplementation::SeriesUIDToInstanceUIDMapType::iterator miter =
    this->Implementation->SeriesUIDToInstanceUIDMap.find(seriesUID);
  ...
  dicom_stl::vector<dicom_stl::pair<int, dicom_stl::string>>& v, bool ascending)
{
  ...
  if (this->Implementation->SeriesUIDToInstanceUIDMap.size() > 0)
  {
    this->GetSliceNumberFilenamePairs(
      (*this->Implementation->SeriesUIDToInstanceUIDMap.begin()).first, v, ascending);
  }
  else
  {
    v.clear();
...
void DICOMAppHelper::GetContours(
  const dicom_stl::string& seriesUID, dicom_stl::vector<dicom_stl::vector<float>>& v)
{
  v.clear();

  DICOMAppHelperImplementation::SeriesUIDToContoursMapType::iterator iter =
    this->Implementation->SeriesUIDToContoursMap.find(seriesUID);
...
void DICOMAppHelper::GetReferencedInstanceUIDs(
  const dicom_stl::string& seriesUID, dicom_stl::vector<dicom_stl::string>& v)
{
  v.clear();

  DICOMAppHelperImplementation::SeriesUIDToReferencedInstanceUIDMapType::iterator iter =
    this->Implementation->SeriesUIDToReferencedInstanceUIDMap.find(seriesUID);
...
void DICOMAppHelper::GetReferencedInstanceUIDs(dicom_stl::vector<dicom_stl::string>& v)
{
  ...
  if (this->Implementation->SeriesUIDToInstanceUIDMap.size() > 0)
  {
    this->GetReferencedInstanceUIDs(
      (*this->Implementation->SeriesUIDToReferencedInstanceUIDMap.begin()).first, v);
  }
  else
  {
    v.clear();
...
void DICOMAppHelper::GetSeriesUIDs(dicom_stl::vector<dicom_stl::string>& v)
{
  ...
  for (miter = this->Implementation->SeriesUIDToInstanceUIDMap.begin();
       miter != this->Implementation->SeriesUIDToInstanceUIDMap.end(); ++miter)
  {
...
void DICOMAppHelper::GetSeriesDescriptions(dicom_stl::vector<dicom_stl::string>& v)
{
  ...
  {
    DICOMAppHelperImplementation::SeriesUIDToSeriesDescriptionMapType::iterator iiter =
      this->Implementation->SeriesUIDToSeriesDescriptionMap.find((*miter).first);
...
void DICOMAppHelper::GetBodyParts(dicom_stl::vector<dicom_stl::string>& v)
{
  ...
  {
    DICOMAppHelperImplementation::SeriesUIDToBodyPartMapType::iterator iiter =
      this->Implementation->SeriesUIDToBodyPartMap.find((*miter).first);
...
void DICOMAppHelper::GetScanOptions(dicom_stl::vector<dicom_stl::string>& v)
{
  ...
  {
    DICOMAppHelperImplementation::SeriesUIDToScanOptionsMapType::iterator iiter =
      this->Implementation->SeriesUIDToScanOptionsMap.find((*miter).first);
