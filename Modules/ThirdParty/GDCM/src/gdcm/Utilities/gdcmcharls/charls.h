...
//   When building with default visibility set to hidden, we must explicitly
//   change the visibility of public API functions
#    define CHARLS_IMEXPORT(returntype) __attribute__((visibility("default"))) returntype __stdcall
#  else
#    define CHARLS_IMEXPORT(returntype) extern returntype __stdcall
#  endif
#endif
...
CHARLS_IMEXPORT(CharlsApiResultType) JpegLsEncode(void* destination, size_t destinationLength, size_t* bytesWritten,
                                                  const void* source, size_t sourceLength,
...
CHARLS_IMEXPORT(CharlsApiResultType) JpegLsDecodeRect(void* uncompressedData, size_t uncompressedLength,
                                                      ...
                                                      struct JlsRect rect, struct JlsParameters* params,
                                                      char* errorMessage);
...
CHARLS_IMEXPORT(CharlsApiResultType) JpegLsEncodeStream(ByteStreamInfo rawStream, size_t& bytesWritten,
                                                        ByteStreamInfo inputStream, const JlsParameters& info,
                                                        char* errorMessage);
CHARLS_IMEXPORT(CharlsApiResultType) JpegLsDecodeStream(ByteStreamInfo output, ByteStreamInfo input,
                                                        const JlsParameters* info, char* errorMessage);
CHARLS_IMEXPORT(CharlsApiResultType) JpegLsReadHeaderStream(ByteStreamInfo input, JlsParameters* params,
                                                            char* errorMessage);
